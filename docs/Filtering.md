...
```
```
filtergenotypes -c 'g.ad.at(1).toDouble / g.dp < 0.2' --remove
```
```
...
**Supported comparisons and transformations:**
 ...
 - String operations: 
   - at: `str.at(index)` -- returns the character at `index`
   ...
   - split: `str.split(delimiter, limit)` -- Functions as `split` above, but will split on the given delimiter to generate at most `limit` elements in the array.  `"1,2,3,4,5".split(",")` would produce `Array("1", "2", "3", "4", "5")`, but `"1,2,3,4,5".split(",", 2)` results in `Array("1", "2,3,4,5")`
 - String conversions:
   - toInt: `str.toInt`
   ...
   - toFloat: `str.toFloat`
 - Array Operations:
   - at: `arr.at(index)` -- get a value from the array, or NA if array or index is missing
   - contains: `arr.contains(elem)` -- returns true if the element is contained in the array, otherwise false
   - sameElements: `arr.sameElements(otherArray)` -- returns true if each element in the two arrays is equivalent
   - size/length: `arr.size` or `arr.length` -- returns the length of the array as an integer
