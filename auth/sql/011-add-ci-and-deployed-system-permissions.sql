...
INSERT INTO `system_permissions` (`name`) VALUES ('read_deployed_system_status');
INSERT INTO `system_permissions` (`name`) VALUES ('update_deployed_system_status');

-- Assign these permissions to the developer role
...
INSERT INTO `system_role_permissions` (`role_id`, `permission_id`) 
SELECT r.id, p.id FROM `system_roles` r, `system_permissions` p 
WHERE r.name = 'developer' AND p.name = 'read_deployed_system_status';

INSERT INTO `system_role_permissions` (`role_id`, `permission_id`) 
SELECT r.id, p.id FROM `system_roles` r, `system_permissions` p 
WHERE r.name = 'developer' AND p.name = 'update_deployed_system_status';
