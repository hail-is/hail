...
print('Fetching "ica_bone_marrow_h5.h5"...')
response = requests.get(f'{URL_ROOT}/ica_bone_marrow_h5.h5')
with open('/tmp/ica_bone_marrow_h5.h5', 'wb') as f:
    f.write(response.content)

f = h5py.File('/tmp/ica_bone_marrow_h5.h5', 'r')

with open('/tmp/ica_bone_marrow.header.tsv', 'w') as f_out:
...
for chunk in range(n_chunks):
    ...
    df.to_csv(f'/tmp/ica_bone_marrow.chunk_{chunk}.tsv', sep='\t', float_format='%i', index=True, header=False)
    chunk_start = i + 1
    chunk_end = min(i + 1 + chunk_size, n_cells + 1)

print('Concatenating chunks...')
with open('/tmp/ica_bone_marrow.tsv', 'w') as f_out:
    call(['cat', '/tmp/ica_bone_marrow.header.tsv', '/tmp/ica_bone_marrow.chunk_*.tsv'], stdout=f_out)

print('Block compressing concatenated file...')
call(['bgzip', '/tmp/ica_bone_marrow.tsv'])

print('Copying block compressed file...')
call(['gsutil', 'cp', '/tmp/ica_bone_marrow.tsv.gz',
      'gs://hail-datasets-raw-data/Human_Cell_Atlas/Human_Cell_Atlas_immune_cells_bone_marrow_raw_counts.tsv.bgz'])

print('Removing temporary files...')
call(['rm', '/tmp/ica_bone_marrow*'])
