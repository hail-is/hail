...
This functionality and more is exposed through **[Python](https://www.python.org/)** and backed by distributed algorithms built on top of **[Apache Spark](http://spark.apache.org/)** to efficiently analyze gigabyte-scale data on a laptop or terabyte-scale data on a cluster, without the need to manually chop up data or manage job failures. Hail also provides a flexible [domain language](https://hail.is/hail/types.html) to express complex quality control and analysis pipelines with concise, readable code.
