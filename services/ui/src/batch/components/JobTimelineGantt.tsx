...
const STEP_COLOR_MAP = new Map<string, string>([
...
function buildGanttRows(job: Job, attempts: Attempt[], showPriorAttempts: boolean): GanttData {
  ...
  if (latest?.start_time_ms != null) {
    ...
    const containerEntries = new Map<string, ContainerStatus | null | undefined>([
      ['input', statuses.input],
      ['main', statuses.main],
    ...
    for (const [, cs] of containerEntries) {
      if (!cs) continue;
    ...
    for (const [container, cs] of containerEntries) {
      if (!cs) continue;
...
function buildColorMap(rows: GanttRow[]): Map<string, string> {
  const map = new Map<string, string>();
  for (const cat of new Set(rows.map((r) => r.category))) {
    map.set(cat, STEP_COLOR_MAP.get(cat) ?? '#9ca3af');
  }
  return map;
}
