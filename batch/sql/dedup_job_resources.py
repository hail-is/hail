...
async def process_chunk_agg_job_resources(counter, db, start, end, quiet=True):
    if start is None:
        assert end is not None
        end_batch_id, end_job_id, end_resource_id = end

        where_statement = '''
WHERE (aggregated_job_resources_v2.batch_id < %s) OR
      (aggregated_job_resources_v2.batch_id = %s AND aggregated_job_resources_v2.job_id < %s) OR
      (aggregated_job_resources_v2.batch_id = %s AND aggregated_job_resources_v2.job_id = %s AND aggregated_job_resources_v2.resource_id < %s)
'''
        query_args = [end_batch_id,
                      end_batch_id, end_job_id,
                      end_batch_id, end_job_id, end_resource_id,
                      ]
    elif end is None:
        assert start is not None
        start_batch_id, start_job_id, start_resource_id = start
...
'''
        query_args = [start_batch_id,
                      ...
                      start_batch_id, start_job_id, start_resource_id,
                      ]
    else:
...
WHERE ((aggregated_job_resources_v2.batch_id > %s) OR
       (aggregated_job_resources_v2.batch_id = %s AND aggregated_job_resources_v2.job_id > %s) OR
       (aggregated_job_resources_v2.batch_id = %s AND aggregated_job_resources_v2.job_id = %s AND aggregated_job_resources_v2.resource_id >= %s))
  AND ((aggregated_job_resources_v2.batch_id < %s) OR
       (aggregated_job_resources_v2.batch_id = %s AND aggregated_job_resources_v2.job_id < %s) OR
       (aggregated_job_resources_v2.batch_id = %s AND aggregated_job_resources_v2.job_id = %s AND aggregated_job_resources_v2.resource_id < %s))
'''
        query_args = [start_batch_id,
...
async def audit_changes(db):
    ...
    async def _process_audit_chunk(db, start, end):
        if start is not None and end is not None:
            start_batch_id, start_job_id = start
            end_batch_id, end_job_id = end
            where_statement = '''
WHERE ((batch_id > %s) OR (batch_id = %s AND job_id >= %s))
  AND ((batch_id < %s) OR (batch_id = %s AND job_id < %s))
        '''
            where_args = [start_batch_id, start_batch_id, start_job_id,
                          end_batch_id, end_batch_id, end_job_id]
        elif start is None and end is not None:
            end_batch_id, end_job_id = end
            where_statement = 'WHERE (batch_id < %s) OR (batch_id = %s AND job_id < %s)'
            where_args = [end_batch_id, end_batch_id, end_job_id]
        else:
        ...
        bad_job_records = db.select_and_fetchall(
            f'''
SELECT old.batch_id, old.job_id, old.deduped_resource_id, old.`usage`, new.`usage`, ABS(new.`usage` - old.`usage`) AS usage_diff
FROM (
  SELECT batch_id, job_id, deduped_resource_id, CAST(COALESCE(SUM(`usage`), 0) AS SIGNED) AS `usage`
  FROM aggregated_job_resources_v2
  LEFT JOIN resources ON resources.resource_id = aggregated_job_resources_v2.resource_id
  {where_statement}
...
''',
        ...
        for record in bad_job_records:
            ...
            failing_jobs.append((record['batch_id'], record['job_id']))
...
async def find_chunk_offsets(db, size):
    ...
    async def _find_chunks(tx) -> List[Optional[Tuple[int, int, str]]]:
        ...
        query = f'''
SELECT t.batch_id, t.job_id, t.resource_id FROM (
  SELECT batch_id, job_id, resource_id
  FROM aggregated_job_resources_v2
  ORDER BY batch_id ASC, job_id ASC, resource_id ASC
) AS t
WHERE MOD((@rank := @rank + 1), %s) = 0;
'''

        offsets = tx.execute_and_fetchall(query, (size,))
        offsets = [(offset['batch_id'], offset['job_id'], offset['resource_id']) async for offset in offsets]
        offsets.append(None)
...
SELECT t.batch_id, t.job_id FROM (
  ...
  FROM aggregated_job_resources_v2
  ORDER BY batch_id ASC, job_id ASC
) AS t
WHERE MOD((@rank := @rank + 1), %s) = 0;
'''
...
async def run_migration(db, chunk_size):
    ...
    if chunk_offsets != [(None, None)]:
        ...
        for start_offset, end_offset in burn_in_chunk_offsets:
            await process_chunk_agg_job_resources(chunk_counter, db, start_offset, end_offset, quiet=False)
        ...
        await bounded_gather(
            *[functools.partial(process_chunk_agg_job_resources, chunk_counter, db, start_offset, end_offset, quiet=False)
              for start_offset, end_offset in chunk_offsets],
            parallelism=10
        )
        print(
