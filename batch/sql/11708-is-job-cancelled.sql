...
BEGIN
    ...
    SELECT NOT j.always_run AND (j.cancelled OR c.cancelled IS NOT NULL) INTO cancelled
    FROM jobs AS j
    LEFT JOIN LATERAL (
...
BEGIN
  ...
  SET job_group_cancelled =
    is_job_group_cancelled(NEW.batch_id, NEW.job_group_id)
    FOR SHARE;
...
BEGIN
  ...
  SET cur_job_group_cancelled =
    is_job_group_cancelled(OLD.batch_id, OLD.job_group_id)
    FOR SHARE;
...
BEGIN
  ...
  SET cur_cancelled =
    is_job_group_cancelled(in_batch_id, in_job_group_id)
    FOR UPDATE;
...
BEGIN
  ...
  SET cur_cancelled =
    is_batch_cancelled(in_batch_id)
    FOR UPDATE;
...
BEGIN
  ...
  SET cur_job_cancel =
    is_job_cancelled(in_batch_id, in_job_id)
    FOR SHARE;
