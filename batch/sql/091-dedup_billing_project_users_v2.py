...
async def process_chunk_agg_bp_user_resources(counter, db, start, end, quiet=True):
    if start is None:
        assert end is not None
        end_billing_project, end_user, end_resource_id = end

        where_statement = '''
WHERE (aggregated_billing_project_user_resources_v2.billing_project < %s) OR
      (aggregated_billing_project_user_resources_v2.billing_project = %s AND aggregated_billing_project_user_resources_v2.`user` < %s) OR
      (aggregated_billing_project_user_resources_v2.billing_project = %s AND aggregated_billing_project_user_resources_v2.`user` = %s AND aggregated_billing_project_user_resources_v2.resource_id < %s)
'''
        query_args = [end_billing_project,
                      end_billing_project, end_user,
                      end_billing_project, end_user, end_resource_id,
                      ]
    elif end is None:
        assert start is not None
        start_billing_project, start_user, start_resource_id = start
...
WHERE (aggregated_billing_project_user_resources_v2.billing_project > %s) OR
      ...
      (aggregated_billing_project_user_resources_v2.billing_project = %s AND aggregated_billing_project_user_resources_v2.`user` = %s AND aggregated_billing_project_user_resources_v2.resource_id >= %s)
'''
        query_args = [start_billing_project,
                      start_billing_project, start_user,
                      start_billing_project, start_user, start_resource_id,
                      ]
    else:
...
WHERE ((aggregated_billing_project_user_resources_v2.billing_project > %s) OR
       (aggregated_billing_project_user_resources_v2.billing_project = %s AND aggregated_billing_project_user_resources_v2.`user` > %s) OR
       (aggregated_billing_project_user_resources_v2.billing_project = %s AND aggregated_billing_project_user_resources_v2.`user` = %s AND aggregated_billing_project_user_resources_v2.resource_id >= %s))
  AND ((aggregated_billing_project_user_resources_v2.billing_project < %s) OR
       (aggregated_billing_project_user_resources_v2.billing_project = %s AND aggregated_billing_project_user_resources_v2.`user` < %s) OR
       (aggregated_billing_project_user_resources_v2.billing_project = %s AND aggregated_billing_project_user_resources_v2.`user` = %s AND aggregated_billing_project_user_resources_v2.resource_id < %s))
'''
        query_args = [start_billing_project,
    ...
    if chunk_offsets != [(None, None)]:
        for offsets in chunk_offsets:
            ...
            if start is not None and end is not None:
                start_bp, start_user = start
                end_bp, end_user = end
                where_statement = '''
WHERE ((billing_project > %s) OR (billing_project = %s AND `user` >= %s))
  AND ((billing_project < %s) OR (billing_project = %s AND `user` < %s))
'''
                where_args = [start_bp, start_bp, start_user,
                              end_bp, end_bp, end_user]
            elif start is None and end is not None:
                end_bp, end_user = end
                where_statement = 'WHERE (billing_project < %s) OR (billing_project = %s AND `user` < %s)'
                where_args = [end_bp, end_bp, end_user]
            else:
...
LEFT JOIN (
  ...
  LEFT JOIN resources ON resources.resource_id = aggregated_billing_project_user_resources_v3.resource_id
  {where_statement}
  GROUP BY billing_project, `user`, deduped_resource_id
  LOCK IN SHARE MODE
) AS new ON old.billing_project = new.billing_project AND old.`user` = new.`user` AND old.deduped_resource_id = new.deduped_resource_id
...
async def find_chunk_offsets(db, size):
    ...
    async def _find_chunks(tx) -> List[Optional[Tuple[int, int, str]]]:
        ...
        query = f'''
SELECT t.billing_project, t.`user`, t.resource_id FROM (
  SELECT billing_project, `user`, resource_id
  FROM aggregated_billing_project_user_resources_v2
  ORDER BY billing_project ASC, `user` ASC, resource_id ASC
) AS t
WHERE MOD((@rank := @rank + 1), %s) = 0;
'''

        offsets = tx.execute_and_fetchall(query, (size,))
        offsets = [(offset['billing_project'], offset['user'], offset['resource_id']) async for offset in offsets]
        offsets.append(None)
...
async def find_chunk_offsets_for_audit(db, size):
    ...
    async def _find_chunks(tx) -> List[Optional[Tuple[int, int, str]]]:
        ...
        query = f'''
SELECT t.billing_project, t.`user` FROM (
  SELECT billing_project, `user`
  FROM aggregated_billing_project_user_resources_v2
  ORDER BY billing_project ASC, `user` ASC
) AS t
WHERE MOD((@rank := @rank + 1), %s) = 0;
'''

        offsets = tx.execute_and_fetchall(query, (size,))
        offsets = [(offset['billing_project'], offset['user']) async for offset in offsets]
        offsets.append(None)
...
async def run_migration(db, chunk_size):
    ...
    if chunk_offsets != [(None, None)]:
        ...
        for start_offset, end_offset in burn_in_chunk_offsets:
            await process_chunk_agg_bp_user_resources(chunk_counter, db, start_offset, end_offset, quiet=False)
