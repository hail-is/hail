...
async def get_batch_jobs_v1(request: web.Request, _, batch_id: int) -> web.Response:
    ...
    resp = await _handle_api_error(_get_jobs, request, batch_id, ROOT_JOB_GROUP_ID, 1, q, last_job_id, recursive=True)
...
async def get_batch_jobs_v2(request: web.Request, _, batch_id: int) -> web.Response:
    q = request.query.get('q', '')
    last_job_id = cast_query_param_to_int(request.query.get('last_job_id'))
    resp = await _handle_api_error(_get_jobs, request, batch_id, ROOT_JOB_GROUP_ID, 2, q, last_job_id, recursive=True)
    assert resp is not None
    return json_response(resp)
...
async def get_job_group_jobs_v1(request: web.Request, _, batch_id: int) -> web.Response:
    ...
    last_job_id = cast_query_param_to_int(request.query.get('last_job_id'))
    resp = await _handle_api_error(_get_jobs, request, batch_id, job_group_id, 1, q, last_job_id, recursive)
    assert resp is not None
    return json_response(resp)
...
async def get_job_group_jobs_v2(request: web.Request, _, batch_id: int) -> web.Response:
    ...
    recursive = cast_query_param_to_bool(request.query.get('recursive'))
    last_job_id = cast_query_param_to_int(request.query.get('last_job_id'))
    resp = await _handle_api_error(_get_jobs, request, batch_id, job_group_id, 2, q, last_job_id, recursive)
    assert resp is not None
    return json_response(resp)
...
@auth.authenticated_users_only()
@add_metadata_to_request
async def create_job_groups_for_update(request: web.Request, userdata: UserData) -> web.Response:
    app = request.app
    db: Database = app['db']
    user = userdata['username']
    ...
    return await _create_job_groups(db, batch_id, update_id, user, job_group_specs)
...
''',
    ...
    for spec in job_specs:
        ...
        if 'absolute_job_group_id' in spec:
            absolute_job_group_id = spec.pop('absolute_job_group_id')
            assert 'in_update_job_group_id' not in spec
            job_group_id = absolute_job_group_id
        else:
            in_update_job_group_id = spec.pop('in_update_job_group_id')
            assert 'absolute_job_group_id' not in spec
            job_group_id = (
                update_start_job_group_id + in_update_job_group_id
            )  # Note: the first jg id in an update is always 0!
        ...
        if job_group_id != ROOT_JOB_GROUP_ID:
            raise web.HTTPBadRequest(reason='child job groups are not supported')
...
async def create_batch_fast(request, userdata):
    ...
    update_id, _, _ = await _create_batch_update(
        batch_id, batch_spec['token'], batch_spec['n_jobs'], batch_spec['n_job_groups'], user, db
    )
...
async def create_batch(request, userdata):
    ...
    n_jobs = batch_spec['n_jobs']
    n_job_groups = batch_spec['n_job_groups']
    if n_jobs > 0 or n_job_groups > 0:
        update_id, _, _ = await _create_batch_update(
            id, batch_spec['token'], batch_spec['n_jobs'], batch_spec['n_job_groups'], userdata['username'], db
        )
    else:
        update_id = None
...
''',
        ...
        validate_job_groups(specs, start_job_group_id)
...
async def _create_job_group(
    ...
    *,
    ...
    timestamp: int,
    n_jobs: int,
    parent_job_group_id: Optional[int],
):
...
''',
    ...
    # Note that commit_batch_update will cleanup any missing ancestor dependencies
    # due to unordered job group inserts via parallel job group creation requests
    if parent_job_group_id is not None:
        await tx.execute_update(
            '''
...
async def update_batch_fast(request, userdata):
    ...
    update_id, start_job_id, start_job_group_id = await _create_batch_update(
        batch_id, update_spec['token'], update_spec['n_jobs'], update_spec['n_job_groups'], user, db
    )
...
''',
        ...
        if record:
            ...
            update_start_job_group_id = ROOT_JOB_GROUP_ID

        await tx.execute_insertone(
            '''
INSERT INTO batch_updates
(batch_id, update_id, token, start_job_id, start_job_group_id, n_jobs, n_job_groups, committed, time_created)
