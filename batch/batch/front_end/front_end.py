...
async def _get_job_log(app, batch_id, job_id) -> Dict[str, Optional[PossiblyTruncatedLog]]:
    ...
    async def load_up_to_fifty_kib(app, batch_id, job_id, c, record) -> Optional[PossiblyTruncatedLog]:
        ...
        fifty_kib = 50 * 1024
        async with s:
            log = await s.read_until_n_or_eof(fifty_kib + 1)
        if len(log) == fifty_kib + 1:
            return (log[:fifty_kib], True)

        return (log, False)

    logs = await asyncio.gather(*(load_up_to_fifty_kib(app, batch_id, job_id, c, record) for c in containers))
...
async def get_job_container_log(request: web.Request, batch_id: int) -> web.StreamResponse:
    app = request.app
    job_id = int(request.match_info['job_id'])
...
@billing_project_users_only()
@add_metadata_to_request
async def rest_get_job_container_log(request: web.Request, _, batch_id: int) -> web.StreamResponse:
