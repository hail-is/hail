...
GROUP BY batch_id, job_group_id, job_id, attempt_id
LOCK IN SHARE MODE;
'''
        )
        ...
        agg_job_resources = tx.execute_and_fetchall(
            '''
SELECT batch_id, job_group_id, job_id, JSON_OBJECTAGG(resource, `usage`) as resources
FROM aggregated_job_resources_v3
LEFT JOIN jobs ON aggregated_job_resources_v3.batch_id = jobs.batch_id AND aggregated_job_resources_v3.job_group_id = jobs.job_group_id
LEFT JOIN resources ON aggregated_job_resources_v3.resource_id = resources.resource_id
GROUP BY batch_id, job_group_id, job_id
LOCK IN SHARE MODE;
'''
        )
...
JOIN job_groups ON job_groups.batch_id = t.batch_id
