...
from ..utils import parse_cpu_in_mcpu, parse_memory_in_bytes
...
class Instance:
    ...
    def from_record(app, record):
        return Instance(
            ...
            json.loads(record['resources']))

    @staticmethod
    async def create(app, name, activation_token, worker_cores_mcpu, resources):
        db = app['db']
...
INSERT INTO instances (name, state, activation_token, token, cores_mcpu,
                       free_cores_mcpu, time_created, last_updated, resources)
VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s);
''',
            (name, state, activation_token, token, worker_cores_mcpu,
             worker_cores_mcpu, now, now, json.dumps(resources)))
        return Instance(
    ...
    def get_resources(self, spec):
        cores_mcpu = parse_cpu_in_mcpu(spec['cpu'])
        memory_mb = int(parse_memory_in_bytes(spec['memory']) / 1024**2 + 0.5)

        result = {}
        for resource in self.resources:
            if resource == 'preemptible-n1-predefined-core':
                result[resource] = cores_mcpu
            else:
                assert resource == 'preemptible-n1-predefined-ram'
                result[resource] = memory_mb
        return result
