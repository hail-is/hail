...
class API():
    ...
    def http(self, verb, url, json=None, timeout=None, cookies=None, headers=None):
        if timeout is None:
        ...
        if json is not None:
            response = verb(url, json=json, timeout=timeout, cookies=cookies, headers=headers)
        else:
            response = verb(url, timeout=timeout, cookies=cookies, headers=headers)
