...
async def create_container(config, name):
    ...
    while True:
        try:
            ...
            log.exception(f'while creating container', exc_info=True)
...
async def start_container(container):
    try:
        ...
        log.exception(f'while starting container', exc_info=True)
...
async def stop_container(container):
    try:
        ...
        log.exception(f'while stopping container', exc_info=True)
...
async def delete_container(container, *args, **kwargs):
    try:
        ...
        log.exception(f'while deleting container', exc_info=True)
    ...
    async def run(self, worker):
        try:
            ...
            async with self.step('creating'):
                ...
                async with worker.create_container_sem:
...
class Worker:
    def __init__(self):
        ...
        self.create_container_sem = asyncio.BoundedSemaphore(2)
