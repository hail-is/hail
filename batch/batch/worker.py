...
def copy_command(src, dst):
    return f'{shq(src)} {shq(dst)}'


def copy(files):
    assert files
    copies = '\n'.join([copy_command(f['from'], f['to']) for f in files])
    return f'''
cat | python3 /batch/copy_files.py --key-file /gsa-key/key.json --project {PROJECT} --parallelism 10 <<EOF
{copies}
EOF
'''
