...
ht_genes = ht_genes.annotate(gene_interval=hl.interval(hl.locus(ht_genes.contig, ht_genes.start, 'GRCh37'), hl.locus(ht_genes.contig, ht_genes.end + 1, 'GRCh37')))
...
ht_genes = ht_genes.select(*(['gene_id', 'gene_interval', 'gene_type', 'strand', 'annotation_source', 'havana_gene', 'gene_status', 'tag']))
ht_genes = ht_genes.rename({'havana_gene': 'havana_gene_id'})
ht_genes = ht_genes.key_by(ht_genes.gene_id)

mt = hl.import_matrix_table('gs://hail-datasets/raw-data/gtex/v7/rna-seq/processed/GTEx_Analysis_2016-01-15_v7_STARv2.4.2a_junctions.tsv.bgz',
                            row_fields={'junction_id': hl.tstr, 'Description': hl.tstr}, row_key='junction_id', missing=' ', entry_type=hl.tfloat)

mt = mt.annotate_cols(sample_id=mt.col_id)
mt = mt.key_cols_by(mt.sample_id)

mt = mt.annotate_rows(contig=mt.junction_id.split('_')[0], start=hl.int(mt.junction_id.split('_')[1]), end=hl.int(mt.junction_id.split('_')[2]) + 1)
mt = mt.annotate_rows(junction_interval=hl.interval(hl.locus(mt.contig, mt.start, 'GRCh37'), hl.locus(mt.contig, mt.end, 'GRCh37')), gene_id=mt.Description)
mt = mt.key_rows_by(mt.junction_interval)

mt = mt.annotate_entries(read_count=hl.int(mt.x))
mt = mt.drop(mt.col_id, mt.junction_id, mt.Description, mt.x, mt.contig, mt.start, mt.end)

mt = mt.annotate_cols(**ht_samples[mt.sample_id])
mt = mt.annotate_rows(**ht_genes[mt.gene_id])

mt.describe()
mt.write('gs://hail-datasets/hail-data/gtex_v7_junction_read_counts.GRCh37.mt', overwrite=True)
