...
**Before adding a new fetch, check whether the page already has the data.** The same value is often already sitting in state somewhere else on the page — under a different endpoint, a different field name, or fetched by a sibling component — and can be derived/filtered client-side for free instead of triggering another request. This is easy to miss because the duplication isn't always obvious (see the `useBatchData` bullets below for three real, non-obvious examples caught only after the redundant fetch was already written). When adding a data-fetching hook or effect, spend a minute checking the page's existing state/props for something that already contains what you need before reaching for a new request.

**A caveat on trusting `/openapi.yaml`:** the schema and the actual handler can drift, and it's not always the schema that's stale — sometimes the handler is missing a field the schema (correctly) already documents. Two real examples found in `services/ui/src/batch/components/useBatchData.ts`'s history: `JobResponse.spec` documented `parent_ids` that the handler deliberately strips before persisting (never trust that field); `JobGroupDetailResponse` documented a `children` field the handler never sets at all (a different, real endpoint — `GET .../job-groups/{id}/job-groups` — is the correct way to list a job group's children). Conversely, `JobListEntryV1Alpha`'s `job_group_id` field was correctly declared in the TypedDict but silently missing from the dict the handler actually returned — a real bug, fixed by adding the field, not by changing the type to match broken behavior. When a schema field doesn't behave as documented, read the handler in `batch/batch/front_end/front_end.py` (or the relevant service) to find out which side is wrong before building against it.

### Data fetching for a batch: use `useBatchData`, not a per-page fetch

For any page or component that needs a batch's status, its full job list, or its job-group hierarchy, use `services/ui/src/batch/components/useBatchData.ts` — don't write a page-local `fetch`/`useState`/`useEffect` trio for these. It exists specifically because that pattern was hand-rolled once (on the CI PR page, before job groups were even involved) and immediately needed non-trivial fixes that a single shared hook gets right once instead of N times:

- **Derive `rootJobGroup` from `batchStatus` instead of fetching `GET .../job-groups/0` separately.** A batch's own aggregate counts (`n_jobs`/`n_completed`/etc.) *are* its root job group's counts — there's no separate row to fetch.
- **Filter the already-fetched job list client-side instead of re-fetching per job group.** `GET .../batches/{id}/jobs` already returns every job recursively (root job group, `recursive=True`); a job group's own jobs are just `jobs.filter(j => j.job_group_id === id)`, no extra request.
- **Cache job-group children keyed by `job_group_id`, not tied to a component's mount lifecycle.** A naive "fetch in `useEffect` on mount" inside a collapsible row re-fetches every time the row unmounts/remounts — which, notably, includes every single collapse if the collapsible component (`CollapsibleItem`) conditionally unmounts its content on close, not just the component's first mount. Keep fetched results in state that outlives the row (a hook like this one, or at minimum a parent that never unmounts), and use `CollapsibleItem`'s `onExpand` callback to trigger a fetch lazily on first expand rather than eagerly for every rendered row.
- **Don't gate data that's already known on a fetch for data that isn't.** Whether a job group *has* sub-groups requires a request; which jobs it directly owns doesn't (see above) — render the second immediately rather than waiting on the first just because they're displayed in the same place.

`useBatchData` owns data fetching and caching only — it has no opinion on *when* to poll. A page keeps owning its own auto-refresh policy (a toggle, `setInterval`, a countdown UI) and calls the hook's `refresh()` on whatever cadence it wants; see `pr.tsx`'s `PrPage` for the pattern (`useBatchData` + a thin auto-refresh `useEffect` on top).

This hook deliberately hand-rolls its own caching/dedup rather than using a data-fetching library (TanStack Query, SWR) — there isn't one in `services/ui` today. If this pattern needs to be replicated for a third domain beyond jobs/job-groups, or a third page beyond CI's PR page and the eventual batch-details page, that's the point to reconsider pulling one in rather than hand-rolling a fourth cache.
