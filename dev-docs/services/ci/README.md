...
Not all steps in `build.yaml` run in every generated batch:

##### Scope based filtering:

Runs generated by CI have purposes, which define their scope:
- `dev` for dev namespace deployments
- `deploy` for production namespace deployments
- `test` for CI test runs

Each step in `build.yaml` can declare which scope or scopes it is allowed to run in. If a step has no explicit list of allowed scopes, it will run in all scopes.

##### File Based Step Selection:

PR `test` batches further filter which steps are worth running based on the list of files changed:

- First, CI will determine the files changed by the PR and compare that to the inputs declared for each step in `build.yaml`.
- For each step that is directly affected by the files changed, the step will be added
- Next, all downstream steps are included, since their results might also depend on the changes in the PR.
- Finally, all dependencies of all steps are included, since they are required in order to run the required
steps.


##### Steps to jobs mapping:

When a list of steps to run is selected, they are mapped into jobs to include in the final batch.
Note that this mapping is not quite 1:1. For example, several test steps shard out into multiple jobs. 
And some deployment steps include cleanup jobs to always run, after the main job set has completed.

##### Exceptions

There are a few exceptions to the general step selection rules:
