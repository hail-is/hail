...
object CallStats {
  def makeSchema: (TStruct, TStruct) = {
    TStruct.applyWDocstring(
      ("AC", TArray(TInt), "Allele count. One element per allele **including reference**. There are two elements for a biallelic variant, or 4 for a variant with three alternate alleles."),
