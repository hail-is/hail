...
/**
  * Computes all eigenvalues (and optionally right eigenvectors) of the given
  * real symmetric matrix X using dsyevr (Relatively Robust Representations):
...
object eigSymR extends UFunc {
  ...
  private def doeigSymR(X: Matrix[Double], rightEigenvectors: Boolean): (DenseVector[Double], Option[DenseMatrix[Double]]) = {
    // assumes X is non-empty and symmetric, caller should check if necessary
    ...
    lapack.dsyevr(JOBZ, RANGE, UPLO, N, A, LDA, 0d, 0d, 0, 0, ABSTOL, M, W.data,
      Z.data, LDZ, ISUPPZ.data, WORK, LWORK, IWORK, LIWORK, info)
    // A value of info.`val` < 0 would tell us that the i-th argument
