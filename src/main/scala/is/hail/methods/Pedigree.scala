...
case class BaseTrio(kid: String, fam: Option[String], dad: Option[String], mom: Option[String],
  sex: Option[Sex], pheno: Option[Phenotype]) extends Trio {
      ...
      val trios = lines.filter(line => !line.value.isEmpty)
        .map {
          _.map { line => // FIXME: check that pedigree makes sense (e.g., cannot be own parent)
            val splitLine = line.split(delimiter) // FIXME: fails on names with spaces, will fix in PR for adding .fam to annotations by giving delimiter option
            if (splitLine.size != 6)
      ...
      Gen.shuffle(sampleIds)
        .map { is =>
          ...
          val trios = groups.map { g =>
            ...
            BaseTrio(kid, fam = None, mom = mom, dad = dad, sex = None, pheno = None)
          }
            .toArray
          new Pedigree(trios)
        }
...
case class Pedigree(trios: IndexedSeq[Trio]) {
  ...
  def phenotypedSamples: Set[String] = trios.iterator.filter(_.pheno.isDefined).map(_.kid).toSet
