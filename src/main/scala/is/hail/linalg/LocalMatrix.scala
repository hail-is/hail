...
class LocalMatrix(val m: BDM[Double]) {
  ...
  // writeBlockMatrix and BlockMatrix.read is safer than toBlockMatrix  FIXME: should have option via temp file?
  ...
  def filterRows(keep: Array[Int]) = LocalMatrix(m.filterRows(keep))

  def filterCols(keep: Array[Int]) = LocalMatrix(m.filterCols(keep))

  def filter(keepRows: Array[Int], keepCols: Array[Int]) = LocalMatrix(m.filter(keepRows, keepCols))
