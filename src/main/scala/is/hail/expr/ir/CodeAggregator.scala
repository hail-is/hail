...
class UnaryCodeAggregator[T, Agg <: RegionValueAggregator : ClassTag : TypeInfo, U : ClassTag]
  (in: Type, val stagedNew: (Code[_], Code[Boolean]) => Code[Agg], val out: Type) {
  def seqOp(rva: Code[RegionValueAggregator], v: Code[_], mv: Code[Boolean]): Code[Unit] =
    mv.mux(
...
class BinaryCodeAggregator[T, U, Agg <: RegionValueAggregator : ClassTag : TypeInfo, V : ClassTag]
  (in: Type, val stagedNew: (Code[_], Code[Boolean], Code[_], Code[Boolean]) => Code[Agg], val out: Type) {
  def seqOp(rva: Code[RegionValueAggregator], v: Code[_], mv: Code[Boolean]): Code[Unit] =
    mv.mux(
...
class TernaryCodeAggregator[T, U, V, Agg <: RegionValueAggregator : ClassTag : TypeInfo, W : ClassTag]
  (in: Type, val stagedNew: (Code[_], Code[Boolean], Code[_], Code[Boolean], Code[_], Code[Boolean]) => Code[Agg], val out: Type) {
  def seqOp(rva: Code[RegionValueAggregator], v: Code[_], mv: Code[Boolean]): Code[Unit] =
    mv.mux(
...
sealed trait UnaryCodeAggregatorCurried[T, U] {
  def apply[Agg <: RegionValueAggregator : ClassTag : TypeInfo]
    (stagedNew: (Code[T], Code[Boolean]) => Code[Agg], out: Type)
    (implicit uct: ClassTag[U], hrt: HailRep[U]): UnaryCodeAggregator[T, Agg, U] =
    new UnaryCodeAggregator(hailType[U], stagedNew.asInstanceOf[(Code[_], Code[Boolean]) => Code[Agg]], out)
}
...
sealed trait BinaryCodeAggregatorCurried[T, U, V] {
  def apply[Agg <: RegionValueAggregator : ClassTag : TypeInfo]
    (stagedNew: (Code[T], Code[Boolean], Code[U], Code[Boolean]) => Code[Agg], out: Type)
    (implicit uct: ClassTag[V], hrt: HailRep[V]): BinaryCodeAggregator[T, U, Agg, V] =
    new BinaryCodeAggregator(hailType[V], stagedNew.asInstanceOf[(Code[_], Code[Boolean], Code[_], Code[Boolean]) => Code[Agg]], out)
}
...
sealed trait TernaryCodeAggregatorCurried[T, U, V, W] {
  def apply[Agg <: RegionValueAggregator : ClassTag : TypeInfo]
    (stagedNew: (Code[T], Code[Boolean], Code[U], Code[Boolean], Code[V], Code[Boolean]) => Code[Agg], out: Type)
    (implicit uct: ClassTag[W], hrt: HailRep[W]): TernaryCodeAggregator[T, U, V, Agg, W] =
    new TernaryCodeAggregator(hailType[W], stagedNew.asInstanceOf[(Code[_], Code[Boolean], Code[_], Code[Boolean], Code[_], Code[Boolean]) => Code[Agg]],
      out)
}
