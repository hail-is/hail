  ...
  val getOption: ((AggOp, Type, Seq[Type], Option[Seq[Type]], Seq[Type])) => Option[CodeAggregator[T] forSome { type T <: RegionValueAggregator }] = lift {
    ...
    case (TakeBy(), in, constArgs@Seq(_: TInt32), None, Seq(key)) =>
      ...
      (in, key) match {
        ...
        case (_: TBoolean, _: TCall) => tbCodeAgg[RegionValueTakeByBooleanIntAggregator](classOf[Int])
        ...
        case (_: TCall, _: TBoolean) => tbCodeAgg[RegionValueTakeByIntBooleanAggregator](classOf[Boolean])
        case (_: TCall, _: TCall) => tbCodeAgg[RegionValueTakeByIntIntAggregator](classOf[Int])
        case (_: TCall, _: TFloat64) => tbCodeAgg[RegionValueTakeByIntDoubleAggregator](classOf[Double])
        case (_: TCall, _) => tbCodeAgg[RegionValueTakeByIntAnnotationAggregator](classOf[Long])

        case (_: TFloat64, _: TBoolean) => tbCodeAgg[RegionValueTakeByDoubleBooleanAggregator](classOf[Boolean])
        case (_: TFloat64, _: TCall) => tbCodeAgg[RegionValueTakeByDoubleIntAggregator](classOf[Int])
        ...
        case (_, _: TCall) => tbCodeAgg[RegionValueTakeByAnnotationIntAggregator](classOf[Int])
