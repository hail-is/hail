...
object UnsafeRow {
  ...
  def readStructBase(t: TStructBase, region: Region, offset: Long): UnsafeRow =
  ...
  def read(t: Type, region: Region, offset: Long): Any = {
    t match {
      ...
      case t: TStructBase =>
        readStructBase(t, region, offset)
      case x: TVariant =>
...
class UnsafeRow(var t: TStructBase,
  var region: Region, var offset: Long) extends Row with KryoSerializable {

  def this(t: TStructBase, rv: RegionValue) = this(t, rv.region, rv.offset)

  def this(t: TStructBase) = this(t, null, 0)
