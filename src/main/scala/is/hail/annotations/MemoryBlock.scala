...
final class MemoryBuffer(var mem: Long, var length: Long, var offset: Long = 0) extends KryoSerializable with Serializable {
  ...
  def loadAddress(off: Long): Long = {
    assert(size <= length)
    assert(off >= 0 && off + 8 <= size, s"tried to read address at $off from region with size $size")
  ...
  def storeBytes(off: Long, src: Array[Byte]) {
    storeBytes(off, src, 0, src.length)
  }

  def storeBytes(off: Long, src: Array[Byte], srcOff: Long, n: Int) {
    assert(size <= length)
    assert(off >= 0 && off + n <= size, s"tried to store $n bytes at $off to region of size $size")
    assert(n <= src.length)
    Memory.copyFromArray(mem + off, src, srcOff, n)
  }
  ...
  def appendBytes(bytes: Array[Byte], n: Int) {
    ...
    storeBytes(allocate(n), bytes, 0, n)
  }

  def appendBytes(from: Array[Byte], fromOff: Long, n: Int) {
    assert(fromOff + n <= from.length)
    storeBytes(allocate(n), from, fromOff, n)
  }
...
class RegionValueBuilder(var region: MemoryBuffer) {
  ...
  def currentOffset(): Long = {
    if (typestk.isEmpty)
      ...
      typestk.head match {
        case t: TStruct =>
          offsetstk.head + t.byteOffsets(i)
        case t: TArray =>
          elementsOffsetstk.head + i * t.elementByteSize
      }
    }
  }
  ...
  def endArray() {
    typestk.head match {
      case t: TArray =>
        val aoff = offsetstk.head
        val length = t.loadLength(region, aoff)
        assert(length == indexstk.head)
