...
class VariantDatasetFunctions(private val vds: VariantSampleMatrix[Genotype]) extends AnyVal {
  ...
  def linregBurden(keyName: String, variantKeySetVA: String, aggregateExpr: String, ySA: String, covSA: Array[String]): (KeyTable, KeyTable) = {
    requireSplit("linear burden regression")
    LinearRegressionBurden(vds, keyName, variantKeySetVA, aggregateExpr, ySA, covSA)
  }
