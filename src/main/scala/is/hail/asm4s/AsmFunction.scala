...
sealed trait SpecializedOrderingFunction[T]
trait BooleanOrderingFunction extends SpecializedOrderingFunction[Boolean] { def apply(x1: Boolean, x2: Boolean): Boolean }
trait IntOrderingFunction extends SpecializedOrderingFunction[Int] { def apply(x1: Int, x2: Int): Boolean }
trait LongOrderingFunction extends SpecializedOrderingFunction[Long] { def apply(x1: Long, x2: Long): Boolean }
trait FloatOrderingFunction extends SpecializedOrderingFunction[Float] { def apply(x1: Float, x2: Float): Boolean }
trait DoubleOrderingFunction extends SpecializedOrderingFunction[Double] { def apply(x1: Double, x2: Double): Boolean }