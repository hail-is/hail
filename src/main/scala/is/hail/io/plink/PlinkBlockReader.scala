...
class PlinkRecord(nSamples: Int, a2Major: Boolean) extends KeySerializedValueRecord[Int, IndexedSeq[Annotation]] {
...
class PlinkBlockReader(job: Configuration, split: FileSplit) extends IndexedBinaryBlockReader[PlinkRecord](job, split) {
  ...
  val a2Major = job.getBoolean("a2Major", true)
  val blockLength = (nSamples + 3) / 4

  seekToFirstBlockInSplit(split.getStart)

  override def createValue(): PlinkRecord = new PlinkRecord(nSamples, a2Major)
