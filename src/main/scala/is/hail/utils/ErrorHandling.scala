...
trait ErrorHandling {
  def fatal(msg: String): Nothing = fatal(msg, null: Throwable)

  def fatal(msg: String, cause: Throwable): Nothing = throw new HailException(msg, None, cause)

  def fatal(msg: String, t: Truncatable): Nothing = fatal(msg, t, null: Throwable)
  ...
  def fatal(msg: String, t1: Truncatable, t2: Truncatable): Nothing = fatal(msg, t1, t2, null)

  def fatal(msg: String, t1: Truncatable, t2: Truncatable, cause: Throwable): Nothing = {
    val (screen1, logged1) = t1.strings
    val (screen2, logged2) = t2.strings
    throw new HailException(format(msg, screen1, screen2), Some(format(msg, logged1, logged2)), cause)
  }
