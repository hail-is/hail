...
class RichCodeMemoryBuffer(val region: Code[MemoryBuffer]) extends AnyVal {
  ...
  def irIntermediateToRegionValue(t: Type): Code[_] => Code[Long] = t match {
    case _: TBoolean => v => appendByte(coerce[Boolean](v).toI.toB)
    case _: TInt32 => v => appendInt32(coerce[Int](v))
    case _: TInt64 => v => appendInt64(coerce[Long](v))
    case _: TFloat32 => v => appendFloat32(coerce[Float](v))
    case _: TFloat64 => v => appendFloat64(coerce[Double](v))
