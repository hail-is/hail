...
object RelativeFuzzyComparable {
  ...
  def relativeEq[T](tolerance: Double, x: T, y: T)(implicit afc: RelativeFuzzyComparable[T]): Boolean =
  ...
  implicit def rfcMaps[K, V](implicit vRFC: RelativeFuzzyComparable[V]): RelativeFuzzyComparable[Map[K, V]] =
