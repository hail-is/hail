...
case class HailSchemaRow(val values: mutable.WrappedArray[Any], override val schema: StructType) extends Row(values) {
  ...
  def getSubrow(id: String): HailSchemaRow = {
    val i = fieldIndex(id)
    ???
  }
}

case class MutableStructSignature(m: mutable.Map[String, (Int, Signature)]) extends Signature {
  def typeOf = null

  def insertSig(toInsert: Signature, path: List[String]): List[Int] = {
    val (mapToUpdate, indexPath) = descendTo(path, makeTree = true)
    val key = path.last
    mapToUpdate.get(key) match {
      case Some((i, remap)) =>
        mapToUpdate += ((key, (i, toInsert)))
        listAppend(indexPath, i)
      case None =>
        val i = mapToUpdate.size
        mapToUpdate += ((key, (mapToUpdate.size, toInsert)))
        listAppend(indexPath, i)
    }
  }

  def removeSig(path: List[String]): List[Int] = {
    val (mapToUpdate, indexPath) = descendTo(path)
    val key = path.last
    mapToUpdate.get(key) match {
      case Some((i, sig)) =>
        m.mapValues { case (index, s) =>
          if (index > i)
            (index - 1, s)
          else
            (index, s)
    ...
    def result(): StructSignature = {
      StructSignature(
        m.mapValues {
          case (i, ms: MutableStructSignature) => (i, ms.result())
          case x => x
        }
          .toMap)
    }

    private def descendTo(path: List[String], makeTree: Boolean = false): (mutable.Map[String, (Int, Signature)], List[Int]) =
    {
      if (path.length == 1) {
        (m, List[Int]())
      }
        ...
        val key = path.head
        m.get(key) match {
          case Some((i, ms: MutableStructSignature)) =>
            val (nextMap, nextPath) = ms.descendTo(path.tail, makeTree)
            (nextMap, nextPath.::(i))
          case Some((i, sigs: StructSignature)) =>
            val ms = MutableStructSignature(sigs)
            m += ((key, (i, ms)))
            val (nextMap, nextPath) = ms.descendTo(path.tail, makeTree)
            (nextMap, nextPath.::(i))
          case None =>
            if (makeTree) {
              val i = m.size
              val ms = MutableStructSignature.empty()
              m += ((key, (i, ms)))
              val (nextMap, nextPath) = ms.descendTo(path.tail, makeTree)
              (nextMap, nextPath.::(i))
            }
  ...
  object MutableStructSignature {
    def apply(sigs: StructSignature): MutableStructSignature = {
      val m = mutable.Map.empty[String, (Int, Signature)]
      sigs.m.foreach {
        pair => m += pair
      }
      MutableStructSignature(m)
    }

    def empty(): MutableStructSignature = MutableStructSignature(mutable.Map.empty[String, (Int, Signature)])
    ...
    def descendNestedMaps(m: mutable.Map[String, Signature],
      ...
      val map = path.foldLeft(m) { (mMap, key) =>
        if (!mMap.contains(key)) {
          val index = mMap.size
          mMap += ((key, MutableStructSignature(mutable.Map.empty[String, Signature], index)))
        }

        mMap(key) match {
          case as: StructSignature =>
            val convertedSigs = MutableStructSignature(as)
            mMap += ((key, convertedSigs))
            indexPath += convertedSigs.index
            convertedSigs.m
          case ms: MutableStructSignature =>
            indexPath += ms.index
            ms.m
          case error =>
  ...
  object AnnotationData {
    ...
    def removeSignature(base: StructSignature,
      ...
      val ms = MutableStructSignature(base)
