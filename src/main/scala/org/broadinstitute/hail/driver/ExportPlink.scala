...
object ExportPlink extends Command {
  ...
  class Options extends BaseOptions {
    @Args4jOption(required = true, name = "-o", aliases = Array("--output"), usage = "Output file base (will generate .bed, .bim, .fam)")
    var output: String = _

    @Args4jOption(required = true, name = "-t", aliases = Array("--tmpdir"), usage = "Directory for temporary files")
    var tmpdir: String = _

    @Args4jOption(required = true, name = "--cutoff", usage = "GQ cutoff below which calls will be dropped")
    var cutoff: Int = _
  }
  ...
  def run(state: State, options: Options): State = {
    ...
    val bedRowRDD = vds
      .rdd
      .map {
        case (v, va, gs) => (v, ExportBedBimFam.makeBedRow(gs))
      }
    bedRowRDD
      .repartitionAndSortWithinPartitions(new RangePartitioner[Variant, String](vds.rdd.partitions.length,bedRowRDD))
      .map(_._2)
      .writeTableSingleFile(options.tmpdir, options.output + ".bed", Some(header), deleteTmpFiles = true)
