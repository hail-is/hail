...
object TDT {
  ...
  def apply(vds: VariantDataset, preTrios: Array[CompleteTrio]): RDD[(Variant, Annotation)] = {

    // remove trios with undefined sex
    ...
    def seqOp(a: MultiArray2[GenotypeType], v: Variant, s: String, g: Genotype): MultiArray2[GenotypeType] = {
      //FIXME don't put dad if X het
      sampleTrioRolesBc.value.get(s).foreach(l => l.foreach { case (tidx, ridx) =>
        if (v.contig == "X" && !v.inParX && ridx == 1 && g.isHet)
    ...
    //    def seqOp2: (Array[(Int, Int)], (Variant, MultiArray2[GenotypeType.GenotypeType])) => Array[(Int, Int)] = {
    //      case (a, (v, ma)) =>
    //        for (i <- a.indices) {
    //          val kid = ma(i, 0)
    //          val dad = ma(i, 1)
    ...
    //          val (t, u) = a(i)
    //
    //          val (nTransmissions, nUntransmissions) = getTransmission(kid, dad, mom, v.copyState(trioSexBc.value(i)))
    //          a(i) = (nTransmissions + t, nUntransmissions + u)
    //        }
    //
    //        a
    //    }
...
//        var nUntrans = 0
    ...
    // per variant:
    val perVariant = transmissionMatrix.map { case (v, transmissions) =>
      val (totalTrans, totalUntrans) = transmissions.fold((0, 0)){ case ((t1,u1), (t2,u2)) => (t1 + t2, u1 + u2)}
      (v, Annotation(totalTrans, totalUntrans, calcTDTstat(totalTrans, totalUntrans)))
    }


    val perSample = transmissionMatrix
      .map(_._2)
