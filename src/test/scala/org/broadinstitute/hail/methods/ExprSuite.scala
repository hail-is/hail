...
class ExprSuite extends SparkSuite {
  ...
  @Test def exprTest() {
    ...
    assert(eval[Boolean]("div(0, 1) == 0").contains(true))
    assert(eval[Boolean]("div(0, -1) == 0").contains(true))
    assert(eval[Boolean]("div(7, 2) == 3").contains(true))
    assert(eval[Boolean]("div(-7, -2) == 3").contains(true))
    assert(eval[Boolean]("div(-7, 2) == -4").contains(true))
    assert(eval[Boolean]("div(7, -2) == -4").contains(true))
    assert(eval[Boolean]("div(-6, 2) == -3").contains(true))
    assert(eval[Boolean]("div(6, -2) == -3").contains(true))

    assert(eval[Boolean]("div(1.0, 2.0) == 0.0").contains(true))
    assert(eval[Boolean]("div(-1.0, 2.0) == -1.0").contains(true))
    assert(eval[Boolean]("div(1.0, -2.0) == -1.0").contains(true))
    assert(eval[Boolean]("div(-1.0, -2.0) == 0.0").contains(true))
