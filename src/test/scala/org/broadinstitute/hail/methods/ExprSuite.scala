...
class ExprSuite extends SparkSuite {
  ...
  @Test def exprTest() {
    ...
    assert(eval[Int]("s[0]").contains("1"))
    assert(eval[Int]("s[1]").contains("2"))
    assert(eval[Int]("s[2]").contains(","))
    assert(eval[Int]("s[3]").contains("3"))
    assert(eval[Int]("s[-1]").contains("8"))
    assert(eval[Int]("s[-2]").contains("7"))
    for (i <- 0 until 11)
      assert(eval[Int](s"s[${i - 11}]") == eval[Int](s"s[$i]"))
