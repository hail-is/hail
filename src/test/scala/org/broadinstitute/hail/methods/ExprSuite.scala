...
class ExprSuite extends SparkSuite {
  ...
  @Test def exprTest() {
    ...
    assert(eval[Double]("""[1000000000.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0].map(x => x * 1.0).sum""").contains(1000000000.0))
    assert(eval[Long]("""[1000000000,10,10,10,10,10,10,10,10,10,10].map(x => x).sum""").contains(IndexedSeq(1000000000,10,10,10,10,10,10,10,10,10,10).sum))
