...
import org.broadinstitute.hail.variant.{Genotype, VDSGens, Variant, VariantDataset, VariantSampleMatrix}
...
class SplitSuite extends SparkSuite {
  ...
  object Spec extends Properties("MultiSplit") {
    property("fakeRef implies wasSplit") =
      forAll(VariantSampleMatrix.gen[Genotype](sc, VDSGens.random)) { (vds: VariantDataset) =>
