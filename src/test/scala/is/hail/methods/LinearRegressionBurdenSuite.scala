...
class LinearRegressionBurdenSuite extends SparkSuite {
  ...
  @Test def sandbox() {
    ...
    val (linregKT, sampleKT) = vds.linregBurden("gene", "va.gene", "s.length()", "sa.pheno.Pheno", covSA = Array("sa.cov.Cov1", "sa.cov.Cov2"))

    sampleKT.toDF(sqlContext).show()

    linregKT.toDF(sqlContext).show()
  }
  ...
  @Test def testWeightedSum() {
    ...
    */
    ...
    val (linregKT, sampleKT) = vds.linregBurden("gene", "va.gene", "gs.map(g => va.weight * g.gt).sum()", "sa.pheno.Pheno", covSA = Array("sa.cov.Cov1", "sa.cov.Cov2"))

    val values = sampleKT.join(linregKT, "inner").collect().map { r => val s = r.asInstanceOf[Row].toSeq
  ...
  @Test def testWeightedSumWithImputation() {
    ...
    */
    ...
    val (linregKT, sampleKT) = vds.linregBurden("Gene", "va.gene", "gs.map(g => va.weight * orElse(g.gt.toDouble, 2 * va.qc.AF)).sum()", "sa.pheno.Pheno", covSA = Array("sa.cov.Cov1", "sa.cov.Cov2"))
  ...
  @Test def testMax() {
    ...
    */
    ...
    val (linregKT, sampleKT) = vds.linregBurden("gene", "va.gene", "gs.map(g => g.gt.toDouble).max()", "sa.pheno.Pheno", covSA = Array("sa.cov.Cov1", "sa.cov.Cov2"))
