...
class LinearRegressionSuite extends SparkSuite {
  ...
  @Test def testWithTwoCov() {
    ...
    val vds = LinearRegressionCommand.run(s.vds, "sa.pheno.Pheno", List("sa.cov.Cov1", "sa.cov.Cov2 + 1 - 1"), "va.linreg", 1, 0.0)
  ...
  @Test def testWithNoCov() {
    ...
    val vds = LinearRegressionCommand.run(s.vds, "sa.pheno.Pheno", List.empty[String], "va.linreg", 1, 0.0)
  ...
  @Test def testWithImportFamBoolean() {
    ...
    val vds = LinearRegressionCommand.run(s.vds, "sa.fam.isCase", List("sa.cov.Cov1", "sa.cov.Cov2"), "va.linreg", 1, 0.0)
  ...
  @Test def testWithImportFam() {
    ...
    val vds = LinearRegressionCommand.run(s.vds, "sa.fam.qPheno", List("sa.cov.Cov1", "sa.cov.Cov2"), "va.linreg", 1, 0.0)
  ...
  @Test def testNonNumericPheno() {
    ...
    interceptFatal("Sample annotation `sa.pheno.Pheno' must be numeric or Boolean, got String") {
      LinearRegressionCommand.run(s.vds, "sa.pheno.Pheno", List("sa.cov.Cov1", "sa.cov.Cov2"), "va.linreg", 1, 0.0)
    }
  }
  ...
  @Test def testFilters() {
    ...
    vds = LinearRegressionCommand.run(s.vds, "sa.pheno.Pheno", List.empty[String], "va.linreg", 4, 0.0)
    ...
    vds = LinearRegressionCommand.run(s.vds, "sa.pheno.Pheno", List.empty[String], "va.linreg", 1, 0.3)

    assert(qBeta(annotationMap(v1)).isEmpty)
    assert(qBeta(annotationMap(v2)).isDefined)

    vds = LinearRegressionCommand.run(s.vds, "sa.pheno.Pheno", List.empty[String], "va.linreg", 1, 0.4)
    ...
    vds = LinearRegressionCommand.run(s.vds, "sa.pheno.Pheno", List.empty[String], "va.linreg", 5, 0.1)
  ...
  @Test def testFiltersFatals() {
    ...
    interceptFatal("Minumum alternate allele count must be a positive integer, got 0") {
      LinearRegressionCommand.run(s.vds, "sa.pheno.Pheno", List.empty[String], "va.linreg", 0, 0.0)
    }

    interceptFatal("Minumum alternate allele frequency must lie in") {
      LinearRegressionCommand.run(s.vds, "sa.pheno.Pheno", List.empty[String], "va.linreg", 1, 2.0)
    }
  }
}