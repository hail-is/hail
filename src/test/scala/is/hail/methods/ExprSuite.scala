...
class ExprSuite extends SparkSuite {
  ...
  @Test def exprTest() {
...

    {
      ...
      left.keySet should contain theSameElementsAs Seq(0, 1)
      assert(left.get(0).contains(0))
      assert(left.get(1).contains(1))
      val right = x.get.asInstanceOf[Row].getAs[Map[Int,Int]](2)
      right.keySet should contain theSameElementsAs Seq(0, 1)
      assert(right.get(0).contains(1))
      assert(right.get(1).contains(2))
    }
