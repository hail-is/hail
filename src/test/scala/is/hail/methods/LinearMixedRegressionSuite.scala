...
class LinearMixedRegressionSuite extends SparkSuite {
  ...
  @Test def fastlmmTest() {
    ...
    */
    ...
    val vds = hc.importPlink(bed = "src/test/resources/fastlmmTest.bed",
      ...
      .annotateSamplesTable(covariates, expr = Some("sa.cov=table._2"))
      .annotateSamplesTable(phenotypes, expr = Some("sa.pheno=table._2"))
  ...
  @Test def filterTest() {
    ...
    var vdsAssoc = hc.importVCF("src/test/resources/regressionLinear.vcf")
      ...
      .annotateSamplesTable(phenotypes, expr = Some("sa.pheno.Pheno = table.Pheno"))
