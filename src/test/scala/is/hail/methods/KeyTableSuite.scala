...
class KeyTableSuite extends SparkSuite {
  ...
  @Test def testSelect() {
    ...
    val select5 = kt2.selectFields(Array("field0.a", "field2", "Sample"))
    assert((select5.key sameElements Array("Sample")) && (select5.columns sameElements Array("a", "field2", "Sample")))

    val select6 = kt2.selectFields(Array("field0.a", "field2"))
    assert((select6.key sameElements Array.empty[String]) && (select6.columns sameElements Array("a", "field2")))

    val select7 = kt2.selectFields(Array("field0.a", "field2.a"), mangle = true)
    assert((select7.key sameElements Array.empty[String]) && (select7.columns sameElements Array("field0.a", "field2.a")))

    TestUtils.interceptFatal("Either rename manually or use the 'mangle' option to handle duplicates")(kt2.selectFields(Array("field0.a", "field2.a")))
    TestUtils.interceptFatal("do not exist in key table.")(kt2.selectFields(Array("notexist")))
