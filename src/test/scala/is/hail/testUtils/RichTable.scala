...
class RichTable(ht: Table) {
  def forall(code: String): Boolean = {
    val (a, t) = ht.aggregateAST(s"AGG.filter(row => let t = $code in !(isDefined(t) && t)).count() == i64#0")
  ...
  def keyByExpr(exprs: (String, String)*): Table =
    ht.selectAST(s"annotate(row, {${ exprs.map { case (n, e) => s"${ prettyIdentifier(n) }: $e" }.mkString(",") }})", Some(exprs.map(_._1).toFastIndexedSeq), Some(0))

  def annotate(exprs: (String, String)*): Table =
    ht.selectAST(s"annotate(row, {${ exprs.map { case (n, e) => s"${ prettyIdentifier(n) }: $e" }.mkString(",") }})", ht.key, ht.key.map(_.length))
}
