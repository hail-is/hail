...
class TableIRSuite extends SparkSuite {
  ...
  @Test def testFilter() {
    ...
    val kt2 = new Table(hc, TableFilter(kt.ir,
      ir.ApplyBinaryPrimOp(ir.EQ(), ir.Ref("field1"), ir.I32(3))))
    assert(kt2.count() == 1)
  }
  ...
  @Test def testFilterGlobals() {
    ...
    val kt2 = new Table(hc, TableFilter(kt.ir,
      ir.ApplyBinaryPrimOp(ir.EQ(), ir.Ref("field1"), ir.Ref("g"))))
    assert(kt2.count() == 1)
  }
  ...
  @Test def testAnnotate() {
    ...
    val kt2 = new Table(hc, TableAnnotate(kt.ir,
      IndexedSeq("a", "field1"),
      IndexedSeq(ir.I32(0), ir.InsertFields(ir.Ref("field1"), Array(("b", ir.Ref("field1")))))))
  ...
  @Test def testAnnotate2() {
    ...
    val kt6 = kt2.annotate("d = a.*")
    assert(kt6.ir.isInstanceOf[TableAnnotate])
    ...
    assert(kt6.select("Sample", "field1 = d.b", "field2 = d.a").same(kt))
