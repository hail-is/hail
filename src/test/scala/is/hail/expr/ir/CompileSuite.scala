...
class CompileSuite {
  ...
  def testShortCircuitBooleans() {
    ...
    val irAnd = MakeTuple(Seq(ApplyBinaryPrimOp(DoubleAmpersand(), ArrayRef(In(0, tin), I32(0)), ArrayRef(In(0, tin), I32(1)))))
    val irOr = MakeTuple(Seq(ApplyBinaryPrimOp(DoublePipe(), ArrayRef(In(0, tin), I32(0)), ArrayRef(In(0, tin), I32(1)))))
    ...
    for (l: Option[Boolean] <- Seq(Some(true), Some(false), None); r: Option[Boolean] <- Seq(Some(true), Some(false), None)) {
      region.clear()
