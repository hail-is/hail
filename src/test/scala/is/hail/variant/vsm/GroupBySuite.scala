...
class GroupBySuite extends SparkSuite {
  ...
  @Test def testAggregateColumnsByKeyOnSmallExampleGoingThroughIR() {
    ...
    val result = mt.keyColsBy("group")
      .aggregateColsByKeyAST(aggExpression)
  ...
  @Test def testResultSchemaFromAggregateColsByKey() {
    ...
    val resultMT = mt.keyColsBy("AC")
      .aggregateColsByKeyAST("{max : AGG.map(g => g.GT.nNonRefAlleles()).max(), " +
  ...
  @Test def testStructParameterInAggregateColsByKey() {
    ...
    val result = mt.keyColsBy("foo").aggregateColsByKeyAST("{max: AGG.map(g => g.GT.nNonRefAlleles()).max()}")
  ...
  @Test def testInitOpInAggregateColsByKey() {
    ...
    val result = mt.aggregateColsByKeyAST("{call_stats: AGG.map(g => g.GT).callStats(g => va.alleles.size())}")

    val ac = result.selectRowsAST("{locus: va.locus, alleles: va.alleles}", None).selectColsAST("{s: sa.s}", None)
      .selectEntriesAST("{AC: g.call_stats.AC}").entriesTable().collect().map(row => row.get(3))
  ...
  @Test def testRandomVSMEquivalence() {
    ...
    val p = forAll(MatrixTable.gen(hc, VSMSubgen.random)
      ...
      if (variants.length != uniqueVariants.size) {
        vSkipped += 1
        val grouped = vsm.aggregateRowsByKeyAST(
          "{ first : AGG.collect()[0] }",
          "first = AGG.collect()[0]")
        ...
        val grouped = vsm
          .aggregateRowsByKeyAST(
            ...
            "GT = AGG.collect()[0].GT, " +
              ...
              "GQ = AGG.collect()[0].GQ, " +
              "PL = AGG.collect()[0].PL")
        assert(vsm.selectRowsAST("{locus: va.locus, alleles: va.alleles}", None).same(grouped))
      }

      val uniqueSamples = vsm.stringSampleIds.toSet
      if (vsm.stringSampleIds.size != uniqueSamples.size) {
        sSkipped += 1
        val grouped = vsm.keyColsBy("s").aggregateColsByKeyAST("{first : AGG.collect()[0]}")
        grouped.numCols == uniqueSamples.size
      } else {
        val grouped = vsm.keyColsBy("s").aggregateColsByKeyAST("{GT : AGG.collect()[0].GT, AD : AGG.collect()[0].AD, DP : AGG.collect()[0].DP, GQ : AGG.collect()[0].GQ, PL : AGG.collect()[0].PL}")
        assert(vsm.selectColsAST("{s: sa.s}", None)
          .same(grouped
  ...
  @Test def testLinregBurden() {
    ...
    val linregMap = resultsVSM.rowsTable().selectAST("{genes: row.genes, beta: row.linreg.beta, standard_error: row.linreg.standard_error, t_stat: row.linreg.t_stat, p_value: row.linreg.p_value}", None, None)
