...
class IndexSuite extends HailSuite {
  ...
  val leafsWithDups = stringsWithDups.zipWithIndex.map { case (s, i) => LeafChild(s, i.toLong, Row()) }
  ...
  @Test def testRangeIterator(): Unit = {
    for (branchingFactor <- 2 to 5) {
      ...
      bounds.foreach(b =>
        index.iterator(b(0).toLong, b(1).toLong).toArray sameElements leafsWithDups.slice(b(0), b(1))
      )
