package is.hail.linalg

import is.hail.TestUtils._
import is.hail.collection.compat.immutable.ArraySeq

import org.junit.jupiter.api.Test

class GridPartitionerSuite {

  private def assertLayout(hg: GridPartitioner, layout: ((Int, Int), Int)*): Unit = {
    layout.foreach { case ((i, j), p) =>
      assertEq(hg.coordinatesBlock(i, j), p, s"at coordinates ${(i, j)}")
    }
    layout.foreach { case ((i, j), p) =>
      assertEq(hg.blockCoordinates(p), (i, j), s"at pid $p")
    }
  }

  @Test
  def squareIsColumnMajor(): Unit =
    assertLayout(GridPartitioner(2, 4, 4), (0, 0) -> 0, (1, 0) -> 1, (0, 1) -> 2, (1, 1) -> 3)

  @Test
  def rectangleMoreRowsIsColumnMajor(): Unit = {
    assertLayout(
      GridPartitioner(2, 6, 4),
      (0, 0) -> 0,
      (1, 0) -> 1,
      (2, 0) -> 2,
      (0, 1) -> 3,
      (1, 1) -> 4,
      (2, 1) -> 5,
    )
  }

  @Test
  def rectangleMoreColsIsColumnMajor(): Unit = {
    assertLayout(
      GridPartitioner(2, 4, 6),
      (0, 0) -> 0,
      (1, 0) -> 1,
      (0, 1) -> 2,
      (1, 1) -> 3,
      (0, 2) -> 4,
      (1, 2) -> 5,
    )
  }

  @Test
  def bandedBlocksTest(): Unit = {
    // 0  3  6  9
    // 1  4  7 10
    // 2  5  8 11
    val gp1 = GridPartitioner(10, 30, 40)
    val gp2 = GridPartitioner(10, 21, 31)

    for (gp <- Seq(gp1, gp2)) {
      assert(gp.bandBlocks(0, 0) sameElements Array(0, 4, 8))

      assert(gp.bandBlocks(-1, 0) sameElements Array(0, 1, 4, 5, 8))
      assert(gp.bandBlocks(-1, 0) sameElements gp.bandBlocks(-10, 0))

      assert(gp.bandBlocks(0, 1) sameElements Array(0, 3, 4, 7, 8, 11))
      assert(gp.bandBlocks(0, 1) sameElements gp.bandBlocks(0, 10))

      assert(gp.bandBlocks(-1, 1) sameElements Array(0, 1, 3, 4, 5, 7, 8, 11))
      assert(gp.bandBlocks(-1, 1) sameElements gp.bandBlocks(-10, 10))

      assert(gp.bandBlocks(-11, 0) sameElements Array(0, 1, 2, 4, 5, 8))

      assert(gp.bandBlocks(0, 11) sameElements Array(0, 3, 4, 6, 7, 8, 10, 11))
      assert(gp.bandBlocks(0, 20) sameElements gp.bandBlocks(0, 11))
      assert(gp.bandBlocks(0, 21) sameElements Array(0, 3, 4, 6, 7, 8, 9, 10, 11))

      assert(gp.bandBlocks(-1000, 1000) sameElements (0 until 12))
    }
  }

  @Test
  def rectangularBlocksTest(): Unit = {
    // 0  3  6  9
    // 1  4  7 10
    // 2  5  8 11
    val gp1 = GridPartitioner(10, 30, 40)
    val gp2 = GridPartitioner(10, 21, 31)

    for (gp <- Seq(gp1, gp2)) {
      assertEq(gp.rectangleBlocks(ArraySeq[Long](0, 1, 0, 1)), ArraySeq(0))
      assertEq(gp.rectanglesBlocks(ArraySeq(ArraySeq[Long](0, 1, 0, 1))), ArraySeq(0))

      assertEq(gp.rectangleBlocks(ArraySeq[Long](0, 10, 0, 10)), ArraySeq(0))

      assertEq(gp.rectangleBlocks(ArraySeq[Long](9, 11, 9, 11)), ArraySeq(0, 1, 3, 4))
      assertEq(gp.rectanglesBlocks(ArraySeq(ArraySeq[Long](9, 11, 9, 11))), ArraySeq(0, 1, 3, 4))

      assertEq(gp.rectangleBlocks(ArraySeq[Long](10, 20, 10, 30)), ArraySeq(4, 7))

      assertEq(
        gp.rectanglesBlocks(ArraySeq(
          ArraySeq[Long](9, 11, 9, 11),
          ArraySeq(10, 20, 10, 30),
          ArraySeq(0, 1, 20, 21),
          ArraySeq(20, 21, 20, 31),
        )),
        ArraySeq(0, 1, 3, 4, 6, 7, 8, 11),
      )

      assertEq(gp.rectangleBlocks(ArraySeq[Long](0, 21, 0, 31)), (0 until 12))
    }
  }
}
