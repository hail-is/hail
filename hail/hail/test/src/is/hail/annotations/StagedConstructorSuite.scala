...
class StagedConstructorSuite extends HailSuite with ScalaCheckDrivenPropertyChecks {
  ...
  @Test(expectedExceptions = Array(classOf[TestFailedException]))
  def testShallowCopyOfPointersFailsAcrossRegions(): Unit = {
    val ptype = PCanonicalStruct("a" -> PCanonicalArray(PInt32()))
    ...
    val ex: RuntimeException =
      ctx.scopedExecution { (hcl, fs, htc, r1) =>
        val invalid =
          r1.pool.scopedRegion { r2 =>
            val offset = ScalaToRegionValue(sm, r2, ptype, value)
            val unsafe = ShallowCopy(hcl, fs, htc, r1)(r1, offset)
            r2.clear()
            unsafe
          }

        intercept[RuntimeException] {
          SafeRow(ptype, invalid)
        }
      }

    ex.getMessage should contain("invalid memory access")
  }

  @Test def testDeepCopy(): Unit =
    forAll(genPTypeVal[PCanonicalStruct](ctx)) { case (t, a) =>
      val DeepCopy = emitCopy(ctx, t, deepCopy = true)

      val copy = ctx.scopedExecution { (hcl, fs, htc, r1) =>
        val valid =
          r1.pool.scopedRegion { r2 =>
            val offset = ScalaToRegionValue(sm, r2, t, a)
            val copy = DeepCopy(hcl, fs, htc, r1)(r2, offset)
            r2.clear()
            copy
          }
