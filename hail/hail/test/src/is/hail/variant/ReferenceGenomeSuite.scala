    ...
    ctx.local(references = ctx.references + (rg.name -> rg)) { ctx =>
...

      {
        "cache gives same base as from file" |: forAll(genLocus(rg)) { l =>
          ...
          val expectedGz = refReaderGz.getSubsequenceAt(contig, pos.toLong, pos.toLong).getBaseString
          ...
          case i: Interval =>
            ...
            def getHtsjdkIntervalSequence: String = {
              ...
              while (ordering.lteq(pos, end) && pos != null) {
                ...
                sb ++= refReader.getSubsequenceAt(pos.contig, pos.position.toLong, endPos.toLong).getBaseString
                pos =
