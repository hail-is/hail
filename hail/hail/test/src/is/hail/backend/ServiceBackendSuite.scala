...
class ServiceBackendSuite extends HailSuite with IdiomaticMockito with OptionValues {
  ...
  @Test def testCollectIncrementally(): Unit =
    runMock { (ctx, jobConfig, batchClient, backend) =>
      ...
      val contexts = ArraySeq.tabulate(3)(_ => Array.emptyByteArray)
        ...
        (id: Int, jobGroupId: Int) =>
          ...
          val response =
            JobGroupResponse(
              ...
              state = if (getJobGroupCalled) Success else Running,
              complete = getJobGroupCalled,
          ...
          getJobGroupCalled = true
          response
      }

      val endTime = Some("")
      var getJobGroupJobsCalled = false
      when(batchClient.getJobGroupJobs(
        ...
        (batchId: Int, _: Int, s: Option[JobState], t: Option[String]) =>
          ...
          t shouldBe (if (getJobGroupJobsCalled) endTime else None)

          // withhold one job to simulate delays in marking a job complete
          val jobs =
            if (getJobGroupJobsCalled) ArraySeq.empty
            else contexts.init.indices.map { i =>
              JobListEntry(
                batch_id = batchId,
                job_id = startJobId + i,
      ...
      batchClient.getJobGroup(any[Int], any[Int]) wasCalled twice
