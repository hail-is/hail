...
private[scalacheck] trait DenseMatrixGenInstances {
  ...
  def genNCubeOfVolumeAtMost(n: Int, size: Int, alpha: Int = 1): Gen[Array[Int]] =
    for {
      v <- containerOfN[Array, Int](n, Gen.binomial(arbitrary[Boolean], n))
      simplexVector: Array[Double] <- sampleDirichlet(v)
      sizeOfSum = math.log(size)
    } yield roundWithConstantSum(simplexVector.map(_ * sizeOfSum)).map(math.exp(_).toInt)

  def sampleDirichlet(dim: Array[Int]): Gen[Array[Double]] = ???
