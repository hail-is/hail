...
import org.scalacheck.Gen.{
  choose, const, containerOf, containerOfN, frequency, identifier, oneOf, option, pick, prob, size,
  stringOfN, zip,
}

private[scalacheck] trait GenomicGenInstances {
  ...
  def genSite(rg: ReferenceGenome): Gen[(String, Int)] =
    oneOf(rg.lengths)

  def genLocus(rg: ReferenceGenome): Gen[Locus] =
    for {
      (contig, length) <- genSite(rg)
  ...
  def genCall(
    nAlleles: Int,
    ploidyGen: Gen[Int] = choose(0, 2),
    phasedGen: Gen[Boolean] = prob(0.5),
  ): Gen[Call] =
    for {
      ploidy <- ploidyGen
      phased <- phasedGen
  ...
  lazy val genCall: Gen[Call] =
    choose(0, 2) flatMap genUnphasedDiploid
  ...
  private[this] def genLocusAlleles(
    siteGen: Gen[(String, Int)],
    nAllelesGen: Gen[Int],
    refGen: Gen[String],
    altGen: Gen[String],
  ): Gen[Annotation] =
    for {
      (contig, length) <- siteGen
      start <- Gen.choose(1, length)
      nAlleles <- nAllelesGen
      altAlleles <- distinctContainerOfN[Array, String](nAlleles - 1, altGen)
      ref <- refGen
  ...
  def genRandomLocusAlleles(rg: ReferenceGenome): Gen[Annotation] =
    genLocusAlleles(
      siteGen = genSite(rg),
      nAllelesGen = frequency(5 -> const(2), 1 -> choose(2, 10)),
      refGen = genDnaString,
      altGen = frequency(10 -> genDnaString, 1 -> const("*")),
    )

  def getPlinkCompatible(rg: ReferenceGenome): Gen[Annotation] =
    genLocusAlleles(
      siteGen = {
        val compatible = (1 until 22).map(_.toString).toSet
        genSite(rg) suchThat { case (contig, _) => compatible.contains(contig) }
      },
      nAllelesGen = Gen.frequency(5 -> const(2), 1 -> choose(2, 10)),
      refGen = genDnaString,
      altGen = Gen.frequency(10 -> genDnaString, 1 -> const("*")),
    )

  def biallelic(rg: ReferenceGenome): Gen[Annotation] =
    genLocusAlleles(
      siteGen = genSite(rg),
      nAllelesGen = const(2),
      refGen = genDnaString,
  ...
  def plinkCompatibleBiallelic(rg: ReferenceGenome): Gen[Annotation] =
    genLocusAlleles(
      siteGen = {
        val compatible = (1 until 22).map(_.toString).toSet
        genSite(rg) suchThat { case (contig, _) => compatible.contains(contig) }
