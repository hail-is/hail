...
private[scalacheck] trait ArbitraryGenomicInstances {
  ...
  def genReferenceGenone(genContigs: Int => Gen[Array[String]]): Gen[ReferenceGenome] =
    for {
      name <- identifier
      n <- sized(k => math.max(1, k))
  ...
  def genPlinkCompatibleReferenceGenome: Gen[ReferenceGenome] =
    sized { s =>
      choose(1, Math.min(21, s)).flatMap { n =>
        resize(n, genReferenceGenone(k => const(Array.tabulate(k)(i => (i + 1).toString))))
      }
    }

  implicit lazy val arbReferenceGenome: Arbitrary[ReferenceGenome] =
    oneOf(
      genReferenceGenone(n => distinctContainerOfN[Array, String](n, identifier)),
