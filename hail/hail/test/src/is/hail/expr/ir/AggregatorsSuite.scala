...
import is.hail.expr.ir.defs.{AggFilter, AggGroupBy, ApplyAggOp, ApplyBinaryPrimOp, ArrayRef, Cast, GetField, I32, InsertFields, MakeStruct, MakeTuple, Ref, Str, StreamAgg, StreamAggScan, StreamRange, TableAggregate, ToArray, ToStream}
...
class AggregatorsSuite extends HailSuite {
  ...
  @Test def testStreamAgg(): Unit = {
    ...
    val ir = StreamAgg(foo, elt.name, ApplyBinaryPrimOp(Add(), ArrayRef(agg1, I32(0)), ArrayRef(agg2, I32(1))))
    assertEvalsTo(ir, 1)
  }
  ...
  @Test def testLetBoundInitOpArg(): Unit = {
    ...
    tir =
      TableMapRows(tir, InsertFields(Ref(TableIR.rowName, tir.typ.rowType), FastSeq("aStr" -> Str("foo"))))
