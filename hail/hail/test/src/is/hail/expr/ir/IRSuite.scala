...
class IRSuite {
  ...
  def testSwitch() = ArraySeq(
    (I32(-1), I32(Int.MinValue), FastSeq(0, Int.MaxValue).map(I32), Int.MinValue: Any),
    (I32(0), I32(Int.MinValue), FastSeq(0, Int.MaxValue).map(I32), 0: Any),
    (I32(1), I32(Int.MinValue), FastSeq(0, Int.MaxValue).map(I32), Int.MaxValue: Any),
    (I32(2), I32(Int.MinValue), FastSeq(0, Int.MaxValue).map(I32), Int.MinValue: Any),
    (NA(TInt32), I32(Int.MinValue), FastSeq(0, Int.MaxValue).map(I32), null: Any),
    (I32(-1), NA(TInt32), FastSeq(0, Int.MaxValue).map(I32), null: Any),
    (I32(0), NA(TInt32), FastSeq(NA(TInt32), I32(0)), null: Any),
  )
  ...
  def testComparisonOpDifferentTypes() = ArraySeq[(Any, Type, Type)](
    (FastSeq(0.0, 0.0), TArray(TFloat64), TArray(TFloat64)),
    (Set(0, 1), TSet(TInt32), TSet(TInt32)),
    (Map(0L -> 5, 3L -> 20), TDict(TInt64, TInt32), TDict(TInt64, TInt32)),
    (
  ...
  def testValueIRParser(ctx: ExecuteContext) =
    valueIRs(ctx).map(a => (a(0).asInstanceOf[IR], a(1).asInstanceOf[BindingEnv[Type]])).toSeq
  ...
  def valueIRs(ctx: ExecuteContext): Array[Array[Object]] = {
    ...
    val vcf = importVCF(getTestResource("sample.vcf"))(ctx)
    ...
    val irs = Array[(IR, BindingEnv[Type] => BindingEnv[Type])](
      ...
      ApplyAggOp(Collect())(I32(0)) -> (_.createAgg),
      ApplyAggOp(CallStats(), I32(2))(call) -> (_.createAgg.bindAgg(
        call.name,
        call.typ,
      )),
      ApplyAggOp(TakeBy(), I32(10))(F64(-2.11), I32(4)) -> (_.createAgg),
  ...
  def testTableIRParser(ctx: ExecuteContext) =
    tableIRs(ctx).map(_(0)).toSeq
  ...
  def testMatrixIRParser(ctx: ExecuteContext) =
    matrixIRs(ctx).map(_(0)).toSeq
  ...
  def testBlockMatrixIRParser(ctx: ExecuteContext) = {
    val fs = ctx.fs
    val read =
      BlockMatrixRead(BlockMatrixNativeReader(fs, getTestResource("blockmatrix_example/0")))
  ...
  @Test def testIRConstruction(implicit ctx: ExecuteContext): Unit = {
    ...
    testBlockMatrixIRParser(ctx): Unit
  }

  @ParameterizedTest
  def testValueIRParser(x: IR, refMap: BindingEnv[Type])(implicit ctx: ExecuteContext): Unit = {
  ...
  @ParameterizedTest
  def testTableIRParser(x: TableIR)(implicit ctx: ExecuteContext): Unit = {
    val s = Pretty.sexprStyle(x, elideLiterals = false)
  ...
  @ParameterizedTest
  def testMatrixIRParser(x: MatrixIR)(implicit ctx: ExecuteContext): Unit = {
    val s = Pretty.sexprStyle(x, elideLiterals = false)
  ...
  @ParameterizedTest
  def testBlockMatrixIRParser(x: BlockMatrixIR)(implicit ctx: ExecuteContext): Unit = {
    val s = Pretty.sexprStyle(x, elideLiterals = false)
  ...
  def testRelationalFunctionsSerialize(ctx: ExecuteContext) = Seq[Any](
    TableFilterPartitions(ArraySeq(1, 2, 3), keep = true),
    VEP(ctx.fs, getTestResource("dummy_vep_config.json"), false, 1, true),
    ...
    PoissonRegression("firth", "a", "c", ArraySeq("d", "e"), ArraySeq("f", "g"), 25, 1e-6),
    Skat("a", "b", "c", "d", ArraySeq("e", "f"), false, 1, 0.1, 100, 0, 0.0),
    LocalLDPrune("x", 0.95, 123, 456),
    PCA("x", 1, false),
    PCRelate(0.00, 4096, Some(0.1), PCRelate.PhiK2K0K1),
    MatrixFilterPartitions(ArraySeq(1, 2, 3), keep = true),
    ForceCountTable(),
    ForceCountMatrixTable(),
    NPartitionsTable(),
    NPartitionsMatrixTable(),
    WrappedMatrixToValueFunction(NPartitionsMatrixTable(), "foo", "bar", FastSeq("a", "c")),
    MatrixExportEntriesByCol(1, "asd", false, true, false),
  ...
  @ParameterizedTest
  def testRelationalFunctionsSerialize(x: Any)(implicit ctx: ExecuteContext): Unit = {
  ...
  def regressionTestUnifyBug(implicit ctx: ExecuteContext): Unit = {
    ...
    val v = ctx.backend.execute(ctx, ir)
  ...
  def nonNullTypesAndValues() = Seq(
    (Int32SingleCodeType, 1: Any),
    (Int64SingleCodeType, 5L: Any),
    (Float32SingleCodeType, 5.5f: Any),
    (Float64SingleCodeType, 1.2: Any),
    (PTypeReferenceSingleCodeType(PCanonicalString()), "foo": Any),
    (PTypeReferenceSingleCodeType(PCanonicalArray(PInt32())), FastSeq(5, 7, null, 3): Any),
    (
