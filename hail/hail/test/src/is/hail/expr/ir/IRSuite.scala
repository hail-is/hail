...
class IRSuite extends HailSuite {
  ...
  @Test def testLet(): Unit = {
    ...
    assertEvalsTo(
      ToArray(mapIR(bindIR(I32(5))(v => rangeIR(v)))(x => x.clone + 2)),
      FastSeq(2, 3, 4, 5, 6),
    )
    assertEvalsTo(
      ToArray(mapIR(
        bindIR(I32(2))(q => mapIR(bindIR(q.clone + I32(3))(v => rangeIR(v)))(x => x.clone + q))
      )(_.clone + I32(3))),
      FastSeq(5, 6, 7, 8, 9),
    )
  ...
  @Test def testStreamZip(): Unit = {
    ...
    def zip(behavior: ArrayZipBehavior, irs: IR*): IR =
      zipIR(irs.toFastSeq, behavior)(irs => MakeTuple.ordered(irs.map(_.clone)))
    ...
    val lenOfLet = StreamLen(bindIR(I32(5))(ref =>
      StreamGrouped(
        mapIR(rangeIR(20))(range_element =>
          InsertFields(
            MakeStruct(IndexedSeq(("num", range_element.clone + ref))),
            IndexedSeq(("y", 12)),
          )
        ),
        3,
  ...
  @Test def testStreamMap(): Unit = {
    ...
    assertEvalsTo(ToArray(mapIR(a)(_.clone + I32(1))), FastSeq(4, null, 8))
  }
  ...
  @Test def testStreamFold(): Unit = {
    ...
    assertEvalsTo(foldIR(IRStream(1, 2, 3), 0)(_.clone + _), 6)
    assertEvalsTo(
      foldIR(IRStream(1, 2, 3), NA(TInt32))(_.clone + _),
      null,
    )
    assertEvalsTo(
      foldIR(IRStream(1, null, 3), NA(TInt32))(_.clone + _),
      null,
    )
    assertEvalsTo(foldIR(IRStream(1, null, 3), 0)(_.clone + _), null)
    assertEvalsTo(
  ...
  @Test def testArrayScan(): Unit = {
    ...
    assertEvalsTo(
      ToArray(streamScanIR(IRStream(1, 2, 3), 0)(_.clone + _)),
      FastSeq(0, 1, 3, 6),
    )
    assertEvalsTo(
      ToArray(streamScanIR(IRStream(1, 2, 3), NA(TInt32))(_.clone + _)),
      FastSeq(null, null, null, null),
    )
    assertEvalsTo(
      ToArray(streamScanIR(IRStream(1, null, 3), NA(TInt32))(_.clone + _)),
      FastSeq(null, null, null, null),
    )
    assertEvalsTo(ToArray(streamScanIR(NA(TStream(TInt32)), 0)(_.clone + _)), null)
    assertEvalsTo(
      ToArray(streamScanIR(MakeStream(IndexedSeq(), TStream(TInt32)), 99)(_.clone + _)),
      FastSeq(99),
    )
    assertEvalsTo(
  ...
  @Test def testNDArrayMap2(): Unit = {
    ...
    val actual = ndMap2(numbers, bools)((n, b) => n.clone + If(b, F64(10), F64(20)))
  ...
  @Test def testNDArrayBroadcasting(): Unit = {
    ...
    def sum(nd1: IR, nd2: IR): IR = ndMap2(nd1, nd2)(_.clone + _)
      ...
      streamAggIR(rangeIR(10)) { elt =>
        aggBindIR(Cast(x * x * elt, TInt64)) { y => // different type to trigger validation errors
          invoke(
            ...
            MakeArray(FastSeq(x.clone), TArray(TInt32)),
          )
        }
      }
    }
  ...
  @Test def testSimpleTailLoop(): Unit = {
    ...
    val triangleSum: IR =
      tailLoop(TInt32, In(0, TInt32), In(1, TInt32)) { case (recur, Seq(x, accum)) =>
        If(x <= 0, accum, recur(FastSeq(x - 1, accum.clone + x)))
      }
    ...
    val triangleSum: IR = tailLoop(TInt32, In(0, TInt32), I32(0)) { case (recur, Seq(x, accum)) =>
      If(
        ...
        tailLoop(TInt32, accum, I32(0)) { case (recur2, Seq(x2, accum2)) =>
          If(x2 <= 0, accum2, recur2(FastSeq(x2 - 5, accum2.clone + x2)))
        },
        recur(FastSeq(x - 1, accum.clone + x)),
      )
    }
  ...
  @Test def testTailLoopNDMemory(): Unit = {
    ...
    val ndSum: IR = tailLoop(
      ...
      If(
        x <= 0,
        accum,
        recur(FastSeq(x - 1, ndMap(accum)(_.clone + x))),
      )
    }
  ...
  @Test def testTreeIRInvariant(): Unit = {
    ...
    TreeIR.verify(ctx, ir1.unsafeClone)
  }
  ...
  @Test def freeVariables(): Unit = {
    ...
    val explodeIR = aggExplodeIR(stream)(x => z + ApplyAggOp(Sum())(x.clone + y))
    ...
    val aggIR = streamAggIR(stream)(x => z + ApplyAggOp(Sum())(x.clone + y))
    assert(
  ...
  @Test def testUUID4(): Unit = {
    ...
    def selfZip(s: => IR, n: Int) =
      ...
      )(elems => MakeArray(elems.map(_.clone), TArray(TString)))
