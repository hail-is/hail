...
class Aggregators2Suite {
  ...
  def assertAggEqualsProcessed(
    ...
    val (_, combAndDuplicate) = CompileWithAggregators[AsmFunction1RegionUnit](
      ctx,
      ArraySeq.fill(nPartitions)(aggSig.state),
      FastSeq(),
      FastSeq(classInfo[Region]),
      UnitInfo,
      Begin(
        ArraySeq.tabulate(nPartitions)(i => DeserializeAggs(i, i, spec, ArraySeq(aggSig.state))) ++
          ArraySeq.range(1, nPartitions).map(i => CombOp(0, i, aggSig)) :+
          SerializeAggs(0, 0, spec, ArraySeq(aggSig.state)) :+
          DeserializeAggs(1, 0, spec, ArraySeq(aggSig.state))
      ),
    )
    ...
    val (Some(PTypeReferenceSingleCodeType(rt: PTuple)), resF) =
      CompileWithAggregators[AsmFunction1RegionLong](
        ...
        FastSeq(classInfo[Region]),
        LongInfo,
        ResultOp.makeTuple(ArraySeq(aggSig, aggSig)),
      )
    ...
    ctx.r.pool.scopedRegion { region =>
      ...
      expectedInit.foreach { v =>
        val (Some(PTypeReferenceSingleCodeType(rt: PBaseStruct)), resOneF) =
          CompileWithAggregators[AsmFunction1RegionLong](
            ...
            FastSeq(classInfo[Region]),
            LongInfo,
            ResultOp.makeTuple(ArraySeq(aggSig)),
          )

        val init = initF(ctx.theHailClassLoader, ctx.fs, ctx.taskContext, region)
        val res = resOneF(ctx.theHailClassLoader, ctx.fs, ctx.taskContext, region)
      ...
      val serializedParts =
        seqOps.grouped(math.ceil(seqOps.length / nPartitions.toDouble).toInt).map { seqs =>
          val init = initF(ctx.theHailClassLoader, ctx.fs, ctx.taskContext, region)
          val seq = withArgs(Begin(seqs))(ctx.theHailClassLoader, ctx.fs, ctx.taskContext, region)
          val write = writeF(ctx.theHailClassLoader, ctx.fs, ctx.taskContext, region)
          ctx.r.pool.scopedSmallRegion { aggRegion =>
            init.newAggState(aggRegion)
            init(region, argOff)
      ...
      ctx.r.pool.scopedSmallRegion { aggRegion =>
        val combOp = combAndDuplicate(ctx.theHailClassLoader, ctx.fs, ctx.taskContext, region)
  ...
  @Test def testCallStats(implicit ctx: ExecuteContext): Unit = {
    ...
    def seqOpArgs(calls: IndexedSeq[Any]) = ArraySeq.tabulate(calls.length)(i =>
      FastSeq[IR](GetField(ArrayRef(a, i), "x"))
    )

    val an = 18
    val ac = ArraySeq(10, 6, 1, 1, 0)
    val af = ac.map(_.toDouble / an)
  ...
  @Test def testTakeBy(implicit ctx: ExecuteContext): Unit = {
    ...
    val seqOpArgs2 = ArraySeq.tabulate(rows2.length)(i =>
    ...
    assertEvalsTo(ta, ArraySeq.tabulate(19)(i => (9999 - i).toString))(
      ctx,
  ...
  @Test def testTake(implicit ctx: ExecuteContext): Unit = {
    ...
    transformations.foreach { case (irF, rowF, subT) =>
      ...
      val seqOpArgs =
        ArraySeq.tabulate(rows.length)(i => FastSeq[IR](irF(ArrayRef(a, i))))
  ...
  @Test def testMin(implicit ctx: ExecuteContext): Unit = {
    ...
    val seqOpArgsNA = ArraySeq.tabulate(8)(i => FastSeq[IR](NA(TInt64)))
  ...
  @Test def testArrayElementsAgg(implicit ctx: ExecuteContext): Unit = {
    ...
    val seq = ArraySeq.tabulate(value.length) { i =>
      seqOpOverArray(
        0,
        ArrayRef(stream, i),
        elt =>
  ...
  @Test def testNestedArrayElementsAgg(implicit ctx: ExecuteContext): Unit = {
    ...
    val seq = ArraySeq.tabulate(10) { i =>
      seqOpOverArray(
        0,
        ArrayRef(stream, i),
  ...
  @Test def testCollectAsSet(implicit ctx: ExecuteContext): Unit = {
    ...
    val eltsPrimitive =
      ArraySeq.tabulate(rows.length)(i => FastSeq(GetField(ArrayRef(rref, i), "b")))
  ...
  @Test def testDownsample(implicit ctx: ExecuteContext): Unit = {
    ...
    assertAggEquals(
      aggSig,
      FastSeq(I32(500)),
      ArraySeq.fill[IndexedSeq[IR]](20)(FastSeq(NA(TFloat64), NA(TFloat64), NA(TArray(TString)))),
      expected = FastSeq(),
      args = FastSeq((a, rows)),
    )
