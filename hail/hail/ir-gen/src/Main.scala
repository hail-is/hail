...
trait IRDSL {
  ...
  type ConstrainedType[+T] <: ConstrainedTypeInterface[T]

  def att(typ: String): ConstrainedType[Attribute]
  def binding: ConstrainedType[Binding]

  def tup[T1, T2](x1: ConstrainedType[T1], x2: ConstrainedType[T2]): ConstrainedType[(T1, T2)]

  def tup[T1, T2, T3](x1: ConstrainedType[T1], x2: ConstrainedType[T2], x3: ConstrainedType[T3])
    : ConstrainedType[(T1, T2, T3)]

  def name: ConstrainedType[Name] = att("Name").asInstanceOf[ConstrainedType[Name]]
  def child: ConstrainedType[Child]
  def tableChild: ConstrainedType[Child]
  def matrixChild: ConstrainedType[Child]
  def bmChild: ConstrainedType[Child]

  trait ConstrainedTypeInterface[+T] {
    def * : ConstrainedType[Seq[T]]
    def + : ConstrainedType[Seq[T]]
    def ? : ConstrainedType[Option[T]]
  }
  ...
  def in[T](name: String, pack: ConstrainedType[T]): Declaration[T]
  ...
  trait NodeDefInterface {
    ...
    def withMethod(methodDef: String): NodeDef
  ...
  implicit def nameDefaultName(t: ConstrainedType[Name]): Declaration[Name] =
    in("name", t)

  implicit def childDefaultName(t: ConstrainedType[Child]): Declaration[Child] =
    in("child", t)
}
...
object IRDSL_Impl extends IRDSL {
  ...
  override def att(typ: String): ConstrainedType[Attribute] = Att_(typ)
  override val binding: ConstrainedType[Binding] = Binding

  override def tup[T1, T2](x1: ConstrainedType[T1], x2: ConstrainedType[T2])
    : ConstrainedType[(T1, T2)] =
    Tup2(x1, x2)

  override def tup[T1, T2, T3](
    x1: ConstrainedType[T1],
    x2: ConstrainedType[T2],
    x3: ConstrainedType[T3],
  ): ConstrainedType[(T1, T2, T3)] =
    Tup3(x1, x2, x3)

  override def child: ConstrainedType[Child] = Child_()
  def child(t: String): ConstrainedType[Child] = Child_(t)
  override def tableChild: ConstrainedType[Child] = child("TableIR")
  override def matrixChild: ConstrainedType[Child] = child("MatrixIR")
  override def bmChild: ConstrainedType[Child] = child("BlockMatrixIR")

  override def in[T](name: String, pack: ConstrainedType[T]): Declaration[T] =
  ...
  case class IntRepr(static: Int = 0, dynamic: String = "") extends Repr[Int] {
    override def typ: ConstrainedType[Int] = Att_[Int]("Int")
  ...
  final case class ChildRepr(typ: Child_, self: String) extends Repr[Child] {
  ...
  final case class BindingRepr(self: String) extends Repr[Binding] {
    override def typ: ConstrainedType[Binding] = Binding
    override def nChildren: IntRepr = 1
  ...
  final case class OptRepr[T](eltType: ConstrainedType[T], self: String) extends Repr[Option[T]] {
    override def typ: ConstrainedType[Option[T]] = Optional(eltType)
    override def toString: String = self
  ...
  object SeqRepr {
    def empty[T](t: ConstrainedType[T]): Static[T] = Static[T](Seq.empty, t)

    trait Unsliced[T] extends SeqRepr[T]

    final case class Static[T](elts: Seq[Repr[T]], eltType: ConstrainedType[T])
        extends Unsliced[T] {
      override def typ: Collection[T] = Collection(eltType)
    ...
    final case class Dynamic[T](elts: String, eltType: ConstrainedType[T]) extends Unsliced[T] {
      override def typ: Collection[T] = Collection(eltType)
    ...
    final private case class Range[T](
      ...
      override def eltType: ConstrainedType[T] = seq.eltType
  ...
  sealed abstract class ConstrainedType[+T] extends ConstrainedTypeInterface[T] {
    override def * : ConstrainedType[Seq[T]] = Collection(this)
    override def + : ConstrainedType[Seq[T]] = Collection(this, allowEmpty = false)
    override def ? : ConstrainedType[Option[T]] = Optional(this)
  ...
  final case class Att_[T](typ: String) extends ConstrainedType[T] {
  ...
  final case class Child_(t: String = "IR") extends ConstrainedType[Child] {
  ...
  case object Binding extends ConstrainedType[Binding] {
    override def generateDeclaration: String = "Binding"
    override def repr(self: String): Repr[Binding] = BindingRepr(self)
  }

  final case class Optional[T](elt: ConstrainedType[T]) extends ConstrainedType[Option[T]] {
  ...
  final case class Collection[T](elt: ConstrainedType[T], allowEmpty: Boolean = true)
      extends ConstrainedType[Seq[T]] {
  ...
  abstract class Tup[T](val elts: ConstrainedType[_]*) extends ConstrainedType[T] {
  ...
  case class Tup2[T1, T2](x1: ConstrainedType[T1], x2: ConstrainedType[T2])
      extends Tup[(T1, T2)](x1, x2)
  ...
  case class NodeDef(
    ...
    override def withMethod(methodDef: String): NodeDef =
...
object Main {
  ...
  private def allNodes: Seq[NodeDef] = {
    ...
    r += node(
      ...
      in("seqOps", child),
      name,
      in("aggSignature", att("IndexedSeq[PhysicalAggSig]")),
    ...
    r += node(
      ...
      in("left", child),
      in("right", child),
      in("lKey", att("IndexedSeq[String]")),
      in("rKey", att("IndexedSeq[String]")),
      in("l", name),
      in("r", name),
      in("joinF", child),
    ...
    r += node(
      ...
      .withTraits(NDArrayIR)
    r += node("NDArrayReindex", in("nd", child), in("indexExpr", att("IndexedSeq[Int]")))
      .withTraits(NDArrayIR)
    r += node("NDArrayAgg", in("nd", child), in("axes", att("IndexedSeq[Int]")))
    ...
    r += node(
      ...
      in("aggSigs", att("IndexedSeq[AggStateSig]")),
    )
    r += node(
      "DeserializeAggs",
      in("startIdx", att("Int")),
    ...
    r += node(
      ...
      in("body", child),
      in("result", child),
      in("signature", att("IndexedSeq[AggStateSig]")),
    )
    r += node(
      "RunAggScan",
      in("array", child),
    ...
    r += node("SelectFields", in("old", child), in("fields", att("IndexedSeq[String]")))
      .typed("TStruct")
    r += node(
      "InsertFields",
      in("old", child),
      in("fields", tup(att("String"), child).*),
      in("fieldOrder", att("Option[IndexedSeq[String]]")).withDefault("None"),
    )
      .typed("TStruct")
