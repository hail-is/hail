...
final class LEB128InputBuffer(in: InputBuffer) extends InputBuffer {
  ...
  override def readBytes(toRegion: Region, toOff: Long, n: Int): Unit = in.readBytes(toRegion, toOff, n)
  ...
  override def readDoubles(to: Array[Double], toOff: Int, n: Int): Unit = in.readDoubles(to, toOff, n)
}
...
final class StreamBlockInputBuffer(in: InputStream) extends InputBlockBuffer {
  ...
  override def seek(offset: Long): Unit = in.asInstanceOf[ByteTrackingInputStream].seek(offset >> 16)
