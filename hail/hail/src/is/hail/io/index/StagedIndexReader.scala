...
import is.hail.expr.ir.{
  BinarySearch, EmitCode, EmitCodeBuilder, EmitMethodBuilder, EmitValue, IEmitCode, TaskFinalizer,
}
...
class StagedIndexReader(
  ...
  private[this] val internalPType =
    internalCodec.encodedType.decodedPType(internalCodec.encodedVirtualType)

  private[this] val leafChildType = leafPType.asInstanceOf[PCanonicalBaseStruct].types(
    1
  ).asInstanceOf[PCanonicalArray].elementType.sType.asInstanceOf[SBaseStruct]

  private[this] val leafChildLocalType =
    SStackStruct(leafChildType.virtualType, leafChildType.fieldEmitTypes)

  private[this] val queryResultStartIndex: Settable[Long] =
    emb.genFieldThisRef[Long]("index_resultIndex")

  private[this] val queryResultStartLeaf: SSettable =
    emb.newPField("index_resultOffset", leafChildLocalType)

  private[this] val leafDec =
    leafCodec.encodedType.buildDecoder(leafCodec.encodedVirtualType, emb.ecb)

  private[this] val internalDec =
    internalCodec.encodedType.buildDecoder(internalCodec.encodedVirtualType, emb.ecb)
  ...
  def queryBound(cb: EmitCodeBuilder, endpoint: SBaseStructValue, leansRight: Value[Boolean])
    : SBaseStructValue = {
    ...
    SStackStruct.constructFromArgs(
      cb,
      null,
      TTuple(TInt64, leafChildType.virtualType),
      EmitCode.present(cb.emb, primitive(index)),
      EmitCode.fromI(cb.emb)(cb => IEmitCode(cb, index ceq nKeys, leaf)),
    )
  }

  /** returns tuple of (start index, end index, starting leaf) memory of starting leaf is not owned
    * by `region`, consumers should deep copy if necessary starting leaf IS MISSING if (end_idx -
    * start_idx == 0)
    */
  def queryInterval(cb: EmitCodeBuilder, interval: SIntervalValue): SBaseStructValue = {
    ...
    val stackInterval = SStackInterval.construct(
      EmitValue.present(startKey),
      EmitValue.present(endKey),
      cb.memoize(!startLeansRight),
      endLeansRight,
    )
    ...
    cb.if_(
      ...
      cb._fatal(
        ...
        endIdx.toS,
        ", query=",
        cb.strValue(interval),
      ),
    )
    cb.if_(
      n > 0L && startIdx >= nKeys,
      cb._fatal("bad start idx: ", startIdx.toS, ", nKeys=", nKeys.toS),
    )

    SStackStruct.constructFromArgs(
      cb,
      null,
      TTuple(TInt64, TInt64, leafChildType.virtualType),
  ...
  private[this] def queryBound(
    ...
    cb.invokeVoid(
      cb.emb.ecb.getOrGenEmitMethod(
        "queryBound",
        ("queryBound", this),
        FastSeq(
          endpoint.st.paramType,
          typeInfo[Boolean],
          typeInfo[Int],
          typeInfo[Long],
          typeInfo[Long],
          leafChildLocalType.paramType,
        ),
        UnitInfo,
      ) { emb =>
        emb.emitWithBuilder { cb =>
          ...
          val interval = SStackInterval.construct(
            EmitValue.present(endpoint),
            EmitValue.present(endpoint),
            cb.memoize(!leansRight),
            leansRight,
          )
          val (startIndex, startLeaf, _) = runQuery(cb, interval, rootLevel, rootOffset,
            rootSuccessorIndex, rootSuccessorLeaf, isPointQuery = true)
  ...
  private[this] def runQuery(
    ...
    def searchChildren(children: SIndexableValue, isInternalNode: Boolean)
      ...
      if (isPointQuery) {
        ...
        arrayOfStructFindIntervalRange(
          cb,
          children,
          startKey,
          startLeansRight,
    ...
    val startLeaf: SStackStructSettable =
      cb.newSLocal(leafChildLocalType, "startOffset").asInstanceOf[SStackStructSettable]
    val endIndex: Settable[Long] = cb.newLocal[Long]("endIndex")

    val successorIndex: Settable[Long] =
      cb.newLocal[Long]("queryInterval_successorIndex", rootSuccessorIndex)
    val successorLeaf: SStackStructSettable =
      cb.newSLocal(leafChildLocalType, "successorLeaf").asInstanceOf[SStackStructSettable]
    ...
    cb.if_(
      level > 0, {
        /* InternalNode( children: IndexedSeq[InternalChild]) InternalChild( index_file_offset:
        ...
        cb.if_(
          if (isPointQuery) const(true).get else start.ceq(end), {
        ...
        cb.if_(
          start < children.loadLength, {
            ...
            cb.if_(
              successorIndex.cne(firstIndex + start.toL),
              cb._fatal("queryInterval broken invariant"),
            )
  ...
  def queryIndex(cb: EmitCodeBuilder, region: Value[Region], absIndex: Value[Long])
    : SBaseStructValue = {
    cb.invokeSCode(
      cb.emb.ecb.getOrGenEmitMethod(
        "queryIndex",
