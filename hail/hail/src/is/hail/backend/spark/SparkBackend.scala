...
import is.hail.utils.compat.ArraySeq
  ...
  override def driverContext(ctx: ExecuteContext): DriverContext =
    new DriverContext {
      ...
      override def collectDistributedArray(
        ...
        type R = (Array[Byte], Int)

        val buffer: mutable.Builder[R, ArraySeq[R]] =
          ArraySeq.newBuilder

        buffer.sizeHint(partsToRun.length)
        ...
        try {
          for (subparts <- partsToRun.grouped(maxStageParallelism)) {
            sc.runJob(
              rdd,
