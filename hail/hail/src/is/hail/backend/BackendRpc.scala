        ...
        Context.scoped(env) { ctx =>
          command match {
            ...
            case Execute(s, fns, codec) =>
              val bufferSpec = BufferSpec.parseOrDefault(ctx, codec)
            ...
            case LoadReferencesFromFASTA(name, fasta, index, xContigs, yContigs, mtContigs, par) =>
              jsonToBytes {
                ...
                Context.putReferences(env)(FastSeq(rg))
                rg.toJSON
              }
          }
        }
...
trait HttpLikeRpc extends BackendRpc {
  ...
  private[this] case class IRTypePayload(ir: String)
  private[this] case class LoadReferencesFromDatasetPayload(path: String)

  private[this] case class FromFASTAFilePayload(
    name: String,
    fasta_file: String,
    index_file: String,
    x_contigs: Array[String],
  ...
  private[this] case class ParseVCFMetadataPayload(path: String)

  private[this] case class ImportFamPayload(
    path: String,
    quant_pheno: Boolean,
    delimiter: String,
    missing: String,
  ...
  private[this] case class ExecutePayload(
    ir: String,
