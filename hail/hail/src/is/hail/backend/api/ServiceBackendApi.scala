...
import is.hail.backend.{Backend, ExecuteContext, HttpLikeRpc, OwningTempFileManager}
...
import is.hail.utils.{
  toRichIterable, using, ErrorHandling, ExecutionTimer, HailWorkerException, ImmutableMap, Logging,
}
...
object ServiceBackendApi extends HttpLikeRpc with Logging {
  ...
  implicit object Reader extends Routing {
    import Routes._
    ...
    override def route(env: Env): Route =
      (env.action: @switch) match {
        case 1 => TypeOf(Value)
        case 2 => TypeOf(Table)
        case 3 => TypeOf(Matrix)
        case 4 => TypeOf(BlockMatrix)
        case 5 => Execute
  ...
  implicit object Writer extends Writer with ErrorHandling {
    ...
    override def scoped[A](env: Env)(f: ExecuteContext => A): (A, Timings) =
      ExecutionTimer.time { timer =>
        ExecuteContext.scoped(
          ...
          env.fs,
          timer,
          new OwningTempFileManager(env.fs),
          env.hcl,
          env.flags,
          new IrMetadata(),
          ImmutableMap.empty,
