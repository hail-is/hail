...
object FutureOps {
  // "unsafe" in that sequencing Future[Lazy[_]] does not guarantee completion order
  def unsafeInterleave[A](f: => A)(implicit ec: ExecutionContext): Future[Lazy[A]] =
    Future {
      val fut = Future(f)
      lazily[A](Await.result(fut, Duration.Inf))
    }
}
...
object Worker {
  ...
  def main(argv: Array[String]): Unit = {
    ...
    val (globals, context, f) =
      timer.enter("read inputs") {
        val downloads: Future[(Array[Byte], Array[Byte], PartitionFn)] =
          for {
            globals <- unsafeInterleave {
              retryTransientErrors {
                using(open(s"$root/globals"))(_.readAllBytes())
              }
            }

            context <- unsafeInterleave {
              retryTransientErrors {
          ...
          } yield (globals, context, partitionFn)
