      ...
      override def collectDistributedArray(
        ...
        partitions.getOrElse(contexts.indices) match {
          ...
          case todo =>
            ...
            val root = s"${ctx.tmpdir}/collectDistributedArray/$token"
            log.info(s"collectDistributedArray: token='$token', nPartitions=${todo.length}")

            val uploadGlobals =
              executor.submit[Unit] { () =>
                retryTransientErrors {
                  ctx.fs.writePDOS(s"$root/globals")(_.write(globals))
                  log.info(s"collectDistributedArray: $token: uploaded globals")
                }
              }
              ...
              executor.submit[Unit] { () =>
                ...
                retryTransientErrors {
                  ctx.fs.writePDOS(s"$root/f") { fos =>
                    ...
                    log.info(s"collectDistributedArray: $token: uploaded function")
                  }
                }
              }
              ...
              executor.submit[Unit] { () =>
                retryTransientErrors {
                  ctx.fs.writePDOS(s"$root/contexts") { os =>
                    ...
                    log.info(s"collectDistributedArray: $token: wrote ${parts.length} contexts")
                  }
                }
              }
            ...
            log.info(s"collectDistributedArray: $token: reading results")
              ...
              jobGroup.state match {
                case Success =>
                  runAllKeepFirstError(executor) {
                    (todo, parts.indices).zipped.map { (partIdx, jobIndex) =>
            ...
            val byterate = results.map(_._1.length).sum / end / 1024 / 1024
