...
import java.io._
import java.util.concurrent.{ExecutorCompletionService, Executors, Future => JFuture}
      ...
      private[this] def collect(root: String, jobGroupId: Int, startJobId: Int, nJobs: Int)
        ...
        def drain(jobs: Iterable[(Int, Option[String])]): Unit = {
          ...
          while (inFlight > 0) {
            ...
            // Drain any others that also completed
        ...
        retryable { attempt =>
          batchClient.getJobGroup(batchId, jobGroupId).state match {
            case Running =>
              try {
                ...
                case _: InterruptedException =>
                  ...
                  val failure = new CancellationException("Cancelled by user").fillInStackTrace()
                  (Some(failure), ArraySeq.empty)
                case t: Throwable if !t.isInstanceOf[ControlThrowable] =>
                  batchClient.cancelJobGroup(batchId, jobGroupId)
                  (Some(t), ArraySeq.empty)
              }
            ...
            case Success =>
              ...
              (None, ArraySeq.unsafeWrapArray(results))
            ...
            case Cancelled =>
              /* The user cancelled the batch - when would we preserve these results for
               * call-caching? */
              val msg = s"Job group $jobGroupId in batch $batchId was cancelled."
              (Some(new CancellationException(msg).fillInStackTrace()), ArraySeq.empty)
          }
        }
      }
