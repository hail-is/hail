...
object CallN {
  def apply(alleles: java.util.List[Int], phased: Boolean): Call =
    apply(alleles.asScala.toFastSeq, phased)
  ...
  def downcode(c: Call, i: Int): Call = {
    (Call.ploidy(c): @switch) match {
      ...
      case 2 =>
        ...
        Call2(
          if (AllelePair.j(p) == i) 1 else 0,
          if (AllelePair.k(p) == i) 1 else 0,
          Call.isPhased(c),
        )
      case _ =>
    ...
    (ploidy(c): @switch) match {
      ...
      case 1 =>
        if (phased)
          ...
          void(sb ++= alleleByIndex(c, 0).toString)
      case 2 =>
        val p = allelePair(c)
        void(sb ++= AllelePair.j(p).toString ++= sep ++= AllelePair.k(p).toString)
      case _ =>
        var i = 0
        val nAlleles = ploidy(c)
        while (i < nAlleles) {
          void(sb ++= alleleByIndex(c, i).toString)
          if (i != nAlleles - 1)
            void(sb ++= sep)
          i += 1
        }
