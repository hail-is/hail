...
import is.hail.expr.ir.{Compiled, InferPType}
...
class RVD(
  ...
  def stabilize(ctx: ExecuteContext, enc: AbstractTypedCodecSpec): RDD[Array[Byte]] = {
    ...
    crdd.cmapPartitions((hcl, ctx, it) => RegionValue.toBytes(hcl, makeEnc, ctx.region, it)).run
  }
  ...
  def keyedEncodedRDD(
    ...
    crdd.cmapPartitions { (hcl, ctx, it) =>
      ...
      it.map { ptr =>
        val keys: Any = SafeRow.selectFields(localRowPType, ctx.region, ptr)(kFieldIdx)
        val bytes = encoder.regionValueToBytes(ctx.region, ptr)
        (keys, bytes)
      }
    }.run
  }
  ...
  def repartition(
    ...
    if (shuffle) {
      ...
      val filtered: RVD = if (filter) filterWithContext[(UnsafeRow, SelectFieldsRow)](
        ...
        { case ((ur, key), _, ctx, ptr) =>
          ur.set(ctx.region, ptr)
  ...
  def distinctByKey(execCtx: ExecuteContext): RVD = {
    ...
    repartition(execCtx, partitioner.strictify())
      .mapPartitions(typ)((_, ctx, it) =>
        OrderedRVIterator(localType, it.toIteratorRV(ctx.region), ctx, sm)
          .staircase
          .map(_.value.offset)
      )
  }
  ...
  def localSort(newKey: IndexedSeq[String]): RVD = {
    ...
    val sortedRDD = crdd.toCRDDRegionValue.cmapPartitions { (_, consumerCtx, it) =>
  ...
  def map[T: ClassTag](f: (HailClassLoader, RVDContext, Long) => T): RDD[T] =
    crdd.cmap(f).run

  def map(newTyp: RVDType)(f: (HailClassLoader, RVDContext, Long) => Long): RVD = {
  ...
  def mapPartitions[T: ClassTag](
    f: (HailClassLoader, RVDContext, Iterator[Long]) => Iterator[T]
  ): RDD[T] = crdd.cmapPartitions(f).run

  def mapPartitions(
    newTyp: RVDType
  )(
    f: (HailClassLoader, RVDContext, Iterator[Long]) => Iterator[Long]
  ): RVD = {
    require(newTyp.kType isPrefixOf typ.kType)
    RVD(
      newTyp,
  ...
  def mapPartitionsWithContext(
    ...
    f: (HailClassLoader, RVDContext, Element[Long]) => Iterator[Long]
  ): RVD =
    RVD(
      newTyp,
      partitioner.coarsen(newTyp.key.length),
  ...
  def mapPartitionsWithContextAndIndex(
    ...
    f: (Int, HailClassLoader, RVDContext, Element[Long]) => Iterator[Long]
  ): RVD =
    RVD(
      newTyp,
      partitioner.coarsen(newTyp.key.length),
  ...
  def mapPartitionsWithIndex[T: ClassTag](
    f: (Int, HailClassLoader, RVDContext, Iterator[Long]) => Iterator[T]
  ): RDD[T] = crdd.cmapPartitionsWithIndex(f).run

  def mapPartitionsWithIndex(
    newTyp: RVDType
  )(
    f: (Int, HailClassLoader, RVDContext, Iterator[Long]) => Iterator[Long]
  ): RVD = {
    require(newTyp.kType isPrefixOf typ.kType)
    RVD(
      newTyp,
  ...
  def mapPartitionsWithIndexAndValue[V](
    ...
    f: (Int, HailClassLoader, RVDContext, V, Iterator[Long]) => Iterator[Long]
  ): RVD = {
    require(newTyp.kType isPrefixOf typ.kType)
    RVD(
      newTyp,
  ...
  def filter(p: (RVDContext, Long) => Boolean): RVD =
    filterWithContext((_, _, _) => (), (_: Any, _, c, l) => p(c, l))

  def filterWithContext[C](
    makeContext: (Int, HailClassLoader, RVDContext) => C,
    f: (C, HailClassLoader, RVDContext, Long) => Boolean,
  ): RVD = {
    val crdd: ContextRDD[Long] =
      this.crdd.cmapPartitionsWithContextAndIndex { (i, hcl, consumerCtx, iteratorToFilter) =>
        val c = makeContext(i, hcl, consumerCtx)
        val producerCtx = consumerCtx.freshContext()
        iteratorToFilter(hcl, producerCtx).filter { ptr =>
          val b = f(c, hcl, consumerCtx, ptr)
          if (b) {
  ...
  def filterOutIntervals(intervals: RVDPartitioner): RVD = {
    ...
    filterWithContext[UnsafeRow](
      ...
      { case (kUR, _, ctx, ptr) =>
        ctx.rvb.start(kType)
        ctx.rvb.selectRegionValue(rowPType, kRowFieldIdx, ctx.region, ptr)
  ...
  def filterToIntervals(intervals: RVDPartitioner): RVD = {
    ...
    val pred: (RVDContext, Long) => Boolean = (ctx: RVDContext, ptr: Long) => {
      val ur = new UnsafeRow(localRowPType, ctx.region, ptr)
      val key = Row.fromSeq(
  ...
  def combine[U: ClassTag, T: ClassTag](
    ...
    mkZero: Compiled[Region => T],
    itF: Compiled[(Region, Iterator[Long]) => T],
    deserialize: Compiled[(Region, U) => T],
    serialize: Compiled[(Region, T) => U],
    combOp: Compiled[(Region, T, T) => T],
    commutative: Boolean,
    treeBranchFactor: Option[Int],
  ): Compiled[Region => T] = {

    var reduced = crdd.cmapPartitions[U] { (hcl, ctx, it) =>
      val write = serialize(hcl, fsBc.value, ctx, ctx.r)
      val elem = itF(hcl, fsBc.value, ctx, ctx.r)
      ctx.r.pool.scopedSmallRegion { aggRegion =>
        Iterator.single(write(aggRegion, elem(aggRegion, it)))
      }
    }

    treeBranchFactor.foreach { factor =>
      val depth = treeAggDepth(getNumPartitions, factor)
      val scale = math.max(
        ...
        reduced =
          reduced
            ...
            .cmapPartitions { (hcl, ctx, it) =>
              ...
              val zero = mkZero(hcl, fs, ctx, ctx.r)
              val read = deserialize(hcl, fs, ctx, ctx.r)
              val write = serialize(hcl, fs, ctx, ctx.r)
              val combine = combOp(hcl, fs, ctx, ctx.r)

              ctx.r.pool.scopedSmallRegion { r =>
                var acc = zero(ctx.r)

                for ((_, (_, v)) <- it) {
                  acc = combine(ctx.r, acc, read(r, v))
                  r.clear()
                }

                Iterator.single(write(ctx.r, acc))
              }
            }
    ...
    for {
      ...
      val ac =
        Combiner(
          zero = zeroFn(r),
          combine = combineFn(r, _, _),
          commutative = commutative,
          associative = true,
        )

      r.pool.scopedSmallRegion { inner =>
        sparkContext.runJob(
          reduced.run,
          (it: Iterator[U]) => singletonElement(it),
          (i, x: U) => {
            ac.combine(i, read(inner, x)); inner.clear()
          },
        )
      }
  ...
  def count(): Long =
    crdd.boundary.cmapPartitions { (_, _, it) =>
      var count = 0L
  ...
  def collect(execCtx: ExecuteContext): Array[Row] = {
    ...
    execCtx.scopedExecution { (hcl, _, _, region) =>
      RegionValue.fromBytes(hcl, dec, region, encodedData.iterator)
        .map { ptr =>
  ...
  def alignAndZipPartitions(
    ...
    zipper: (HailClassLoader, RVDContext, Iterator[RegionValue], Iterator[RegionValue]) => Iterator[
      RegionValue
    ]
  ): RVD = {
  ...
  def intervalAlignAndZipPartitions(
    ...
    zipper: PStruct => (
      RVDType,
      (HailClassLoader, RVDContext, Iterator[RegionValue], Iterator[RegionValue]) => Iterator[
        RegionValue
      ],
    )
  ): RVD = {
    ...
    val partitionKeyedIntervals = that.crdd.cmapPartitions { (hcl, ctx, it) =>
      val encoder = new ByteArrayEncoder(hcl, makeEnc)
      TaskContext.get().addTaskCompletionListener[Unit](_ => encoder.close()): Unit
      it.flatMap { ptr =>
  ...
  private[rvd] def destabilize(
    ...
    (
      rowPType,
      ContextRDD.weaken(stable).cmapPartitions { (hcl, ctx, it) =>
...
object RVD extends Logging {
  ...
  def getKeys(
    ...
    val localType = typ
    val sm = ctx.stateManager
    crdd.cmapPartitionsWithContext { (hcl, consumerCtx, it) =>
      val producerCtx = consumerCtx.freshContext()
      val wrv = WritableRegionValue(sm, localType.kType, consumerCtx.region)
      it(hcl, producerCtx).map { ptr =>
  ...
  def getKeyInfo(
    ...
    val sm = ctx.stateManager
    val keyInfo = keys.cmapPartitionsWithIndex { (i, _, ctx, it) =>
      if (it.hasNext)
        Iterator.single(RVDPartitionInfo(
          ctx,
          sm,
          localType,
          partitionKey,
  ...
  def unify(execCtx: ExecuteContext, rvds: Seq[RVD]): Seq[RVD] = {
    ...
    rvds.map { rvd =>
      ...
      rvd.map(newRVDType)((_, ctx, ptr) =>
        unifiedRowPType.copyFromAddress(sm, ctx.region, srcRowPType, ptr, false)
      )
    }
  }
...
class BroadcastRVD(backend: SparkBackend, rvd: RVD) extends Serializable {
  ...
  def computePartition(idx: Int, hcl: HailClassLoader, region: Region, partitionRegion: Region)
    : Iterator[Long] = {
    val ctx = new RVDContext(partitionRegion, region)
    val it = crdd.iterator(bcPartitions.value(idx), TaskContext.get())
    it.flatMap(_.apply(hcl, ctx))
  }
}
