...
/** Rich wrapper for an indexed sequence.
  ...
  *   - a search key 'x: U',
  *   - a key comparison 'lt: (U, U) => Boolean' (the most generic versions allow the search key 'x'
  *     to be of a different type than the elements of the sequence, and take one or two mixed type
  *     comparison functions),
  *   - and a key projection 'k: (T) => U',
  * assume the following preconditions for all 0 <= i <= j < a.size (writing < for 'lt'):
  *   1. if 'x' < k(a(i)) then 'x' < k(a(j)) 2. if k(a(j)) < 'x' then k(a(i)) < 'x'
  * These can be rephrased as 1: 'x' < k(_) partitions a, and 2: k(_) < 'x' partitions a. (Actually,
  * upperBound only needs 1. and lowerBound only needs 2.)
  */
class RichIndexedSeq[T](val a: IndexedSeq[T]) extends AnyVal {
  ...
  /** Returns 'start' <= i <= 'end' such that p(k(a(j))) is false for all j in ['start', i), and
    * p(k(a(j))) is true for all j in [i, 'end').
    *
    * Assumes p(k(_)) partitions a, i.e. for all 0 <= i <= j < a.size, if p(k(a(i))) then
    * p(k(a(j))).
    */
  ...
  /** Perform binary search until either an index i is found for which k(a(i)) is incomparible with
    * 'x', or it is certain that no such i exists. In the first case, call 'found'(l, i, u), where
    * [l, u] is the current range of the search. In the second case, call 'notFound'(j), where
    * k(a(i)) < x for all i in [0, j) and x < k(a(i)) for all i in [j, a.size).
    */
