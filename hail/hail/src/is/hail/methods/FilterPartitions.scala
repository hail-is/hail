...
import is.hail.types.virtual.{MatrixType, TableType}

case class TableFilterPartitions(parts: IndexedSeq[Int], keep: Boolean)
...
case class MatrixFilterPartitions(parts: IndexedSeq[Int], keep: Boolean)
    extends MatrixToMatrixFunction {
  override def preservesPartitionCounts: Boolean = false
