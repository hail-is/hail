...
class RichCodePrefixCoder(val pc: Value[PrefixCoder]) {
  def toByteArray(cb: EmitCodeBuilder): Value[Array[Byte]] = cb.memoize(pc.invoke[Array[Byte]]("toByteArray"))

  def encodeBool(cb: EmitCodeBuilder, b: Code[Boolean]) = cb += pc.invoke[Boolean, Unit]("encodeBool", b)
  ...
  def encodeFloat(cb: EmitCodeBuilder, v: Code[Float]) = cb += pc.invoke[Float, Unit]("encodeFloat", v)

  def encodeDouble(cb: EmitCodeBuilder, v: Code[Double]) = cb += pc.invoke[Double, Unit]("encodeDouble", v)
  ...
  def writeBytes(cb: EmitCodeBuilder, bs: Code[Array[Byte]]) = cb += pc.invoke[Array[Byte], Unit]("writeBytes", bs)
}
