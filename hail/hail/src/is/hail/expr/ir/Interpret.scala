...
object Interpret extends Logging {
  ...
  private def run(
    ...
    def interpret(ir: IR, env: Env[Any], args: IndexedSeq[(Any, Type)]): Any =
      ir match {
        case I32(x) => x
        case I64(x) => x
        case F32(x) => x
        ...
        case x @ EncodedLiteral(codec, value) =>
          ctx.scopedExecution { (_, _, _, r) =>
            val (pt, addr) = codec.decodeArrays(ctx, x.typ, value.ba, r)
        ...
        case CastRename(v, _) => interpret(v, env, args)
        case NA(_) => null
        ...
        case StreamRange(start, stop, step, _, errorID) =>
          ...
          val stepValue = interpret(step, env, args)
          if (stepValue == 0) fatal("Array range cannot have step size 0.", errorID)
          else if (startValue == null || stopValue == null || stepValue == null) null
          else Range(
            startValue.asInstanceOf[Int],
            stopValue.asInstanceOf[Int],
        ...
        case _: CastToArray | _: ToArray | _: ToStream =>
          ...
          if (cValue == null)
            ...
            val ordering =
              tcoerce[TIterable](c.typ).elementType.ordering(ctx.stateManager).toOrdering
            cValue match {
              case s: Set[_] =>
        ...
        case StreamZip(as, names, body, behavior, errorID) =>
          ...
          if (aValues.contains(null))
            ...
            (0 until len).map { i =>
              val e =
                env.bindIterable(names.zip(aValues.map(a =>
                  if (i >= a.length) null else a.apply(i)
                )))
          ...
          if (streams.contains(null))
            ...
            def advance(i: Int): Unit = {
              ...
              while (j != 0 && tournament(j) != -1) {
                val challenger = tournament(j)
                if (
                  challenger != k && (winner == k || ordering.lteq(get(challenger), get(winner)))
                ) {
                  tournament(j) = winner
                  winner = challenger
                }
        ...
        case Die(message, _, errorId) =>
          val message_ = interpret(message, env, args).asInstanceOf[String]
          fatal(if (message_ != null) message_ else "<exception message missing>", errorId)
        case Trap(child) =>
          try
            Row(null, interpret(child, env, args))
          catch {
        ...
        case ir: AbstractApplyNode[_] =>
          val in = Ref(freshName(), TTuple(ir.args.map(_.typ): _*))
          val argPType = PType.canonical(in.typ).setRequired(true).asInstanceOf[PTuple]
          ...
          val (Some(rt), mkApply) =
            Compile[AsmFunction2RegionLongLong](
              ...
              FastSeq((
                in.name,
                SingleCodeEmitParamType(true, PTypeReferenceSingleCodeType(argPType)),
              )),
          ...
          val inArgs = ir.args.map(interpret(_, env, args))
          ...
          ctx.scopedExecution { (hcl, fs, htc, r) =>
            ...
            rvb.start(argPType)
            rvb.startTuple()
            for ((argv, t) <- inArgs.zip(argPType.types))
              rvb.addAnnotation(t.virtualType, argv)
            rvb.endTuple()
            val offset = rvb.end()
        ...
        case x @ TableAggregate(child, query) =>
          ...
          val wrapped = if (aggSigs.isEmpty) {
            ...
            val read: Compiled[(Region, WrappedByteArray) => Long] =
              aggSigs.deserialize(ctx, spec).map { f => (r, a) =>
                val res = f(r, a.bytes)
                a.clear()
                res
              }

            val write: Compiled[(Region, Long) => WrappedByteArray] =
              aggSigs.serialize(ctx, spec).map { fn => (r, offset) =>
                new WrappedByteArray(fn(r, offset))
              }

            val combOpF = aggSigs.combOpF(ctx, spec)

            val seqF: Compiled[(Region, Iterator[Long]) => Long] = {
              (hcl, fs, ctx, r) =>
                val globalsOffset = globalsBc.value.readRegionValue(r, hcl)
                val init = initOp(hcl, fs, ctx, r)
                val seqOps = partitionOpSeq(hcl, fs, ctx, r)
                (aggRegion, it) =>
                  init.newAggState(aggRegion)
                  init(r, globalsOffset)
                  seqOps.setAggState(aggRegion, init.getAggOffset())

                  aggRegion.pool.scopedSmallRegion { inner =>
                    for (ptr <- it) {
                      seqOps(inner, globalsOffset, ptr)
                      inner.clear()
                    }
                  }
            ...
            // writes the `Zero` value into the caller-provided region
            val mkZero: Compiled[Region => Long] = { (hcl, fs, htc, outer) =>
              val initF = initOp(hcl, fs, htc, outer)
              r =>
                initF.newAggState(r)
                initF(r, globalsBc.value.readRegionValue(r, hcl))
                initF.getAggOffset()
            }

            val rvF =
              value.rvd.combine[WrappedByteArray, Long](
                ctx.fsBc,
                mkZero,
                seqF,
                read,
                ...
                combOpF,
                isCommutative,
                someIf(useTreeAggregate, ctx.branchingFactor),
              )
            ...
            ctx.scopedExecution { (hcl, fs, htc, r) =>
              val offset = rvF(hcl, fs, htc, r)(r)
              val resF = f(hcl, fs, htc, r)

              resF.setAggState(r, offset)
    ...
    ctx.time(interpret(ir, env, args))
  }
}
