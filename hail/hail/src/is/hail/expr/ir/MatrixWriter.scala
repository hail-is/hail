    ...
    new MatrixWriterComponents {
      ...
      override val stage: TableStage =
        lowered.mapContexts { oldCtx =>
          ...
          )((ctxElt, pf) => makestruct("oldCtx" -> ctxElt, "writeCtx" -> pf))
        }(GetField(_, "oldCtx"))
      ...
      override def finalizeWrite(parts: Atom, globals: Atom): IR =
        M.eval {
          for {
            ...
            _ <- WriteMetadata(
              MakeArray(
                selectIR(colInfo, "partitionCounts", "distinctlyKeyed", "firstKey", "lastKey")
              ),
              TableSpecWriter(
                s"$path/cols",
            ...
            files <- mapArray(parts)(GetField(_, "filePath"))

            rowsIndexSpec = IndexSpec.defaultAnnotation(ctx, "../../index", pKey)
            _ <- WriteMetadata(
              files,
              RVDSpecWriter(
                s"$path/rows/rows",
            ...
            entriesIndexSpec =
              IndexSpec.defaultAnnotation(ctx, "../../index", pKey, withOffsetField = true)
            ...
            _ <- WriteMetadata(
              mapArray(parts) { part =>
                insertIR(
                  selectIR(part, "partitionCounts", "distinctlyKeyed"),
                  "firstKey" -> makestruct(),
                  "lastKey" -> makestruct(),
                )
              },
            ...
            _ <- WriteMetadata(
              makestruct(
                ...
                "rows" -> mapArray(parts)(part => GetField(part, "partitionCounts")),
              ),
              MatrixSpecWriter(path, tm, "rows/rows", "globals/rows", "cols/rows", "entries/rows"),
            )

            _ <- WriteMetadata(makestruct(), RelationalCommit(path))
            _ <- WriteMetadata(makestruct(), RelationalCommit(s"$path/globals"))
            _ <- WriteMetadata(makestruct(), RelationalCommit(s"$path/cols"))
            _ <- WriteMetadata(makestruct(), RelationalCommit(s"$path/rows"))
            _ <- WriteMetadata(makestruct(), RelationalCommit(s"$path/entries"))
          } yield Void()
        }
    }
  }
  ...
  override def lower(
    ...
    ts.mapContexts { oldCtx =>
      ...
      zip2(oldCtx, ToStream(partFiles), ArrayZipBehavior.AssertSameLength) { (ctxElt, pf) =>
        makestruct("__old_ctx" -> ctxElt, "__part_file" -> pf)
      }
    }(GetField(_, "__old_ctx"))
      .mapCollectWithContextsAndGlobals("matrix_vcf_writer") { (rows, ctxRef) =>
        val partFile = GetField(ctxRef, "__part_file") + UUID4() + Str(ext)
        val ctx = makestruct("cols" -> GetField(ts.globals, colsFieldName), "partFile" -> partFile)
        WritePartition(rows, ctx, lineWriter)
      } { (parts, globals) =>
        val ctx = makestruct("cols" -> GetField(globals, colsFieldName), "partFiles" -> parts)
        val commit = VCFExportFinalizer(tm, path, appendStr, metadata, exportType, tabix)
        WriteMetadata(ctx, commit)
      }
  }
...
case class MatrixGENWriter(
  ...
  override def lower(
    ...
    ts.mapContexts { oldCtx =>
      ...
      zip2(oldCtx, ToStream(partFiles), ArrayZipBehavior.AssertSameLength) { (ctx, pf) =>
        makestruct("__old_ctx" -> ctx, "__part_file" -> pf)
      }
    }(GetField(_, "__old_ctx"))
      .mapCollectWithContextsAndGlobals("matrix_gen_writer") { (rows, ctxRef) =>
        val ctx = GetField(ctxRef, "__part_file") + UUID4()
        WritePartition(rows, ctx, GenVariantWriter(tm, entriesFieldName, precision))
      } { (parts, globals) =>
        val cols = ToStream(GetField(globals, colsFieldName))
        val sampleFileName = Str(s"$path.sample")
        val writeSamples = WritePartition(cols, sampleFileName, new GenSampleWriter)
...
case class MatrixBGENWriter(
  ...
  override def lower(
    ...
    ts.mapContexts { oldCtx =>
      ...
      // hint: don't writeHeader
      val variantCounts =
        if (writeHeader) ToStream(ts.countPerPartition().deepCopy)
        else mapIR(rangeIR(ts.numPartitions))(_ => NA(TInt64))
      ...
      zipIR(FastSeq(oldCtx, variantCounts, partFiles), ArrayZipBehavior.AssertSameLength) {
        case Seq(ctx, vc, pf) =>
          makestruct("__old_ctx" -> ctx, "__num_variants" -> vc, "__part_file" -> pf)
      }
    }(GetField(_, "__old_ctx"))
      .mapCollectWithContextsAndGlobals("matrix_vcf_writer") { (rows, ctx) =>
        val writeCtx = makestruct(
          "cols" -> GetField(ts.globals, colsFieldName),
          "numVariants" -> GetField(ctx, "__num_variants"),
          "partFile" -> (GetField(ctx, "__part_file") + UUID4()),
        )
        WritePartition(rows, writeCtx, partWriter)
      } { (results, globals) =>
        val ctx = makestruct("cols" -> GetField(globals, colsFieldName), "results" -> results)
  ...
  override def lower(
    ...
    ts.mapContexts { oldCtx =>
      ...
      zip2(oldCtx, ToStream(files), ArrayZipBehavior.AssertSameLength) { (ctxElt, pf) =>
        makestruct("__old_ctx" -> ctxElt, "__files" -> pf)
      }
    }(GetField(_, "__old_ctx"))
      .mapCollectWithContextsAndGlobals("matrix_plink_writer") { (rows, ctxRef) =>
        bindIRs(UUID4(), GetField(ctxRef, "__files")) { case Seq(id, files) =>
          val bedFile = GetTupleElement(files, 0) + id
          val bimFile = GetTupleElement(files, 1) + id
          val ctx = makestruct("bedFile" -> bedFile, "bimFile" -> bimFile)
          WritePartition(rows, ctx, lineWriter)
        }
      } { (parts, globals) =>
        val commit = PLINKExportFinalizer(tm, path, tmpBedDir + "/header")
        val famWriter =
          TableTextPartitionWriter(tm.colsTableType.rowType, "\t", writeHeader = false)
        val cols = ToStream(GetField(globals, colsFieldName))
        val fpath = WritePartition(cols, Str(path + ".fam"), famWriter)
        Begin(FastSeq(
...
case class PLINKPartitionWriter(typ: MatrixType, entriesFieldName: String) extends PartitionWriter {
  ...
  private[this] val locusIdx = typ.rowType.fieldIdx("locus")
  private[this] val allelesIdx = typ.rowType.fieldIdx("alleles")
  private[this] val varidIdx = typ.rowType.fieldIdx("varid")
  private[this] val cmPosIdx = typ.rowType.fieldIdx("cm_position")
...
case class MatrixBlockMatrixWriter(
  ...
  override def lower(
    ...
    val rowCountIR = ts.mapCollect("matrix_block_matrix_writer_partition_counts")(StreamLen(_))
    ...
    val zippedWithStarts =
      ts.mapContexts { oldContextsStream =>
        zipIR(
          FastSeq(
            oldContextsStream,
          ...
          ArrayZipBehavior.AssertSameLength,
        ) { case Seq(oldCtx, partStart, partStop) =>
          makestruct(
            "__mw_old_ctx" -> oldCtx,
            "__mw_start_idx" -> partStart.toI,
            "__mw_stop_idx" -> partStop.toI,
          )
        }
      }(GetField(_, "__mw_old_ctx"))

    // Now label each row with its idx.
    val partsZippedWithIdx =
      zippedWithStarts.mapPartitionWithContext { (part, ctx) =>
        zip2(
          part,
          rangeIR(GetField(ctx, "__mw_start_idx"), GetField(ctx, "__mw_stop_idx")),
          ArrayZipBehavior.AssertSameLength,
        )((partRow, idx) => insertIR(partRow, "__per_row_idx" -> idx))
      }
    ...
    val rowIdxPartitioner =
      new RVDPartitioner(ctx.stateManager, TStruct("__per_row_idx" -> TInt32), inputRowIntervals)
    ...
    val desiredRowStarts = ArraySeq.tabulate(numBlockRows)(_ * blockSize)
    val desiredRowStops = desiredRowStarts.drop(1) :+ numRows.toInt
    val desiredRowIntervals =
      desiredRowStarts
        .view
        .zip(desiredRowStops)
    ...
    val blockSizeGroupsPartitioner =
      RVDPartitioner.generate(
        ctx.stateManager,
        TStruct("__per_row_idx" -> TInt32),
        desiredRowIntervals,
      )
    val rowsInBlockSizeGroups: TableStage =
      keyedByRowIdx.repartitionNoShuffle(ctx, blockSizeGroupsPartitioner)

    def createBlockMakingContexts(tablePartsStreamIR: Atom): IR =
      flatten(zip2(tablePartsStreamIR, rangeIR(numBlockRows), ArrayZipBehavior.AssertSameLength) {
        (tableSinglePartCtx, blockRowIdx) =>
          mapIR(rangeIR(numBlockCols)) { blockColIdx =>
            makestruct(
              "__old_table_ctx" -> tableSinglePartCtx,
              "__block_start" -> (blockColIdx * I32(blockSize)),
              "__block_size" -> If(
                blockColIdx ceq I32(numBlockCols - 1),
                lastBlockNumCols,
                blockSize,
              ),
              "__block_col_idx" -> blockColIdx,
              "__block_row_idx" -> blockRowIdx,
            )
          }
      })

    val tableOfNDArrays =
      rowsInBlockSizeGroups
        .mapContexts(createBlockMakingContexts)(GetField(_, "__old_table_ctx"))
        .mapPartitionWithContext { (part, ctx) =>
          M.eval {
            for {
              blockStart <- GetField(ctx, "__block_start")
              blockSize <- GetField(ctx, "__block_size")
              blockEnd <- blockStart + blockSize

              data <- streamAggIR(part) { row =>
                makestruct(
                  "__num_rows" ->
                    ApplyAggOp(Count())(),
                  "__result_idx" ->
                    ArrayRef(
                      ApplyAggOp(
                        FastSeq(I32(1)),
                        FastSeq(GetField(row, "__per_row_idx")),
                        Take(),
                      ),
                      I32(0),
                    ),
                  "__block_data" -> {
                    val slices = sliceArrayIR(GetField(row, entriesFieldName), blockStart, blockEnd)
                    val elem = Ref(Name("__elem"), TIterable.elementType(slices.typ))
                    val collect = ApplyAggOp(Collect())(GetField(elem, entryField))
                    AggExplode(ToStream(slices), elem.name, collect, isScan = false)
                  },
                )
              }

              numRowsOfBlock <- GetField(data, "__num_rows")
              idxOfResult <- GetField(data, "__result_idx")
              ...
              makestruct(
                ...
                "__block_col_idx" -> GetField(ctx, "__block_col_idx"),
                "__block" -> ndarray,
              )
            )
          }
    ...
    val pathsWithColMajorIndices =
      tableOfNDArrays.mapCollect("matrix_block_matrix_writer") { partition =>
        ToArray(mapIR(partition) { singleNDArrayTuple =>
          M.eval {
            for {
              rowIdx <- GetField(singleNDArrayTuple, "__block_row_idx")
              colIdx <- GetField(singleNDArrayTuple, "__block_col_idx")
              colMajorIndex <- rowIdx + (colIdx * numBlockRows)
              blockPath <- strConcat(s"$path/parts/part-", colMajorIndex, "-", UUID4())
              ndarray <- GetField(singleNDArrayTuple, "__block")
            } yield maketuple(colMajorIndex, WriteValue(ndarray, blockPath, writer))
          }
        })
      }

    val sortedColMajorPairs =
      sortIR(flatten(pathsWithColMajorIndices)) { (l, r) =>
        GetTupleElement(l, 0) < GetTupleElement(r, 0)
      }

    val flatPaths = mapArray(sortedColMajorPairs)(GetTupleElement(_, 1))
...
case class MatrixNativeMultiWriter(
  ...
  def lower(
    ...
    M.eval {
      for {
        _ <- M.defer { b =>
          components.foreach(c => b.memoize(c.setup))
          components.foreach(_.stage.letBindings.foreach { case (name, value) =>
            b.strictMemoize(value, name)
          })
          Void()
        }
        ...
        _ <- M.defer { b =>
          components.zipWithIndex.foreach { case (c, i) =>
            val part = b.memoize(sliceArrayIR(result, partCounts(i), partCounts(i + 1)))
            b.memoize(c.finalizeWrite(part, c.stage.globals))
          }
          Void()
        }
      } yield Void()
