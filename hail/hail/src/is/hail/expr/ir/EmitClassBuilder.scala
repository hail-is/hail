...
class EmitModuleBuilder(val ctx: ExecuteContext, val modb: ModuleBuilder) {
  ...
  def newEmitClass[C](name: String, sourceFile: Option[String] = None)(implicit cti: TypeInfo[C])
    : EmitClassBuilder[C] =
  ...
  def literalsResult()
    : (Array[(VirtualTypeWithReq, Any, PType, Int)], Array[(EncodedLiteral, PType, Int)]) =
    (
...
trait WrappedEmitClassBuilder[C] extends WrappedEmitModuleBuilder {
  ...
  def newPSettable(sb: SettableBuilder, pt: SType, name: String = null): SSettable =
    ecb.newPSettable(sb, pt, name)
  ...
  def newEmitField(name: String, et: EmitType): EmitSettable =
    ecb.newEmitField(name, et.st, et.required)

  def newEmitField(name: String, pt: SType, required: Boolean): EmitSettable =
    ecb.newEmitField(name, pt, required)
  ...
  def addLiteral(cb: EmitCodeBuilder, v: Any, t: VirtualTypeWithReq): SValue =
    ecb.addLiteral(cb, v, t)

  def addEncodedLiteral(cb: EmitCodeBuilder, encodedLiteral: EncodedLiteral) =
    ecb.addEncodedLiteral(cb, encodedLiteral)

  def getPType[T <: PType: TypeInfo](t: T): Code[T] = ecb.getPType(t)

  def getType[T <: Type: TypeInfo](t: T): Code[T] = ecb.getType(t)

  def newEmitMethod(name: String, argsInfo: IndexedSeq[ParamType], returnInfo: ParamType)
    : EmitMethodBuilder[C] =
    ecb.newEmitMethod(name, argsInfo, returnInfo)

  def newEmitMethod(
    name: String,
    argsInfo: IndexedSeq[MaybeGenericTypeInfo[_]],
    returnInfo: MaybeGenericTypeInfo[_],
  ): EmitMethodBuilder[C] =
    ecb.newEmitMethod(name, argsInfo, returnInfo)

  def newStaticEmitMethod(name: String, argsInfo: IndexedSeq[ParamType], returnInfo: ParamType)
    : EmitMethodBuilder[C] =
    ecb.newStaticEmitMethod(name, argsInfo, returnInfo)

  def genEmitMethod(baseName: String, argsInfo: IndexedSeq[ParamType], returnInfo: ParamType)
    : EmitMethodBuilder[C] =
    ecb.genEmitMethod(baseName, argsInfo, returnInfo)

  def genStaticEmitMethod(baseName: String, argsInfo: IndexedSeq[ParamType], returnInfo: ParamType)
    : EmitMethodBuilder[C] =
  ...
  def genEmitMethod[A1: TypeInfo, A2: TypeInfo, R: TypeInfo](baseName: String)
    : EmitMethodBuilder[C] =
    ecb.genEmitMethod[A1, A2, R](baseName)

  def genEmitMethod[A1: TypeInfo, A2: TypeInfo, A3: TypeInfo, R: TypeInfo](baseName: String)
    : EmitMethodBuilder[C] =
...
final class EmitClassBuilder[C](val emodb: EmitModuleBuilder, val cb: ClassBuilder[C])
  ...
  def newPSettable(sb: SettableBuilder, st: SType, name: String = null): SSettable =
  ...
  def setLiteralsArray(cb: EmitCodeBuilder, arr: Code[Array[Long]]): Unit =
    cb.assign(decodedLiteralsField, arr)

  lazy val partitionRegion: Settable[Region] = genFieldThisRef[Region]("partitionRegion")

  private[this] lazy val _taskContext: Settable[HailTaskContext] =
    genFieldThisRef[HailTaskContext]("taskContext")
  ...
  private[this] def encodeLiterals(): Array[AnyRef] = {
    ...
    val mb2 = newEmitMethod(
      ...
      FastSeq[ParamType](typeInfo[Array[AnyRef]]),
      typeInfo[Unit],
    )
    ...
    mb2.voidWithBuilder { cb =>
      ...
      cb.assign(
        decodedLiteralsField,
        Code.newArray[Long](literals.length + preEncodedLiterals.length),
      )

      val ib = cb.newLocal[InputBuffer](
        "ib",
        spec.buildCodeInputBuffer(Code.newInstance[ByteArrayInputStream, Array[Byte]](
          Code.checkcast[Array[Byte]](allEncodedFields(0))
        )),
      )
      ...
      literals.zipWithIndex.foreach { case ((_, _, pt, arrIdx), i) =>
        lits.loadField(cb, i)
          .consume(
            ...
            pc => cb += (decodedLiteralsField(arrIdx) = pt.store(cb, partitionRegion, pc, false)),
          )
      }
      ...
      preEncodedLiterals.zipWithIndex.foreach { case ((encLit, pt, arrIdx), index) =>
        ...
        cb.assign(
          ib,
          spec.buildCodeInputBuffer(Code.newInstance[
            ArrayOfByteArrayInputStream,
            Array[Array[Byte]],
          ](Code.checkcast[Array[Array[Byte]]](allEncodedFields(index + 1)))),
        )
  ...
  def addAggStates(aggSigs: IndexedSeq[agg.AggStateSig]): agg.TupleAggregatorState = {
    ...
    val setF = newEmitMethod(
      "setAggState",
      FastSeq[ParamType](typeInfo[Region], typeInfo[Long]),
      typeInfo[Unit],
    )
    ...
    val setNSer =
      newEmitMethod("setNumSerialized", FastSeq[ParamType](typeInfo[Int]), typeInfo[Unit])
    val setSer = newEmitMethod(
      "setSerializedAgg",
      FastSeq[ParamType](typeInfo[Int], typeInfo[Array[Byte]]),
      typeInfo[Unit],
    )
    val getSer =
    ...
    val (nfcode, states) = EmitCodeBuilder.scoped(newF) { cb =>
      ...
      cb += (_aggOff := _aggRegion.load().allocate(
        states.storageType.alignment,
        states.storageType.byteSize,
      ))
    ...
    storeF.voidWithBuilder(cb => _aggState.store(cb))

    setNSer.emit(_aggSerialized := Code.newArray[Array[Byte]](setNSer.getCodeParam[Int](1)))

    setSer.emit(_aggSerialized.load().update(
      setSer.getCodeParam[Int](1),
      setSer.getCodeParam[Array[Byte]](2),
    ))
  ...
  def backend(): Code[BackendUtils] = {
    if (_backendField == null) {
      ...
      val mb =
        newEmitMethod("setBackend", FastSeq[ParamType](typeInfo[BackendUtils]), typeInfo[Unit])
  ...
  def getOrdering(t1: SType, t2: SType, sortOrder: SortOrder = Ascending): CodeOrdering = {
    val baseOrd =
      memoizedComparisons.getOrElseUpdate((t1, t2), CodeOrdering.makeOrdering(t1, t2, this))
    sortOrder match {
  ...
  def getOrderingFunction(
    ...
    { (cb: EmitCodeBuilder, v1: EmitValue, v2: EmitValue) =>
      ...
      cb.memoize[op.ReturnType](coerce[op.ReturnType](r))(
        op.rtti,
        implicitly[op.ReturnType =!= Unit],
      )
    }
  }
  ...
  def getOrderingFunction(t1: SType, t2: SType, op: CodeOrdering.Op)
    : CodeOrdering.F[op.ReturnType] =
  ...
  def newEmitMethod(
    ...
    new EmitMethodBuilder[C](
      argsInfo.map(ai => CodeParamType(ai.base)),
      CodeParamType(returnInfo.base),
      this,
  ...
  def makeAddPartitionRegion(): Unit = {
    ...
    val mb =
      newEmitMethod("addPartitionRegion", FastSeq[ParamType](typeInfo[Region]), typeInfo[Unit])
  ...
  def makeAddHailClassLoader(): Unit = {
    ...
    val mb = newEmitMethod(
      "addHailClassLoader",
      FastSeq[ParamType](typeInfo[HailClassLoader]),
      typeInfo[Unit],
    )
    mb.voidWithBuilder(cb => emodb.setHailClassLoader(cb, mb.getCodeParam[HailClassLoader](1)))
  }

  def makeAddFS(): Unit = {
    cb.addInterface(typeInfo[FunctionWithFS].iname)
    val mb = newEmitMethod("addFS", FastSeq[ParamType](typeInfo[FS]), typeInfo[Unit])
    mb.voidWithBuilder(cb => emodb.setFS(cb, mb.getCodeParam[FS](1)))
  }

  def makeAddTaskContext(): Unit = {
    cb.addInterface(typeInfo[FunctionWithTaskContext].iname)
    val mb =
      newEmitMethod("addTaskContext", FastSeq[ParamType](typeInfo[HailTaskContext]), typeInfo[Unit])
  ...
  def makeAddReferenceGenomes(): Unit = {
    ...
    val mb = newEmitMethod(
      "addReferenceGenomes",
      FastSeq[ParamType](typeInfo[Array[ReferenceGenome]]),
      typeInfo[Unit],
    )
    mb.voidWithBuilder { cb =>
      ...
      cb.if_(
        rgs.length().cne(const(rgFields.length)),
        cb._fatal(
          "Invalid number of references, expected ",
          rgFields.length.toString,
          " got ",
          rgs.length().toS,
        ),
      )
      ...
      Option(emodb._rgMapField).foreach { fld =>
        cb.assign(
          fld,
          Code.invokeStatic1[ReferenceGenome, Array[ReferenceGenome], Map[String, ReferenceGenome]](
            "getMapFromArray",
            rgs,
          ),
        )
  ...
  def resultWithIndex(print: Option[PrintWriter] = None)
    ...
    ctx.time {
      ...
      new ((HailClassLoader, FS, HailTaskContext, Region) => C) with java.io.Serializable {
        ...
        override def apply(hcl: HailClassLoader, fs: FS, htc: HailTaskContext, region: Region): C = {
          if (theClass == null) {
            this.synchronized {
              if (theClass == null) {
  ...
  def defineEmitMethod(
    name: String,
    paramTys: IndexedSeq[ParamType],
    retTy: ParamType,
  )(
    body: EmitMethodBuilder[C] => Unit
  ): EmitMethodBuilder[C] = {
  ...
  def genEmitMethod[A1: TypeInfo, A2: TypeInfo, A3: TypeInfo, R: TypeInfo](baseName: String)
    ...
    genEmitMethod(
      baseName,
      FastSeq[ParamType](typeInfo[A1], typeInfo[A2], typeInfo[A3]),
      typeInfo[R],
    )
  ...
  def genEmitMethod[A1: TypeInfo, A2: TypeInfo, A3: TypeInfo, A4: TypeInfo, R: TypeInfo](
    ...
    genEmitMethod(
      baseName,
      FastSeq[ParamType](typeInfo[A1], typeInfo[A2], typeInfo[A3], typeInfo[A4]),
      typeInfo[R],
    )
  ...
  def genEmitMethod[
    ...
    genEmitMethod(
      baseName,
      FastSeq[ParamType](typeInfo[A1], typeInfo[A2], typeInfo[A3], typeInfo[A4], typeInfo[A5]),
      typeInfo[R],
    )
...
object EmitFunctionBuilder {
  ...
  def apply[R: TypeInfo](ctx: ExecuteContext, baseName: String)
    : EmitFunctionBuilder[AsmFunction0[R]] =
    EmitFunctionBuilder[AsmFunction0[R]](
      ctx,
      baseName,
      FastSeq[MaybeGenericTypeInfo[_]](),
      GenericTypeInfo[R],
    )

  def apply[A: TypeInfo, R: TypeInfo](ctx: ExecuteContext, baseName: String)
  ...
  def apply[A: TypeInfo, B: TypeInfo, R: TypeInfo](ctx: ExecuteContext, baseName: String)
  ...
  def apply[A: TypeInfo, B: TypeInfo, C: TypeInfo, R: TypeInfo](
    ctx: ExecuteContext,
    baseName: String,
  ...
  def apply[A: TypeInfo, B: TypeInfo, C: TypeInfo, D: TypeInfo, R: TypeInfo](
    ctx: ExecuteContext,
    baseName: String,
  ...
  def apply[A: TypeInfo, B: TypeInfo, C: TypeInfo, D: TypeInfo, E: TypeInfo, R: TypeInfo](
    ctx: ExecuteContext,
    baseName: String,
...
trait FunctionWithAggRegion {
  ...
  /* stores agg regions into the top agg region, so that all agg resources are referenced solely by
   * that region */
...
class EmitMethodBuilder[C](
  ...
  def voidWithBuilder(f: (EmitCodeBuilder) => Unit): Unit =
...
trait WrappedEmitMethodBuilder[C] extends WrappedEmitClassBuilder[C] {
  ...
  def newEmitLocal(name: String, pt: SType, required: Boolean): EmitSettable =
    emb.newEmitLocal(name, pt, required)
}
