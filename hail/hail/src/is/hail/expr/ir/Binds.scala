...
// A for expressions and a family of relational values (`Algebra.Table` etc.)
// for the other IR kinds, reached only through `denote`.
trait Algebra[A] {
  // the relational members of this domain's family of values
  type TableValue <: Algebra.Table[A]
  type MatrixValue <: Algebra.Matrix[A]
  type BlockMatrixValue <: Algebra.BlockMatrix[A]
  ...
  def elementOf(a: A): A
  def selectFields(a: A, fields: IndexedSeq[String]): A
  def firstField(a: A): A
  def array(element: A): A
  def stream(element: A): A
  def tuple(as: IndexedSeq[A]): A
  ...
  def strengthened(a: A): A
}

object Algebra {
  trait Table[+T] extends Any {
    def global: T
    def row: T
  }

  trait Matrix[+T] extends Any {
    def global: T
    def col: T
    def row: T
    def entry: T
  ...
  trait BlockMatrix[+T] extends Any {
    def element: T
  }
...
private object TypeAlgebra extends Algebra[Type] {
  class TableValue(val typ: TableType) extends AnyVal with Algebra.Table[Type] {
    override def global: Type = typ.globalType
    override def row: Type = typ.rowType
  }

  class MatrixValue(val typ: MatrixType) extends AnyVal with Algebra.Matrix[Type] {
    override def global: Type = typ.globalType
    override def col: Type = typ.colType
    override def row: Type = typ.rowType
    override def entry: Type = typ.entryType
  }

  class BlockMatrixValue(val typ: BlockMatrixType) extends AnyVal with Algebra.BlockMatrix[Type] {
    override def element: Type = typ.elementType
  }

  override def denote(ir: IR): Type = ir.typ
  override def denote(table: TableIR): TableValue = new TableValue(table.typ)
  override def denote(matrix: MatrixIR): MatrixValue = new MatrixValue(matrix.typ)
  override def denote(bm: BlockMatrixIR): BlockMatrixValue = new BlockMatrixValue(bm.typ)
...
private object StructuralDomain extends Algebra[Unit] {
  ...
  override def elementOf(a: Unit): Unit = ()
  override def selectFields(a: Unit, fields: IndexedSeq[String]): Unit = ()
  override def firstField(a: Unit): Unit = ()
  override def array(element: Unit): Unit = ()
  override def stream(element: Unit): Unit = ()
  override def tuple(as: IndexedSeq[Unit]): Unit = ()
  override def lift(t: Type): Unit = ()
  override def meet(as: IndexedSeq[Unit]): Unit = ()
  override def weakened(a: Unit): Unit = ()
  override def strengthened(a: Unit): Unit = ()
}
...
object Bindings {
  ...
  /** Returns the environment of the `i`th child of `ir` in an arbitrary domain, given the
  ...
  def get[T: ClassTag: Algebra](ir: BaseIR, i: Int): Bindings[T] =
    ir match {
      case ir: BlockMatrixIR => BlockMatrixBindings(ir, i)
      case ir: MatrixIR => MatrixBindings(ir, i)
      case ir: TableIR => TableBindings(ir, i)
      case ir: IR => ValueBindings(ir, i)
    }
  ...
  private object BlockMatrixBindings {
    def apply[T](ir: BlockMatrixIR, i: Int)(implicit A: Algebra[T]): Bindings[T] =
      ir match {
  ...
  private object MatrixBindings {
    ...
    def apply[T](ir: MatrixIR, i: Int)(implicit A: Algebra[T]): Bindings[T] =
      ir match {
  ...
  private object TableBindings {
    ...
    def apply[T](ir: TableIR, i: Int)(implicit A: Algebra[T]): Bindings[T] =
      ir match {
  ...
  private object ValueBindings {
    def apply[T: ClassTag](ir: IR, i: Int)(implicit A: Algebra[T]): Bindings[T] =
      ir match {
        ...
        case TableAggregate(child, _) =>
          if (i == 1)
            Bindings.inFreshScope(
              TableBindings.rowBindings(A.denote(child)),
              eval = TableBindings.globalBindings,
              agg = Some(TableBindings.rowBindings),
            )
          else Bindings(agg = AggEnv.Drop, scan = AggEnv.Drop, dropEval = true)
        case MatrixAggregate(child, _) =>
          if (i == 1)
            Bindings.inFreshScope(
              MatrixBindings.entryBindings(A.denote(child)),
              eval = MatrixBindings.globalBindings,
              agg = Some(MatrixBindings.entryBindings),
            )
          else Bindings(agg = AggEnv.Drop, scan = AggEnv.Drop, dropEval = true)
