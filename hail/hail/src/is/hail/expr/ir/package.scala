...
package object ir extends CompileOps {
  ...
  implicit def booleanToIR(b: Boolean): IR with TrivialIR = if (b) True() else False()
  ...
  def bindIRs(values: IR*)(body: IndexedSeq[TrivialIR] => IR): IR = {
    val bindings = values.toFastSeq.map(freshName() -> _)
    Let(bindings, body(bindings.map(b => Ref(b._1, b._2.typ))))
  }

  def bindIR(v: IR)(body: TrivialIR => IR): IR =
    bindIRs(v) { case Seq(ref) => body(ref) }

  def relationalBindIR(v: IR)(body: TrivialIR => IR): IR = {
  ...
  def streamAggIR(stream: IR)(f: TrivialIR => IR): StreamAgg = {
    val ref = Ref(freshName(), tcoerce[TStream](stream.typ).elementType)
    StreamAgg(stream, ref.name, f(ref))
  }

  def streamAggScanIR(stream: IR)(f: TrivialIR => IR): StreamAggScan = {
    val ref = Ref(freshName(), tcoerce[TStream](stream.typ).elementType)
    StreamAggScan(stream, ref.name, f(ref))
  }

  def forIR(stream: IR)(f: TrivialIR => IR): IR = {
    val ref = Ref(freshName(), tcoerce[TStream](stream.typ).elementType)
    StreamFor(stream, ref.name, f(ref))
  }

  def filterIR(stream: IR)(f: TrivialIR => IR): IR = {
    val ref = Ref(freshName(), tcoerce[TStream](stream.typ).elementType)
    StreamFilter(stream, ref.name, f(ref))
  }

  def mapIR(stream: IR)(f: TrivialIR => IR): IR = {
    val ref = Ref(freshName(), tcoerce[TStream](stream.typ).elementType)
    StreamMap(stream, ref.name, f(ref))
  }

  def mapArray(array: IR)(f: TrivialIR => IR): IR =
    ToArray(mapIR(ToStream(array))(f))

  def flatMapIR(stream: IR)(f: TrivialIR => IR): IR = {
    val ref = Ref(freshName(), tcoerce[TStream](stream.typ).elementType)
  ...
  def foldIR(stream: IR, zero: IR)(f: (TrivialIR, TrivialIR) => IR): StreamFold = {
  ...
  def fold2IR(
    ...
    seqs: ((TrivialIR, IndexedSeq[TrivialIR]) => IR)*
  )(
    result: IndexedSeq[TrivialIR] => IR
  ): IR = {
    val elt = Ref(freshName(), tcoerce[TStream](stream.typ).elementType)
  ...
  def streamScanIR(stream: IR, zero: IR)(f: (TrivialIR, TrivialIR) => IR): IR = {
    val elt = Ref(freshName(), tcoerce[TStream](stream.typ).elementType)
    val accum = Ref(freshName(), zero.typ)
    StreamScan(stream, zero, accum.name, elt.name, f(accum, elt))
  }

  def sortIR(stream: IR)(f: (TrivialIR, TrivialIR) => IR): IR = {
  ...
  def zipJoin2IR(streams: IndexedSeq[IR], key: IndexedSeq[String])(f: (TrivialIR, TrivialIR) => IR)
    : IR = {
  ...
  def streamSumIR(stream: IR): IR =
    foldIR(stream, 0) { case (accum, elt) => accum.clone + elt }
  ...
  def bmMap(bm: BlockMatrixIR, needsDense: Boolean)(f: TrivialIR => IR): BlockMatrixMap = {
  ...
  def aggBindIR(v: IR, isScan: Boolean = false)(body: TrivialIR => IR): IR = {
    val ref = Ref(freshName(), v.typ)
    AggLet(ref.name, v, body(ref), isScan = isScan)
  }

  def aggExplodeIR(v: IR, isScan: Boolean = false)(body: TrivialIR => IR): AggExplode = {
  ...
  def tailLoop(
    ...
    f: (IndexedSeq[IR] => IR, IndexedSeq[TrivialIR]) => IR
  ): IR = {
  ...
  def mapPartitions(child: TableIR)(f: (TrivialIR, TrivialIR) => IR): TableMapPartitions = {
