...
case object LegacyInterpretNonCompilablePass extends LoweringPass {
  ...
  override def transform(ctx: ExecuteContext, ir: BaseIR): BaseIR = LowerOrInterpretNonCompilable(ctx, ir)
}
...
case class LowerToDistributedArrayPass(t: DArrayLowering.Type) extends LoweringPass {
  ...
  override def transform(ctx: ExecuteContext, ir: BaseIR): BaseIR = LowerToCDA(ir.asInstanceOf[IR], t, ctx)
}
