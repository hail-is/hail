  ...
  def execute(ctx: ExecuteContext, r: LoweringAnalyses, ir: TableIR): TableExecuteIntermediate = {
    ...
    ir match {
      ...
      case TableAggregateByKey(child, expr) =>
        ...
        val extracted = agg.Extract(ctx, expr, r.requirednessAnalysis)
      ...
      case TableMapRows(child, newRow) =>
        val extracted = agg.Extract(ctx, newRow, r.requirednessAnalysis, isScan = true)
      ...
      case TableHead(child, n) =>
        val prev = recur(child).asTableValue(ctx)
        TableValueIntermediate(prev.copy(rvd = prev.rvd.head(n, RowCounts.partitionCounts(child))))
      case TableTail(child, n) =>
        val prev = recur(child).asTableValue(ctx)
        TableValueIntermediate(prev.copy(rvd = prev.rvd.tail(n, RowCounts.partitionCounts(child))))
