        ...
        ToArray(mapIR(StreamGroupByKey(
          ...
          M.eval {
            for {
                ...
                streamAggIR(oneGroup) { elem =>
                  M.agg {
                    for {
                      ...
                      "eachPartSorted" -> AggFold.all(GetField(data, "isSorted")),
              ...
              branchingFactor <-
                If(
                  (branchingFactor > 1) && (numSamples ceq 0),
                  Die(
  ...
  private def orderedGroupBy[T: ClassTag, U](is: IndexedSeq[T], func: T => U)
    ...
    is.foreach { element =>
      ...
      if (currentGroup.isEmpty) {
        ...
        result += lastKeySeen.get -> currentGroup.toFastSeq
        currentGroup.clear()
        currentGroup.append(element)
        lastKeySeen = Some(key)
      }
