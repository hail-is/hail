...
class ReservoirSampleRVAS(val eltType: VirtualTypeWithReq, val kb: EmitClassBuilder[_])
  ...
  val storageType: PCanonicalTuple =
    PCanonicalTuple(true, PInt32Required, PInt64Required, PInt64Required, builder.stateType)
  ...
  def seqOp(cb: EmitCodeBuilder, elt: EmitCode): Unit = {
    ...
    cb.if_(
      ...
      eltVal.toI(cb)
        ...
        cb.if_(
          rand.invoke[Double]("nextDouble") * seenSoFar.toD <= maxSize.toD, {
  ...
  def combine(cb: EmitCodeBuilder, other: ReservoirSampleRVAS): Unit = {
    ...
    cb.if_(
      other.builder.size < maxSize, {

        cb.assign(j, 0)
        cb.while_(
          j < other.builder.size, {
          ...
          builder.size < maxSize, {
            ...
            cb.while_(
              j < maxSize, {
                ...
                cb.if_(
                  x * (totalWeightLeft + totalWeightRight) <= totalWeightLeft, {
                    ...
                    cb.if_(
                      idxToSample < leftSize,
                      builder.overwrite(
                        cb,
                        cb.memoize(builder.loadElement(cb, leftSize)),
                        idxToSample,
                        false,
                      ),
                    )
                    ...
                    other.builder.loadElement(cb, idxToSample).toI(cb).consume(
                      cb,
                      newBuilder.setMissing(cb),
                    ...
                    cb.if_(
                      idxToSample < rightSize,
                      other.builder.overwrite(
                        cb,
                        cb.memoize(other.builder.loadElement(cb, rightSize)),
                        idxToSample,
                        false,
                      ),
                    )
...
class ReservoirSampleAggregator(typ: VirtualTypeWithReq) extends StagedAggregator {
  ...
  override protected def _initOp(cb: EmitCodeBuilder, state: ReservoirSampleRVAS, init: Array[EmitCode])
    : Unit = {
  ...
  override protected def _seqOp(cb: EmitCodeBuilder, state: ReservoirSampleRVAS, seq: Array[EmitCode])
    : Unit = {
