...
object Extract {
  ...
  def apply(ctx: ExecuteContext, ir: IR, r: RequirednessAnalysis, isScan: Boolean = false)
    ...
    val result = bindIR(sigs.resultsOp)(postAggIR)
  ...
  private def extract(
    ...
    // Bindings in scope for init op arguments. Will also be in scope in seqOps and post-agg IR.
    ...
    ir match {
      case Block(bindings, body) =>
        var newEnv = env
        val bindingsTemp = Array.newBuilder[(Name, IR => IR)]
        bindingsTemp.sizeHint(bindings)
        for (binding <- bindings) binding match {
          case Binding(name, value, Scope.EVAL) =>
            val newValue =
              this.extract(value, newEnv, initBindings, initBuilder, seqBuilder, memo, r, isScan)
        ...
        newBindings.foreach { case b @ (name, f) =>
          if (newEnv.eval(name).usedInInitOp) initBindings += ((name, f(null)))
          else bindingsTemp += b
        }

        aggRes =>
          Block(
            bindingsTemp.result().map { case (name, value) => Binding(name, value(aggRes)) },
            newBody(aggRes),
          )
      ...
      case AggGroupBy(key, aggIR, _) =>
        ...
        val transformed = transformedF(GetField(newRef, "value"))

        aggRes =>
          ToDict(StreamMap(
            ToStream(GetTupleElement(aggRes, i)),
            newRef.name,
      ...
      case x @ AggArrayPerElement(a, elementName, indexName, aggBody, knownLength, _) =>
        ...
        val transformedF = this.extract(aggBody, env, initBindings, newAggs, newSeq,
          newMemo, r, isScan)
        ...
        val transformed = transformedF(newRef)

        aggRes =>
          Let(
            FastSeq(rUID.name -> GetTupleElement(aggRes, i)),
            ToArray(StreamMap(
      ...
      case x: StreamAggScan =>
        ...
        _ => x
      case x =>
        val fs = x.children.zipWithIndex.map {
          case (child: IR, i) =>
            ...
            this.extract(child, newEnv, initBindings, initBuilder, seqBuilder, memo,
              r, isScan)
          case (child, _) => (_: IR) => child
        }.toFastSeq
        aggRes =>
          x.mapChildrenWithIndex { case (_: IR, i) =>
            fs(i)(aggRes)
          }
    }
  }
