...
//        env.eval.lookupOption(name).foreach(_.union(requestedType))
      ...
      case a @ (_: ApplyAggOp | _: ApplyScanOp) =>
        val (initOpArgs, seqOpArgs, op) = a match {
          case ApplyAggOp(initOpArgs, seqOpArgs, op) => (initOpArgs.map(_.typ), seqOpArgs.map(_.typ), op)
          case ApplyScanOp(initOpArgs, seqOpArgs, op) => (initOpArgs.map(_.typ), seqOpArgs.map(_.typ), op)
        }
