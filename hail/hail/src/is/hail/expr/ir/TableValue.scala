...
import is.hail.sparkextras.ContextRDD.Element
import is.hail.sparkextras.implicits._
import is.hail.types.physical.{PArray, PCanonicalArray, PCanonicalStruct, PInt32Required, PStruct}
...
object TableValue extends Logging {
  ...
  def parallelize(ctx: ExecuteContext, rowsAndGlobal: IR, nPartitions: Option[Int]): TableValue = {
    ...
    val rvd = ContextRDD.parallelize(encRows, encRows.length)
      .cmapPartitions { (hcl, ctx, it) =>
        it.flatMap { case (nRowPartition, arr) =>
          val bais = new ByteArrayDecoder(hcl, makeDec)
          bais.set(arr)
    ...
    TableValue(
      ...
      new RVD(
        ...
        ContextRDD.parallelize(Range(0, nPartitions), nPartitions)
          .cmapPartitionsWithIndex { case (i, _, ctx, _) =>
...
case class TableValue(ctx: ExecuteContext, typ: TableType, globals: BroadcastRow, rvd: RVD)
  ...
  def filter(pred: IR): TableValue = {
    ...
    copy(rvd =
      rvd.filterWithContext[(AsmFunction3RegionLongLongBoolean, Long)](
        ...
        { case ((p, glob), _, ctx, ptr) => p(ctx.region, ptr, glob) },
      )
    )
  }
  ...
  def mapPartitions(globalName: Name, partitionStreamName: Name, body: IR, allowedOverlap: Int)
    ...
    val fsBc = ctx.fsBc
    val itF = {
      (idx: Int, hcl: HailClassLoader, consumerCtx: RVDContext, partition: Element[Long]) =>
        val boxedPartition = new NoBoxLongIterator {
          ...
          override def init(partitionRegion: Region, elementRegion: Region): Unit =
            iter = partition(hcl, new RVDContext(partitionRegion, elementRegion))
  ...
  def mapRows(extracted: IndependentExtractedAggs): TableValue = {
    ...
    if (ctx.getFlag("distributed_scan_comb_op") != null && extracted.sigs.shouldTreeAggregate) {
      ...
      val files = ArraySeq.unsafeWrapArray(rvd.mapPartitionsWithIndex { (i, hcl, ctx, it) =>
