...
object IRParser {
    ...
    identifier(it) match {
      ...
      case "ArraySlice" =>
        ...
        ir_value_children(ctx)(it).map {
          ...
          case Array(a, start, stop, step) => ArraySlice.untyped(a, start, Some(stop), step, errorID)
        }
      ...
      case "ArrayZeros" => ir_value_expr(ctx)(it).map(ArrayZeros.apply)
      case "ArraySort" =>
        val l = name(it)
        val r = name(it)
        for {
      ...
      case "StreamZipJoinProducers" =>
        ...
        } yield StreamZipJoinProducers.untyped(ctxs, ctxName, makeProducer, key, curKey, curVals, body)
      case "StreamZipJoin" =>
      ...
      case "StreamLeftIntervalJoin" =>
        ...
        for {
          ...
          right <- ir_value_expr(ctx)(it)
          body <- ir_value_expr(ctx)(it)
        } yield StreamLeftIntervalJoin.untyped(left, right, lKeyFieldName, rIntervalName, lname, rname,
          body)
      ...
      case "ApplyAggOp" =>
        ...
        } yield ApplyAggOp.untyped(initOpArgs, seqOpArgs, aggSig)
      case "ApplyScanOp" =>
        val aggOp = agg_op(it)
        for {
          initOpArgs <- ir_value_exprs(ctx)(it)
          seqOpArgs <- ir_value_exprs(ctx)(it)
          aggSig = AggSignature(aggOp, null, null)
        } yield ApplyScanOp.untyped(initOpArgs, seqOpArgs, aggSig)
      case "AggFold" =>
        val accumName = name(it)
        val otherAccumName = name(it)
        val isScan = boolean_literal(it)
      ...
      case "TableWrite" =>
        ...
        table_ir(ctx)(it).map(child => TableWrite.untyped(child, deserialize[TableWriter](writerStr)))
      ...
      case "CollectDistributedArray" =>
        ...
        } yield CollectDistributedArray.untyped(ctxs, globals, cname, gname, body, dynamicID, staticID)
      case "JavaIR" =>
      ...
      case "WriteValue" =>
        ...
        ir_value_children(ctx)(it).map {
          ...
          case Array(value, path, stagingFile) => WriteValue.untyped(value, path, writer, Some(stagingFile))
        }
      ...
      case "ReadPartition" =>
        ...
        ir_value_expr(ctx)(it).map(context => ReadPartition.untyped(context, rowType, reader))
    }
  }
  ...
  def annotateTypes(ctx: ExecuteContext, ir: BaseIR, env: BindingEnv[Type]): BaseIR = {
    def run(ir: BaseIR, env: BindingEnv[Type]): BaseIR = {
      ...
      rw match {
        case Ref(name, _) => Ref(name, env.eval(name))
        case x @ Recur(name, _, _) =>
          val TTuple(IndexedSeq(_, TupleField(_, rt))) = env.eval.lookup(name)
          x.copy(typ = rt)
        case x @ ApplyAggOp(initOpArgs, seqOpArgs, aggSig) =>
          val initOpArgTypes = initOpArgs.map(_.typ)
          val seqOpArgTypes = seqOpArgs.map(_.typ)
          x.copy(aggSig = aggSig.copy(initOpArgs = initOpArgTypes, seqOpArgs = seqOpArgTypes))
        case x@ ApplyScanOp(initOpArgs, seqOpArgs, aggSig) =>
          val initOpArgTypes = initOpArgs.map(_.typ)
          val seqOpArgTypes = seqOpArgs.map(_.typ)
          x.copy(aggSig = aggSig.copy(initOpArgs = initOpArgTypes, seqOpArgs = seqOpArgTypes))
        case x@ ApplyComparisonOp(op, l, r) =>
          x.copy(op=op.copy(l.typ, r.typ))
