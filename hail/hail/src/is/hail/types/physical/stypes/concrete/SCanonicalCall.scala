...
case object SCanonicalCall extends SCall {
  override def _coerceOrCopy(cb: EmitCodeBuilder, region: Value[Region], value: SValue, deepCopy: Boolean)
    : SValue =
    value.st match {
...
class SCanonicalCallValue(val call: Value[Int]) extends SCallValue {
  ...
  override def forEachAllele(cb: EmitCodeBuilder)(alleleCode: Value[Int] => Unit): Unit = {
    ...
    cb.if_(
      p.ceq(2), {
        cb.if_(
          call2 < Genotype.nCachedAllelePairs, {
            cb.assign(
              j,
              Code.invokeScalaObject1[Int, Int](Genotype.getClass, "cachedAlleleJ", call2),
            )
            cb.assign(
              k,
              Code.invokeScalaObject1[Int, Int](Genotype.getClass, "cachedAlleleK", call2),
            )
          }, {
            cb.assign(
              k,
              (Code.invokeStatic1[Math, Double, Double](
                "sqrt",
                const(8d) * call2.toD + 1d,
              ) / 2d - 0.5).toI,
            )
      ...
      cb.if_(
        p.ceq(1),
        alleleCode(call2),
        cb.if_(p.cne(0), cb.append(Code._fatal[Unit](const("invalid ploidy: ").concat(p.toS)))),
      ),
    )
  }
    ...
    cb.switch(
      ...
      FastSeq(
        ...
        { () =>
          ...
          val newCall = Code.invokeScalaObject2[Int, Boolean, Int](
            Call1.getClass,
            "apply",
            checkAndTranslate(cb, allele),
            isPhased(cb),
          )
          cb.assign(repr, newCall)
        }, // ploidy 1
        { () =>
          val allelePair = cb.newLocal[Int](
            "allelePair",
            Code.invokeScalaObject1[Int, Int](Call.getClass, "allelePairUnchecked", call),
          )
          val j = cb.newLocal[Int](
            "allele_j",
            Code.invokeScalaObject1[Int, Int](AllelePair.getClass, "j", allelePair),
          )
          val k = cb.newLocal[Int](
            "allele_k",
            Code.invokeScalaObject1[Int, Int](AllelePair.getClass, "k", allelePair),
          )
          ...
          cb.assign(
            repr,
            Code.invokeScalaObject4[Int, Int, Boolean, Int, Int](
              Call2.getClass,
              "withErrorID",
  ...
  override def sizeToStoreInBytes(cb: EmitCodeBuilder): SInt64Value =
    new SInt64Value(this.pt.byteSize)
}
...
final class SCanonicalCallSettable(override val call: Settable[Int])
    extends SCanonicalCallValue(call) with SSettable {
