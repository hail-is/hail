...
abstract class EType extends BaseType with Serializable with Requiredness {
  ...
  type StagedInplaceDecoder =
    (EmitCodeBuilder, Value[Region], Value[Long], Value[InputBuffer]) => Unit

  final def buildEncoder(ctx: ExecuteContext, t: PType)
    : (OutputBuffer, HailClassLoader) => Encoder = {
    val f = EType.buildEncoder(ctx, this, t)
    (out: OutputBuffer, theHailClassLoader: HailClassLoader) =>
      new CompiledEncoder(out, theHailClassLoader, f)
  }

  final def buildDecoder(ctx: ExecuteContext, requestedType: Type)
    : (PType, (InputBuffer, HailClassLoader) => Decoder) = {
  ...
  final def buildStructDecoder(ctx: ExecuteContext, requestedType: TStruct)
    : (PStruct, (InputBuffer, HailClassLoader) => Decoder) = {
  ...
  final def buildEncoder(st: SType, kb: EmitClassBuilder[_]): StagedEncoder = {
    ...
    { (cb: EmitCodeBuilder, sv: SValue, ob: Value[OutputBuffer]) =>
      cb.invokeVoid(mb, cb.this_, sv, ob)
    }
  }

  final def buildEncoderMethod(st: SType, kb: EmitClassBuilder[_]): EmitMethodBuilder[_] =
    kb.getOrGenEmitMethod(
      s"ENCODE_${st.asIdent}_TO_$asIdent",
      (st, this, "ENCODE"),
  ...
  final def buildDecoderMethod[T](t: Type, kb: EmitClassBuilder[_]): EmitMethodBuilder[_] = {
    ...
    kb.getOrGenEmitMethod(
      s"DECODE_${asIdent}_TO_${st.asIdent}",
      (t, this, "DECODE"),
  ...
  final def buildInplaceDecoderMethod(pt: PType, kb: EmitClassBuilder[_]): EmitMethodBuilder[_] =
    kb.getOrGenEmitMethod(
      s"INPLACE_DECODE_${asIdent}_TO_${pt.asIdent}",
  ...
  final def buildSkip(kb: EmitClassBuilder[_])
    : (EmitCodeBuilder, Value[Region], Value[InputBuffer]) => Unit = {
  ...
  override final def pretty(sb: StringBuilder, indent: Int, compact: Boolean): Unit = {
    if (required)
      sb.append("+")
