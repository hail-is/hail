...
/** ChunkCache minimizes calls to free and allocate by holding onto chunks when they are no longer
  ...
  * certain amount, the size is rounded up to the nearest power of 2 and the small chunk cache is
  * checked for available chunk. If bigger, the big chunk cache returns the chunk whose size is the
  * ceiling match. If the size requested is at least 90 percent of the size of the chunk returned,
  * then that chunk is used. If no acceptable chunk is found, a new chunk is created. If the chunk
  * created plus the current allocation is greater than peak usage, than chunks from the cache are
  * deallocated until this condition is not true or the cache is empty. When freeChunk is called on
  * RegionPool, the chunks get put in the cache that corresponds to their size. freeAll releases all
  * chunks and is called when RegionPool is closed.
  */
...
private class ChunkCache(allocator: Long => Long, freer: Long => Unit) {
  ...
  def freeChunksFromCacheToFit(pool: RegionPool, sizeToFit: Long): Unit = {
    ...
    while (
      (sizeToFit + pool.getTotalAllocatedBytes) > pool.getHighestTotalUsage &&
  ...
  def getChunk(pool: RegionPool, size: Long): (Long, Long) = {
    ...
    if (size <= biggestSmallChunk) {
      ...
      if (
        closestSize != null && (closestSize.getKey == size
