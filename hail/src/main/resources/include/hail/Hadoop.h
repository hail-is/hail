...
class HadoopConfig {
  private:
    ...
    jclass rich_hadoop_conf_class = up_.env()->FindClass("is/hail/utils/richUtils/RichHadoopConfiguration");
    jmethodID unsafe_writer_method_id = up_.env()->GetMethodID(rich_hadoop_conf_class, "unsafeWriter", "(Ljava/lang/String;)Ljava/io/OutputStream;");
...
std::shared_ptr<OutputStream> HadoopConfig::unsafe_writer(const char *path) {
  ...
  jobject joutput_stream = up_.env()->CallObjectMethod(jhadoop_config_, unsafe_writer_method_id, jpath);
