...
package object experimental {
  ...
  def calcFilterAlleleFreq(ac: Int, an: Int, ci: Double): Double =
    calcFilterAlleleFreq(ac, an, ci, lower = 1e-10, upper = 2, tol = 1e-7, precision = 1e-6)

  def haplotypeFreqEM(gtCounts: IndexedSeq[Int]): IndexedSeq[Double] = {

    assert(
      gtCounts.size == 9,
      "haplotypeFreqEM requires genotype counts for the 9 possible genotype combinations.",
    )
    ...
    /** Constant quantities for each of the different haplotypes: n.AB => 2*n.AABB + n.AaBB + n.AABb
      * n.Ab => 2*n.AAbb + n.Aabb + n.AABb n.aB => 2*n.aaBB + n.AaBB + n.aaBb n.ab => 2*n.aabb +
      * n.aaBb + n.Aabb
      */
    ...
    var p_next =
      (const_counts +:+ new DenseVector(Array.fill[Double](4)(_gtCounts(4) / 2.0))) /:/ nHaplotypes
      ...
      p_next =
        (const_counts +:+
          (new DenseVector(Array[Double](
            ...
            p_cur(1) * p_cur(2), // n.aB
            p_cur(0) * p_cur(3), // n.ab
          )) * (_gtCounts(4) / ((p_cur(0) * p_cur(3)) + (p_cur(1) * p_cur(2)))))) / nHaplotypes
