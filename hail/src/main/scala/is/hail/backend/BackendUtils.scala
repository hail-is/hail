...
class BackendUtils(mods: Array[(String, (FS, Int, Region) => BackendUtils.F)]) {
  ...
  def collectDArray(backendContext: BackendContext, fs: FS, modID: String, contexts: Array[Array[Byte]], globals: Array[Byte], tsd: Option[TableStageDependency]): Array[Array[Byte]] = {
    if (contexts.isEmpty)
      return Array()
