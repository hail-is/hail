...
object DateFormatUtils {
  def parseDateFormat(str: String, locale: Locale): DateTimeFormatter = {
    ...
    def spec(c: Char): Unit = c match {
      ...
      case 'j' => fmt.appendValue(ChronoField.DAY_OF_YEAR)
      ...
      case 't' => char('\t')
      case 'U' => fmt.appendValue(ChronoField.ALIGNED_WEEK_OF_YEAR, 2) // NOTE: sunday first day
      case 'u' => fmt.appendValue(ChronoField.DAY_OF_WEEK) // NOTE: 1-7
      case 'v' => alternating("e-b-Y")
      case 'W' => fmt.appendValue(ChronoField.ALIGNED_WEEK_OF_YEAR, 2) // NOTE: monday first day
