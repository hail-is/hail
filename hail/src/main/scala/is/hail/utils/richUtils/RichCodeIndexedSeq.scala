...
import is.hail.asm4s.{Code, LineNumber, MethodBuilder, TypeInfo, Value}
import is.hail.expr.ir.EmitMethodBuilder

class RichCodeIndexedSeq[T](cs: IndexedSeq[Code[T]]) {
  def cacheEntries(mb: EmitMethodBuilder[_], ti: TypeInfo[T])(implicit line: LineNumber): (Code[Unit], IndexedSeq[Value[T]]) = {
