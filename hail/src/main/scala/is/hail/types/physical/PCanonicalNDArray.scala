...
final case class PCanonicalNDArray(elementType: PType, nDims: Int, required: Boolean = false)
  ...
  assert(
    !elementType.containsPointers,
    "ndarrays do not currently support elements which contain arrays, ndarrays, or strings",
  )
  ...
  override def unstagedLoadStrides(addr: Long): IndexedSeq[Long] =
    (0 until nDims).map(dimIdx => this.loadStride(addr, dimIdx))
  ...
  private def getElementAddress(
    ...
    cb.newLocal[Long](
      "pcndarray_get_element_addr",
      indices.zipWithIndex.map { case (requestedElementIndex, strideIndex) =>
  ...
  def setElement(
    ...
    elementType.storeAtAddress(
      cb,
      getElementAddress(cb, indices, ndAddress),
      region,
      newElement,
      deepCopy,
    )

  private def getElementAddressFromDataPointerAndStrides(
    indices: IndexedSeq[Value[Long]],
    dataFirstElementPointer: Value[Long],
  ...
  def loadElementFromDataAndStrides(
    cb: EmitCodeBuilder,
    indices: IndexedSeq[Value[Long]],
    ndDataAddress: Value[Long],
    strides: IndexedSeq[Value[Long]],
  ): Code[Long] = {
  ...
  def constructUninitialized(
    ...
    constructByCopyingDataPointer(
      shape,
      makeColumnMajorStrides(shape, cb),
      this.allocateData(shape, region),
      cb,
      region,
    )
  ...
  def constructByCopyingArray(
    ...
    val mb = cb.emb.ecb.getOrGenEmitMethod(
      "pcndarray_construct_by_copying_array",
      cacheKey,
      FastSeq[ParamType](classInfo[Region], dataCode.st.paramType) ++ (0 until 2 * nDims).map(_ =>
        CodeParamType(LongInfo)
      ),
      sType.paramType,
    ) { mb =>
      mb.emitSCode { cb =>
    ...
    cb.invokeSCode(
      mb,
      FastSeq[Param](cb.this_, region, SCodeParam(dataCode)) ++ (newShape.map(
        CodeParam(_)
      ) ++ strides.map(CodeParam(_))): _*
    )
      .asNDArray
  ...
  def constructByCopyingDataPointer(
    ...
    shapeType.storeAtAddress(
      cb,
      cb.newLocal[Long]("construct_shape", this.representation.fieldOffset(ndAddr, "shape")),
      region,
      SStackStruct.constructFromArgs(
        cb,
        region,
        shapeType.virtualType,
        shape.map(s => EmitCode.present(cb.emb, primitive(s))): _*
      ),
      false,
    )
    strideType.storeAtAddress(
      cb,
      cb.newLocal[Long]("construct_strides", this.representation.fieldOffset(ndAddr, "strides")),
      region,
      SStackStruct.constructFromArgs(
        cb,
        region,
        strideType.virtualType,
        strides.map(s => EmitCode.present(cb.emb, primitive(s))): _*
      ),
      false,
    )
  ...
  def constructByActuallyCopyingData(
    ...
    val numDataBytes = cb.newLocal(
      "constructByActuallyCopyingData_numDataBytes",
      Region.getSharedChunkByteSize(oldDataAddr),
    )
    cb.if_(numDataBytes < 0L, cb._fatal("numDataBytes was ", numDataBytes.toS))
    val newDataAddr = cb.newLocal(
      "constructByActuallyCopyingData_newDataAddr",
      region.allocateSharedChunk(numDataBytes),
    )
  ...
  def unstagedStoreAtAddress(
    ...
    if (equalModuloRequired(srcPType)) { // The situation where you can just memcpy
      Region.copyFrom(
        srcAddress,
        destAddress,
        this.representation.field("shape").typ.byteSize + this.representation.field(
          "strides"
        ).typ.byteSize,
      )
  ...
  def storeAtAddress(
    ...
    shapeType.storeAtAddress(
      cb,
      cb.newLocal[Long]("construct_shape", this.representation.fieldOffset(targetAddr, "shape")),
      region,
      SStackStruct.constructFromArgs(
        cb,
        region,
    ...
    strideType.storeAtAddress(
      ...
      cb.newLocal[Long](
        "construct_strides",
        this.representation.fieldOffset(targetAddr, "strides"),
      ),
      region,
      SStackStruct.constructFromArgs(
        cb,
