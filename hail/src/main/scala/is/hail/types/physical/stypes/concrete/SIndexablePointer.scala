...
final class SIndexablePointerSettable(
  ...
  def store(cb: EmitCodeBuilder, pc: SValue): Unit = pc match {
    case pc: SIndexablePointerValue =>
      cb.assign(a, pc.a)
      cb.assign(length, pc.length)
      cb.assign(elementsAddress, pc.elementsAddress)
  }
}
