...
import is.hail.types.{RIterable, RPrimitive, TypeWithRequiredness, VirtualTypeWithReq, virtual}
...
object AggStateSig {
  ...
  def apply(op: AggOp, initOpArgs: Seq[(IR, TypeWithRequiredness)], seqOpArgs: Seq[(IR, TypeWithRequiredness)]): AggStateSig = {
    ...
    op match {
      ...
      case NDArraySum() => NDArraySumStateSig(seqVTypes.head)
...
case class LinearRegressionStateSig(xt: VirtualTypeWithReq) extends AggStateSig(Array[VirtualTypeWithReq](xt), None)
...
object Extract {
  ...
  def getAgg(sig: PhysicalAggSig): StagedAggregator = sig match {
    ...
    case PhysicalAggSig(LinearRegression(), LinearRegressionStateSig(xt)) => new LinearRegressionAggregator(xt)
