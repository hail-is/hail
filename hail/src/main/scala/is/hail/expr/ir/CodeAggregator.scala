...
abstract class BaseCodeAggregator2[Agg <: RegionValueAggregator : ClassTag : TypeInfo] {
  def out: Type

  def initOpArgTypes: Option[Array[Type]]

  def seqOpArgTypes:  Array[Type]
...
case class CodeAggregator2[Agg <: RegionValueAggregator : ClassTag : TypeInfo](
  out: Type,
  constrArgTypes: Array[Type] = Array.empty[Type],
  initOpArgTypes: Option[Array[Type]] = None,
  seqOpArgTypes: Array[Type] = Array.empty[Type]) extends BaseCodeAggregator2[Agg] {

  def initOp(mb: EmitMethodBuilder, rva: Code[RegionValueAggregator], vs: Array[Code[_]], ms: Array[Code[Boolean]]): Code[Unit] = {
    assert(initOpArgTypes.isDefined && vs.length == ms.length)
    val argTypes = initOpArgTypes.get.flatMap(t => Array[Class[_]](TypeToIRIntermediateClassTag(t).runtimeClass, classOf[Boolean]))
    val args = vs.zip(ms).flatMap { case (v, m) => Array(v, m) }
    Code.checkcast[Agg](rva).invoke("initOp", argTypes, args)(classTag[Unit])
  }
  ...
  def seqOp(mb: EmitMethodBuilder, region: Code[Region], rva: Code[RegionValueAggregator], vs: Array[Code[_]], ms: Array[Code[Boolean]]): Code[Unit] = {
    assert(vs.length == ms.length)
    val argTypes = seqOpArgTypes.flatMap(t => Array[Class[_]](TypeToIRIntermediateClassTag(t).runtimeClass, classOf[Boolean]))
    val args = vs.zip(ms).flatMap { case (v, m) => Array(v, m) }
    Code.checkcast[Agg](rva).invoke("seqOp", Array(classOf[Region]) ++ argTypes, Array(region) ++ args)(classTag[Unit])
  }
  ...
  def stagedNew(v: Array[Code[_]], m: Array[Code[Boolean]]): Code[Agg] = {
    ...
    anyArgMissing.mux(
      Code._throw(Code.newInstance[RuntimeException, String]("Aggregators must have non missing constructor arguments")),
      Code.newInstance[Agg](constrArgTypes.map(TypeToIRIntermediateClassTag(_).runtimeClass), v))
  }
}
//
//case class KeyedCodeAggregator2[Agg <: RegionValueAggregator : ClassTag : TypeInfo](
//  out: TDict,
//  constrArgTypes: Array[Class[_]] = Array.empty[Class[_]],
//  initOpArgTypes: Option[Array[Class[_]]] = None,
//  seqOpArgTypes: Array[Class[_]] = Array.empty[Class[_]]) extends BaseCodeAggregator[Agg] {
...
//  }
//
//  def seqOp(mb: EmitMethodBuilder, region: Code[Region], krva: Code[RegionValueAggregator], args: Array[EmitTriplet]): Code[Unit] = {
//
