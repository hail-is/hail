...
class NDArraySumAggregator(ndVTyp: VirtualTypeWithReq) extends StagedAggregator {
  ...
  override protected def _seqOp(cb: EmitCodeBuilder, state: State, seq: Array[EmitCode]): Unit = {
    ...
    val seqOpMethod = cb.emb.genEmitMethod(
      "ndarray_sum_aggregator_seq_op",
      FastSeq(nextNDCode.emitParamType),
      CodeParamType(UnitInfo),
    )

    seqOpMethod.voidWithBuilder { cb =>
      val nextNDInput = seqOpMethod.getEmitParam(cb, 1)
      nextNDInput.toI(cb).consume(
        cb,
        {},
        { case nextNDPV: SNDArrayValue =>
...
object NDArraySumAggregator {
  ...
  def addValues(
    cb: EmitCodeBuilder,
    region: Value[Region],
    leftNdValue: SNDArrayValue,
    rightNdValue: SNDArrayValue,
  ): Unit = {
    cb.if_(
      !leftNdValue.sameShape(cb, rightNdValue),
