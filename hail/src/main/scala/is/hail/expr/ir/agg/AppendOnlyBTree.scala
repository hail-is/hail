...
class AppendOnlyBTree(kb: EmitClassBuilder[_], val key: BTreeKey, region: Value[Region], root: Settable[Long], maxElements: Int = 2) {
  ...
  private def insert(cb: EmitCodeBuilder, nodec: Value[Long], insertIdxc: Value[Int], kc: EmitCode, childC: Value[Long]): Value[Long] = {
    ...
    val insertAt = kb.getOrDefineEmitMethod("btree_insert",
      ...
      def promote(cb: EmitCodeBuilder, idx: Int): Unit = {
        ...
        cb.define(Lfound)
        key.copy(cb, loadKey(cb, node, idx), cb.invokeCode(insertAt, cb.this_, parent(cb), upperBound, key.loadCompKey(cb, nikey), newNode))
      ...
      def splitAndInsert(cb: EmitCodeBuilder): Code[Long] = {
        ...
        cb.if_(insertIdx > splitIdx, {
          ...
          promote(cb, splitIdx)
          cb.assign(out, cb.invokeCode(insertAt, cb.this_, newNode, cb.memoize(insertIdx - splitIdx - 1), k, child))
        }, {
  ...
  def getOrElseInitialize(cb: EmitCodeBuilder, k: EmitCode): Code[Long] =
    cb.invokeCode[Long](getF(k.emitParamType), cb.this_, root, k)
  ...
  def bulkStore(cb: EmitCodeBuilder, obCode: Value[OutputBuffer]
    ...
    f.voidWithBuilder { cb =>
      ...
      cb.if_(!isLeaf(cb, node), {
        cb.invokeVoid(f, cb.this_, loadChild(cb, node, -1), ob)
      })
      val Lexit = CodeLabel()
      (0 until maxElements).foreach { i =>
        cb.if_(hasKey(cb, node, i), {
          ...
          cb.if_(!isLeaf(cb, node), {
            cb.invokeVoid(f, cb.this_, loadChild(cb, node, i), ob)
          })
        }, {
