...
abstract class BlockMatrixWriter {
  ...
  def loweredTyp: Type
  def lower(ctx: ExecuteContext, s: BlockMatrixStage2, relationalBindings: Map[String, IR], eltR: TypeWithRequiredness): IR =
...
case class BlockMatrixNativeWriter(
  ...
  override def lower(ctx: ExecuteContext, s: BlockMatrixStage2, relationalBindings: Map[String, IR], eltR: TypeWithRequiredness): IR = {
    if (stageLocally)
    ...
    val paths = s.collectBlocks(relationalBindings, "block_matrix_native_writer") { (ctx, idx, block) =>
...
case class BlockMatrixBinaryWriter(path: String) extends BlockMatrixWriter {
  ...
  override def lower(ctx: ExecuteContext, s: BlockMatrixStage2, relationalBindings: Map[String, IR], eltR: TypeWithRequiredness): IR = {
    val nd = s.collectLocal(relationalBindings, "block_matrix_binary_writer")
