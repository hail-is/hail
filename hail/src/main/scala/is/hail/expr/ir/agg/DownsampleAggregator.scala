...
class DownsampleState(val kb: EmitClassBuilder[_], labelType: VirtualTypeWithReq, maxBufferSize: Int = 256) extends AggregatorState {
  ...
  def insertIntoTree(cb: EmitCodeBuilder, binX: Code[Int], binY: Code[Int], point: Code[Long], deepCopy: Boolean): Unit = {
    ...
    cb += mb.invokeCode(binX, binY, point)
  }
  ...
  def dumpBuffer(cb: EmitCodeBuilder): Unit = {
    ...
    val mb = kb.getOrGenEmitMethod(name, (this, name), FastIndexedSeq[ParamType](), UnitInfo) { mb =>
      ...
      mb.voidWithBuilder { cb =>
        cb.ifx(buffer.size.cne(0), cb += Code._return[Unit](Code._empty))
      ...
      mb.voidWithBuilder { cb =>
        ...
        l.toI(cb)
          .consume(cb,
            ...
            {
              sc => pointType.fieldType("label").storeAtAddress(cb, pointType.fieldOffset(pointStaging, "label"), region, sc, deepCopy = true)
            }
          )
        pointType.fieldType("y").storeAtAddress(cb, pointType.fieldOffset(pointStaging, "y"), region, y, true)
        ...
        { xcode =>
          y.toI(cb)
            .consume(cb,
              ...
              ycode => cb += mb.invokeCode(xcode, ycode, l)
            )
        })
  }
