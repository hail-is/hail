...
class AppendOnlySetState(val kb: EmitClassBuilder[_], vt: VirtualTypeWithReq)
    ...
    tree.foreach(cb) { (cb, eoffCode) =>
      ...
      f(
        ...
        EmitCode.fromI(cb.emb)(cb =>
          IEmitCode(cb, key.isKeyMissing(cb, eoff), key.loadKey(cb, eoff))
        ),
      )
    }
