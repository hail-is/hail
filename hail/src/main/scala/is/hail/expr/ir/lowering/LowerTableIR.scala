    ...
    def lower(tir: TableIR): TableStage = {
      ...
      tir match {
        ...
        case TableHead(child, targetNumRows) =>
          ...
          a table stage that consumes this CDA to generate the contexts."

          So the final IR generated by this thing will use CDA twice? Once to map the partitions to their sizes
          and use recur to find out how many partitions to take. Then a TableStage will be created that actually
          subsets to the desired number of partitions, and when that TableStage is turned to IR it'll have its own CDA
           */
