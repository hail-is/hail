...
object Simplify {
  ...
  private[this] def valueRules: PartialFunction[IR, IR] = {
    ...
    case ApplyIR("indexArray", _, _, Seq(a, i@I32(v)), errorID) if v >= 0 =>
      ArrayRef(a, i, errorID)

    case ApplyIR("contains", _, _, Seq(CastToArray(x), element), _) if x.typ.isInstanceOf[TSet] => invoke("contains", TBoolean, x, element)

    case ApplyIR("contains", _, _, Seq(Literal(t, v), element), _) if t.isInstanceOf[TArray] =>
      invoke("contains", TBoolean, Literal(TSet(t.asInstanceOf[TArray].elementType), v.asInstanceOf[IndexedSeq[_]].toSet), element)

    case ApplyIR("contains", _, _, Seq(ToSet(x), element), _) if x.typ.isInstanceOf[TArray] => invoke("contains", TBoolean, x, element)
    ...
    case ApplyIR("annotate", _, _, Seq(s, MakeStruct(fields)), _) =>
