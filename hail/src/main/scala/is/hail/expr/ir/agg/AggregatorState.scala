...
trait AggregatorState {
  ...
  def deserializeFromBytes(cb: EmitCodeBuilder, bytes: SBinaryValue): Unit = {
    val lazyBuffer = kb.getOrDefineLazyField[MemoryBufferWrapper](
      Code.newInstance[MemoryBufferWrapper](),
      ("AggregatorStateBufferWrapper"),
    )
  ...
  def serializeToRegion(cb: EmitCodeBuilder, t: PBinary, r: Value[Region]): SValue = {
    val lazyBuffer = kb.getOrDefineLazyField[MemoryWriterWrapper](
      Code.newInstance[MemoryWriterWrapper](),
      ("AggregatorStateWriterWrapper"),
    )
...
class TypedRegionBackedAggState(val typ: VirtualTypeWithReq, val kb: EmitClassBuilder[_])
    extends AbstractTypedRegionBackedAggState(typ.canonicalPType)
...
abstract class AbstractTypedRegionBackedAggState(val ptype: PType) extends RegionBackedAggState {
  ...
  override def store(
    ...
    cb.if_(
      region.isValid,
      cb.if_(dest.cne(off), cb += Region.copyFrom(off, dest, const(storageType.byteSize))),
    )
    super.store(cb, regionStorer, dest)
  }
  ...
  def apply(i: Int): AggregatorState = {
    if (i >= states.length)
      throw new RuntimeException(
        s"tried to access state $i, but there are only ${states.length} states"
      )
    states(i)
  }

  def toCode(f: (Int, AggregatorState) => Unit): Unit =
    (0 until nStates).foreach(i => f(i, states(i)))

  def toCodeWithArgs(
    cb: EmitCodeBuilder,
    f: (EmitCodeBuilder, Int, AggregatorState) => Unit,
  ): Unit =
    (0 until nStates).foreach(i => f(cb, i, states(i)))
...
class TupleAggregatorState(
  ...
  private def getRegion(i: Int): (EmitCodeBuilder, Value[Region]) => Unit = {
    (cb: EmitCodeBuilder, r: Value[Region]) =>
      cb += r.setFromParentReference(topRegion, rOff + const(i), states(i).regionSize)
  }

  private def setRegion(i: Int): (EmitCodeBuilder, Value[Region]) => Unit = {
    (cb: EmitCodeBuilder, r: Value[Region]) =>
      cb += topRegion.setParentReference(r, rOff + const(i))
  }

  private def getStateOffset(cb: EmitCodeBuilder, i: Int): Value[Long] =
    cb.memoize(storageType.loadField(off, i))
  ...
  def newState(cb: EmitCodeBuilder): Unit =
    states.toCode((i, s) => s.newState(cb, getStateOffset(cb, i)))
