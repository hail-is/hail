...
import is.hail.rvd.{AbstractRVDSpec, IndexSpec, MakeRVDSpec, RVDPartitioner}
import is.hail.types.{RTable, TableType}
import is.hail.types.encoded.{EBaseStruct, EType}
import is.hail.types.physical.{PArray, PBaseStruct, PCanonicalBaseStructSettable, PCanonicalString, PCanonicalStruct, PCode, PInt64, PStream, PString, PStruct, PType}
...
case class TableNativeWriter(
  ...
  override def lower(ctx: ExecuteContext, ts: TableStage, t: TableIR, r: RTable): IR = {
    ...
    val metadataWriter = MetadataNativeWriter(path, overwrite, rowSpec, globalSpec, t.typ, partitioner, pKey)
...
class NativeTableMetadata(path: String, typ: TableType, partitioner: RVDPartitioner, rowSpec: AbstractTypedCodecSpec, globalSpec: AbstractTypedCodecSpec, keyType: PStruct) {
  ...
  def write(fs: FS, globalPath: String, partFiles: Array[String], partitionCounts: Array[Long]): Unit = {
    // globalMetadata
    val gSpec = MakeRVDSpec(FastIndexedSeq(), globalSpec, Array(globalPath), RVDPartitioner.unkeyed(1))
    gSpec.write(fs, globalsPath)

    // rowMetadata
    val rSpec = MakeRVDSpec(keyType.fieldNames, rowSpec, partFiles, partitioner, IndexSpec.emptyAnnotation("../index", coerce[PStruct](keyType)))
    rSpec.write(fs, rowsPath)
...
case class MetadataNativeWriter(
  path: String,
  overwrite: Boolean,
  rowSpec: AbstractTypedCodecSpec,
  globalSpec: AbstractTypedCodecSpec,
  typ: TableType,
  ...
  val metadata = new NativeTableMetadata(path, typ, partitioner, rowSpec, globalSpec, pKey)
  def writeMetadata(
