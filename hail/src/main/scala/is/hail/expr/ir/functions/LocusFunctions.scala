...
object LocusFunctions extends RegistryFunctions {
  ...
  def registerLocusCodePredicate(methodName: String): Unit =
    registerSCode1(methodName, tlocus("T"), TBoolean, (_: Type, _: SType) => SBoolean) {
      case (_, cb, _, locus: SLocusValue, _) =>
        primitive(
          cb.memoize(
            cb.emb
              .getReferenceGenome(locus.st.rg).invoke[Locus, Boolean](
                methodName,
    ...
    registerSCode1(methodName, tlocus("T"), TBoolean, (_: Type, _: SType) => SBoolean) {
      case (_, cb, _, locus: SLocusValue, _) =>
        primitive(
          cb.memoize(
            cb.emb
              .getReferenceGenome(locus.st.rg).invoke[String, Boolean](
                methodName,
  ...
  def registerAll(): Unit = {
    ...
    registerSCode1("contig_idx", tlocus("T"), TInt32, (_: Type, _: SType) => SInt32) {
      case (_, cb, _, locus: SLocusValue, _) =>
        primitive(
          cb.memoize(
            cb.emb
              .getReferenceGenome(locus.st.rg).invoke[String, Int](
                "getContigIndex",
                locus.contig(cb).loadString(cb),
              )
          )
    ...
    registerSCode1("position", tlocus("T"), TInt32, (_: Type, _: SType) => SInt32) {
      case (_, cb, _, locus: SLocusValue, _) =>
