  ...
  def analyzeIR(node: IR): Boolean = {
    ...
    node match {
      ...
      case ArraySlice(a, start, stop, step, _) =>
        ...
        val stopReq = if (stop.isDefined) lookup(stop.get).required else true
        requiredness.union(
      ...
      case StreamFlatMap(a, _, body) =>
        requiredness.union(lookup(a).required)
        tcoerce[RIterable](requiredness).elementType
          .unionFrom(lookupAs[RIterable](body).elementType)
      ...
      case ApplyAggOp(_, seqOpArgs, aggSig) => // FIXME round-tripping through ptype
        val emitResult = agg
          .PhysicalAggSig(
            aggSig.op,
            agg.AggStateSig(aggSig.op, seqOpArgs.map(s => s -> lookup(s))),
          ).emitResultType
      ...
      case x: AbstractApplyNode[_] => // FIXME: round-tripping via PTypes.
        ...
        requiredness.unionFrom(
          x.implementation.computeReturnEmitType(x.returnType, argP).typeWithRequiredness.r
        )
