...
import is.hail.expr.ir.{BlockMatrixIR, Copy, ExecuteContext, IR, MatrixIR, Pretty, TableIR}

object LowerIR {
  def lower(ir: IR, typesToLower: DArrayLowering.Type, ctx: ExecuteContext): IR = ir match {
    case node if node.children.forall(_.isInstanceOf[IR]) =>
      Copy(node, ir.children.map { case c: IR => lower(c, typesToLower, ctx) })
    ...
    case node if node.children.exists(n => n.isInstanceOf[TableIR]) && node.children.forall(n => n.isInstanceOf[TableIR] || n.isInstanceOf[IR]) =>
      LowerTableIR(ir, typesToLower, ctx)
