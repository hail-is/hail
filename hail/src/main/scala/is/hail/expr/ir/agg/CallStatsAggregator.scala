...
object CallStatsState {
  ...
  val stateType: PCanonicalTuple =
    PCanonicalTuple(true, callStatsInternalArrayType, callStatsInternalArrayType)
...
class CallStatsState(val kb: EmitClassBuilder[_]) extends PointerBasedRVAState {
  ...
  def updateAlleleCountAtIndex(
    cb: EmitCodeBuilder,
    idx: Code[Int],
    length: Code[Int],
    updater: Code[Int] => Code[Int],
  ): Unit = {
    cb.assign(
      addr,
      CallStatsState.callStatsInternalArrayType.loadElement(alleleCounts, length, idx),
    )
  ...
  def copyFromAddress(cb: EmitCodeBuilder, src: Value[Long]): Unit = {
    cb.assign(
      ...
      CallStatsState.stateType.store(
        cb,
        region,
        CallStatsState.stateType.loadCheapSCode(cb, src),
        deepCopy = true,
      ),
    )
    loadNAlleles(cb)
...
class CallStatsAggregator extends StagedAggregator {
    ...
    nAlleles.toI(cb)
      .consume(
        ...
        { sc =>
          ...
          cb.assign(
            state.off,
            state.region.allocate(
              CallStatsState.stateType.alignment,
              CallStatsState.stateType.byteSize,
            ),
          )
  ...
  protected def _seqOp(cb: EmitCodeBuilder, state: State, seq: Array[EmitCode]): Unit = {
    ...
    call.toI(cb).consume(
      ...
      { case call: SCallValue =>
        ...
        cb.if_((i > 1) && hom, state.updateHomCountAtIndex(cb, lastAllele, state.nAlleles, _ + 1))
      },
    )
  }
  ...
  protected def _combOp(
    ...
    val i = state.kb.genFieldThisRef[Int]()
    cb.if_(
      other.nAlleles.cne(state.nAlleles),
      cb += Code._fatal[Unit]("hl.agg.call_stats: length mismatch"), {
        cb.assign(i, 0)
        cb.while_(
          i < state.nAlleles, {
            state.updateAlleleCountAtIndex(
              cb,
              i,
              state.nAlleles,
              _ + other.alleleCountAtIndex(i, state.nAlleles),
            )
            state.updateHomCountAtIndex(
              cb,
              i,
              state.nAlleles,
              _ + other.homCountAtIndex(i, state.nAlleles),
            )
            cb.assign(i, i + 1)
          },
        )
  ...
  protected def _result(cb: EmitCodeBuilder, state: State, region: Value[Region]): IEmitCode = {
    ...
    val ac = acType.constructFromElements(cb, region, state.nAlleles, deepCopy = true) { (cb, i) =>
      val acAtIndex =
        cb.newLocal[Int]("callstats_result_acAtIndex", state.alleleCountAtIndex(i, state.nAlleles))
    ...
    cb.if_(
      ...
      rt.setFieldMissing(cb, addr, "AF"), {
        ...
        val af =
          afType.constructFromElements(cb, region, state.nAlleles, deepCopy = true) { (cb, i) =>
            val acAtIndex = cb.newLocal[Int](
              "callstats_result_acAtIndex",
              state.alleleCountAtIndex(i, state.nAlleles),
            )
    ...
    val homCount =
      homCountType.constructFromElements(cb, region, state.nAlleles, deepCopy = true) { (cb, i) =>
        val homCountAtIndex = cb.newLocal[Int](
          "callstats_result_homCountAtIndex",
          state.homCountAtIndex(i, state.nAlleles),
        )
        IEmitCode.present(cb, primitive(homCountAtIndex))
      }

    homCountType.storeAtAddress(
      cb,
      rt.fieldOffset(addr, "homozygote_count"),
      region,
      homCount,
      deepCopy = false,
    )
