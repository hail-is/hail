...
//    case Ref(name, t) if t != null => FastSeq(prettyIdentifier(name), t.parsableString())  // For debug purposes
    ...
    case ApplyIR(function, typeArgs, _, _) => FastSeq(prettyIdentifier(function), prettyTypes(typeArgs), ir.typ.parsableString())
    case Apply(function, typeArgs, _, t, _) => FastSeq(prettyIdentifier(function), prettyTypes(typeArgs), t.parsableString())
    case ApplySeeded(function, _, seed, t) => FastSeq(prettyIdentifier(function), seed.toString, t.parsableString())
    case ApplySpecial(function, typeArgs, _, t, _) => FastSeq(prettyIdentifier(function), prettyTypes(typeArgs), t.parsableString())
