...
object Pretty {
  ...
  def apply(ir: BaseIR, width: Int = 80, elideLiterals: Boolean = true, maxLen: Int = -1): String = {
    ...
    def pretty(ir: BaseIR): Doc = {
      val header: Iterable[Doc] = ir match {
      ...
      val body: Iterable[Doc] = ir match {
        ...
        case InsertFields(old, fields, fieldOrder) =>
          ...
          FastSeq(pretty(old), prettyStringsOpt(fieldOrder), vsep(fieldDocs))
      ...
      */
      ...
      nest(2, group(concat("(", prettyClass(ir), vsep(Iterable.concat(FastSeq(nest(2, concat(header.map(d => group(concat(line, d)))))), body)), ")")))
    }
