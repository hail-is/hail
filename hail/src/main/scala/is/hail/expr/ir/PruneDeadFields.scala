...
object PruneDeadFields {
  ...
  def memoizeTableIR(
    ...
    tir match {
      ...
      case tir: TableParallelize =>
        ...
        memoizeValueIR(ctx, tir, 0, typ, memo)
      case _: TableRange =>
      ...
      case tir: TableGen =>
        val Seq(contextState, globalState) =
          memoizeValueIR(ctx, tir, 2, TStream(requestedType.rowType), memo)
        ...
        memoizeValueIR(ctx, tir, 0, TStream(contextsElemType), memo)
        memoizeValueIR(ctx, tir, 1, globalsType, memo)
      ...
      case TableMapPartitions(child, _, _, body, requestedKey, _) =>
        ...
        val Seq(globalState, partitionState) = memoizeValueIR(ctx, tir, 1, reqRowsType, memo)
      ...
      case TableMapGlobals(child, _) =>
        val Seq(globalState) = memoizeValueIR(ctx, tir, 1, requestedType.globalType, memo)
        memoizeTableIR(
          ctx,
          child,
          requestedType.copy(
  ...
  def memoizeMatrixIR(
    ...
    mir match {
      ...
      case MatrixMapGlobals(child, _) =>
        val Seq(globalState) = memoizeValueIR(ctx, mir, 1, requestedType.globalType, memo)
        memoizeMatrixIR(
          ctx,
          child,
      ...
      case MatrixAggregateRowsByKey(child, _, _) =>
        val Seq(entryGlobalState, entryColState, entryRowState, entryEntryState) =
          memoizeValueIR(ctx, mir, 1, requestedType.entryType, memo)
        val Seq(rowGlobalState, rowRowState) =
          memoizeValueIR(ctx, mir, 2, requestedType.rowValueStruct, memo)
        val childDep = MatrixType(
      ...
      case MatrixAggregateColsByKey(child, _, _) =>
        ...
        val Seq(colGlobalState, colColState) =
          memoizeValueIR(ctx, mir, 2, requestedType.colValueStruct, memo)
    ...
    bmir match {
      ...
      case _ =>
        bmir.children.zipWithIndex.foreach {
          ...
          case (ir: IR, i) => memoizeValueIR(ctx, bmir, i, ir.typ, memo)
        }
    }
  }
  ...
  def memoizeAndGetDep(
    ...
    val Seq(globalState, rowState) = memoizeValueIR(ctx, ir, childIdx, requestedType, memo)
  ...
  def memoizeValueIREntryBindings(
    ctx: ExecuteContext,
    ir: MatrixIR,
    childIdx: Int,
    requestedType: Type,
    ...
    val bindings = memoizeValueIR(ctx, ir, childIdx, requestedType, memo)
  ...
  def memoizeValueIR(
    ...
    memoizeValueIR(ctx, ir, childIdx, requestedType, memo, BindingEnv.empty.createAgg.createScan)
  ...
  def memoizeValueIR(
    ...
    def recur(ir: IR, childIdx: Int, requestedType: Type): IndexedSeq[TypeState] =
      memoizeValueIR(ctx, ir, childIdx, requestedType, memo, env)

    def recurMapMax(ir: IR, childIdx: Int, bindingsMap: mutable.Map[String, TypeState]): Unit =
      recurMap(ir, childIdx, ir.getChild(childIdx).asInstanceOf[IR].typ, bindingsMap)

    def recurMap(
      ir: IR,
      childIdx: Int,
      requestedType: Type,
      bindingsMap: mutable.Map[String, TypeState],
    ): Unit = {
...
//
//    def recur(ir: IR, requestedType: Type): Unit =
//      memoizeValueIR(ctx, ir, requestedType, memo, env)
    ...
    ir match {
      ...
      case Block(bindings, _) =>
        val typeStates = mutable.AnyRefMap.empty[String, TypeState]
        recurMap(ir, bindings.length, requestedType, typeStates)
        for (i <- bindings.indices.reverse)
          recurMap(ir, i, typeStates(bindings(i).name).newType, typeStates)
...
//        env.eval.lookupOption(name).foreach(_.union(requestedType))
      ...
      case StreamFold2(_, accum, valueName, seq, _) =>
        ...
        val seqBindings = mutable.AnyRefMap.empty[String, TypeState]
        seq.indices.foreach(i => recurMapMax(ir, accum.length + 1 + i, seqBindings))
      ...
      case RunAggScan(_, name, _, _, _, _) =>
        val bindings = mutable.AnyRefMap.empty[String, TypeState]
        recurMap(ir, 3, TIterable.elementType(requestedType), bindings)
        recurMapMax(ir, 2, bindings)
        recurMax(ir, 1)
