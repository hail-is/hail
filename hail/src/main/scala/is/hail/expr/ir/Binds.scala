...
object Bindings {
  ...
  def apply(x: BaseIR, i: Int): Iterable[(String, Type)] = x match {
    ...
    case CollectDistributedArray(contexts, globals, cname, gname, _, _, _, _, _) => if (i == 2) Array(cname -> tcoerce[TStream](contexts.typ).elementType, gname -> globals.typ) else empty
...
object AggBindings {
  ...
  def apply(x: BaseIR, i: Int, parent: BindingEnv[_]): Option[Iterable[(String, Type)]] = {
    ...
    x match {
      ...
      case CollectDistributedArray(_, _, _, _, _, _, _, _, _) if (i == 2) => None
...
object ChildEnvWithoutBindings {
  def apply[T](ir: BaseIR, i: Int, env: BindingEnv[T]): BindingEnv[T] = {
    ir match {
      ...
      case CollectDistributedArray(_, _, _, _, _, _, _, _, _) => if (i == 2) BindingEnv(relational = env.relational) else env
