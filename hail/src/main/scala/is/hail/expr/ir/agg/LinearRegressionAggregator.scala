...
import is.hail.types.physical.stypes.concrete.{
  SBaseStructPointer, SIndexablePointer, SIndexablePointerValue,
}
...
object LinearRegressionAggregator {
  ...
  val resultPType: PCanonicalStruct = PCanonicalStruct(
    required = false,
    "xty" -> optVector,
    "beta" -> optVector,
    "diag_inv" -> optVector,
...
class LinearRegressionAggregator() extends StagedAggregator {
  ...
  def initOpF(state: State)(cb: EmitCodeBuilder, kc: Code[Int], k0c: Code[Int]): Unit = {
    ...
    cb += Region.storeAddress(
      stateType.fieldOffset(state.off, 0),
      vector.zeroes(cb, state.region, k),
    )
    cb += Region.storeAddress(
      stateType.fieldOffset(state.off, 1),
      vector.zeroes(cb, state.region, k * k),
    )
            ...
            cb.while_(
              i < k, {
                cb += Region.storeDouble(
                  sptr,
                  Region.loadDouble(sptr) + (Region.loadDouble(xptr) * y),
                )
                cb.assign(i, i + 1)
              ...
              i < k, {
                ...
                cb.while_(
                  j < k, {
                    ...
                    cb += Region.storeDouble(
                      sptr,
                      Region.loadDouble(sptr) + (Region.loadDouble(xptr) * Region.loadDouble(xptr2)),
                    )
                    cb.assign(j, j + 1)
                    cb.assign(sptr, sptr + scalar.byteSize)
  ...
  protected def _result(cb: EmitCodeBuilder, state: State, region: Value[Region]): IEmitCode = {
    val resAddr = cb.newLocal[Long](
      ...
      Code.invokeScalaObject4[Region, Long, Long, Int, Long](
        LinearRegressionAggregator.getClass,
        "computeResult",
        region,
