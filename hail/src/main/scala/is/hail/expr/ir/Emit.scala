    ...
    val result: IEmitCode = (ir: @unchecked) match {
      ...
      case CollectDistributedArray(contexts, globals, cname, gname, body, dynamicID, staticID,
        ...
        emitStream(contexts, cb, region).map(cb) { case ctxStream: SStreamValue =>
          ...
          val contextSpec: TypedCodecSpec = TypedCodecSpec(ctx.executeContext, contextPTuple, bufferSpec)
          val globalSpec: TypedCodecSpec = TypedCodecSpec(ctx.executeContext, globalPTuple, bufferSpec)
          ...
          bodyFB.emitWithBuilder { cb =>
            ...
            bodySpec = TypedCodecSpec(ctx.executeContext, bodyResult.st.storageType().setRequired(true), bufferSpec)
