...
import is.hail.expr.ir.analyses.{CompilationUnitSize, InLoopCriticalPath, ParentPointers}
...
object EmitContext {
  def analyze(ctx: ExecuteContext, ir: IR): EmitContext = {
    ctx.timer.time("EmitContext.analyze") {
      ...
      val compilationUnitSize = CompilationUnitSize(ir)
      val inLoopCriticalPath = InLoopCriticalPath(ir, ParentPointers(ir))
      new EmitContext(ctx, requiredness, usesAndDefs, compilationUnitSize, inLoopCriticalPath)
    }
  }
}
...
class EmitContext(
  ...
  val compilationUnitSize: Memo[Int],
            ...
            IEmitCode(cb, missing, {
              ...
              new NDArrayEmitter(newShape, ndsArrayPValue.st.elementType.asInstanceOf[SNDArray].elementType) {
                override def outputElement(cb: EmitCodeBuilder, idxVars: IndexedSeq[Value[Long]]): SCode = {
                  ...
                  ndsArrayPValue.loadElement(cb, whichNDArrayToRead).get(cb).asNDArray.memoize(cb, "ndarray_to_load_element_from").loadElement(transformedIdxs, cb)
                }
              }
            })
          }
