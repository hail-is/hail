    ...
    (ir: @unchecked) match {
      ...
      case Let(name, value, body) => value.pType match {
        case streamType: PCanonicalStream =>
          ...
          val outerRegion = StagedRegion(region).asParent(separateRegions, "Let value")
      ...
      case StreamFor(a, valueName, body) =>
        ...
        assert(separateRegions == streamType.separateRegions)
        val outerRegion = StagedRegion(region).asParent(separateRegions, "StreamFor")
  ...
  private[ir] def emitI(ir: IR, cb: EmitCodeBuilder, region: Value[Region], env: E,
    ...
    def emitNDArrayColumnMajorStrides(ir: IR): IEmitCode = {
      emitI(ir).map(cb){case pNDCode: PNDArrayCode =>
        ...
        {
          cb.assign(pAnswer, LinalgCodeUtils.createColumnMajorCode(pNDValue, cb, region))
        })
        pAnswer
      }
    }
    ...
    val result: IEmitCode = (ir: @unchecked) match {
      ...
      case GroupByKey(collection) =>
        ...
        val outerRegion = StagedRegion(region).asParent(separateRegions, "GroupByKey")
      ...
      case ToArray(a) =>
        ...
        val separateRegions = ctx.smm.lookup(a).separateRegions
        assert(separateRegions == streamType.separateRegions)
        val outerRegion = StagedRegion(region).asParent(separateRegions, "ToArray")
      ...
      case x@StreamFold(a, zero, accumName, valueName, body) =>
        ...
        val separateRegions = ctx.smm.lookup(a).separateRegions
        assert(separateRegions == streamType.separateRegions)
        val outerRegion = StagedRegion(region).asParent(separateRegions, "StreamFold")
        val eltRegion = outerRegion.createChildRegion(mb)
        ...
        streamOpt.flatMap(cb) { stream =>
          ...
          cb.assign(xAcc, emitI(zero, eltRegion.code).map(cb)(_.castTo(cb, eltRegion.code, accType)))
          ...
          stream.asStream.stream.getStream(eltRegion).forEachI(cb, { elt =>
            ...
            // empty.
            cb.assign(xElt, elt)
            cb.assign(xAcc, emitI(body, eltRegion.code, env.bind(accumName -> xAcc, valueName -> xElt))
      ...
      case x@StreamFold2(a, acc, valueName, seq, res) =>
        ...
        val outerRegion = StagedRegion(region).asParent(separateRegions, "StreamFold2")
        val eltRegion = outerRegion.createChildRegion(mb)
        val tmpRegion = outerRegion.createChildRegion(mb)
        ...
        streamOpt.flatMap(cb) { stream =>
          ...
          (accVars, acc).zipped.foreach { case (xAcc, (_, x)) =>
            cb.assign(xAcc, emitI(x, eltRegion.code).map(cb)(_.castTo(cb, eltRegion.code, xAcc.pt)))
          }
          stream.asStream.stream.getStream(eltRegion).forEachI(cb, { elt =>
            ...
            (tmpAccVars, seq).zipped.foreach { (accVar, ir) =>
              cb.assign(accVar,
                emitI(ir, eltRegion.code, env = seqEnv)
      ...
      case ShuffleWrite(idIR, rowsIR) =>
        ...
        val outerRegion = StagedRegion(region).asParent(rowsPType.separateRegions, "ShuffleWrite")
        val eltRegion = outerRegion.createChildRegion(mb)
      ...
      case x@CollectDistributedArray(contexts, globals, cname, gname, body, tsd) =>
        ...
        val outerRegion = StagedRegion(region).asParent(separateRegions, "CDA")
        ...
        def etToTuple(cb: EmitCodeBuilder, et: EmitCode, t: PType): SBaseStructPointerCode = {
          PCanonicalTuple(false, t).constructFromFields(cb, region, FastIndexedSeq(et), deepCopy = false)
        }
  ...
  private def emit(
    ...
    val result: EmitCode = (ir: @unchecked) match {
      ...
      case x@(_: ArraySort | _: ToSet | _: ToDict) =>
        ...
        val outerRegion = StagedRegion(region).asParent(separateRegions, "ArraySort")
      ...
      case x@StreamLen(a) =>
        ...
        val outerRegion = StagedRegion(region).asParent(separateRegions, "StreamLen")
      ...
      case x@WritePartition(stream, pctx, writer) =>
        ...
        val outerRegion = StagedRegion(region).asParent(separateRegions, "WritePartition")
        COption.toEmitCode(
