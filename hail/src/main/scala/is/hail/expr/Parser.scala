...
object Parser extends JavaTokenParsers {
  ...
  def ir_value_expr_1(env: IRParserEnvironment): Parser[ir.IR] = {
    "I32" ~> int32_literal ^^ { x => ir.I32(x) } |
      ...
      "ArrayMap" ~> ir_identifier ~ ir_value_expr(env) ~ ir_value_expr(env) ^^ { case name ~ a ~ body => ir.ArrayMap(a, name, body) } |
      "ArrayFilter" ~> ir_identifier ~ ir_value_expr(env) ~ ir_value_expr(env) ^^ { case name ~ a ~ body => ir.ArrayFilter(a, name, body) } |
      "ArrayFlatMap" ~> ir_identifier ~ ir_value_expr(env) ~ ir_value_expr(env) ^^ { case name ~ a ~ body => ir.ArrayFlatMap(a, name, body) } |
      "ArrayFold" ~> ir_identifier ~ ir_identifier ~ ir_value_expr(env) ~ ir_value_expr(env) ~ ir_value_expr(env) ^^ { case accumName ~ valueName ~ a ~ zero ~ body => ir.ArrayFold(a, zero, accumName, valueName, body) } |
      "ArrayScan" ~> ir_identifier ~ ir_identifier ~ ir_value_expr(env) ~ ir_value_expr(env) ~ ir_value_expr(env) ^^ { case accumName ~ valueName ~ a ~ zero ~ body => ir.ArrayScan(a, zero, accumName, valueName, body) } |
      "ArrayFor" ~> ir_identifier ~ ir_value_expr(env) ~ ir_value_expr(env) ^^ { case name ~ a ~ body => ir.ArrayFor(a, name, body) } |
      "AggFilter" ~> ir_value_expr(env) ~ ir_value_expr(env) ^^ { case cond ~ aggIR => ir.AggFilter(cond, aggIR) } |
      "AggExplode" ~> ir_identifier ~ ir_value_expr(env) ~ ir_value_expr(env) ^^ { case name ~ array ~ aggBody => ir.AggExplode(array, name, aggBody) } |
