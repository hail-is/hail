  ...
  def buildPrefixEncoder(pt: PArray, mb: EmitMethodBuilder, array: Code[Long],
    ...
    val writeMissingBytes =
      if (!pt.elementType.required) {
        out.writeBytes(array + const(pt.lengthHeaderBytes), pt.nMissingBytes(prefixLen).toI)
      } else
        Code._empty[Unit]
  ...
  def _buildDecoder(
    ...
    Code(
      ...
      if (elementType.required) {
        ...
        in.readBytes(region, array + const(t.lengthHeaderBytes), t.nMissingBytes(len).toI),
      i := 0,
      Code.whileLoop(
        i < len,
        Code(
