...
abstract class PBaseStruct extends PType {
  ...
  final val fieldRequired: Array[Boolean] = types.map(_.required)
  final val allFieldsRequired: Boolean = fieldRequired.forall(_ == true)

  final val fieldIdx: Map[String, Int] =
    fields.map(f => (f.name, f.index)).toMap

  final val fieldNames: Array[String] = fields.map(_.name).toArray
