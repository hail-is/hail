...
object IndexReaderBuilder {
  ...
  def apply(keyType: Type, annotationType: Type): (Configuration, String, Int) => IndexReader = {
    ...
    val leafDecoder = codecSpec.buildDecoder(leafType.physicalType, leafType)
    val internalDecoder = codecSpec.buildDecoder(internalType.physicalType, internalType)
