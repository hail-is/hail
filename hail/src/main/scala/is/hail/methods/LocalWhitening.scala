...
class LocalWhitening(
  ...
  vecSize: SizeValue,
  _w: Value[Long],
  chunksize: Value[Long],
  _blocksize: Value[Long],
  region: Value[Region],
  normalizeAfterWhitening: Boolean,
) {
  val m = vecSize
  ...
  val Q = cb.memoizeField(
    matType.constructUninitialized(FastSeq(m, w), cb, region),
    "LW_Q",
  ).asNDArray.coerceToShape(cb, m, w)

  val R = cb.memoizeField(
    matType.constructUninitialized(FastSeq(w, w), cb, region),
    "LW_R",
  ).asNDArray.coerceToShape(cb, w, w)

  val work1 = cb.memoizeField(
    matType.constructUninitialized(FastSeq(wpb, wpb), cb, region),
    "LW_work1",
  ).asNDArray.coerceToShape(cb, wpb, wpb)

  val work2 = cb.memoizeField(
    matType.constructUninitialized(FastSeq(wpb, b), cb, region),
    "LW_work2",
  ).asNDArray.coerceToShape(cb, wpb, b)

  val Rtemp = cb.memoizeField(
    matType.constructUninitialized(FastSeq(wpb, wpb), cb, region),
    "LW_Rtemp",
  ).asNDArray.coerceToShape(cb, wpb, wpb)

  val Qtemp = cb.memoizeField(
    matType.constructUninitialized(FastSeq(m, b), cb, region),
    "LW_Qtemp",
  ).asNDArray.coerceToShape(cb, m, b)

  val Qtemp2 = cb.memoizeField(
    matType.constructUninitialized(FastSeq(m, w), cb, region),
    "LW_Qtemp2",
  ).asNDArray.coerceToShape(cb, m, w)
  ...
  val work3: SNDArrayValue = cb.memoizeField(
    vecType.constructUninitialized(FastSeq(work3len), cb, region),
    "LW_work3",
  ).asNDArray
  ...
  val T: SNDArrayValue =
    cb.memoizeField(vecType.constructUninitialized(FastSeq(Tlen), cb, region), "LW_T").asNDArray
  ...
  def whitenBlockPreOrthogonalized(
    cb: EmitCodeBuilder,
    Q1: SNDArrayValue,
    Q2: SNDArrayValue,
    Qout: SNDArrayValue,
    ...
    cb.for_(
      ...
      i.toL < w + n,
      cb.assign(i, i + 1),
      work1.setElement(FastSeq(i, i), primitive(1.0), cb),
    )
    ...
    cb.for_(
      ...
      cb.assign(i, i + 1), {
        ...
        SNDArray.tpqrt(
          R.slice(cb, (i + 1, null), (i + 1, null)),
          R.slice(cb, (i, i + 1), (i + 1, null)),
          T,
          work3,
          blocksize,
          cb,
        ...
        SNDArray.tpmqrt(
          ...
          R.slice(cb, (i, i + 1), (i + 1, null)),
          T,
          work1.slice(cb, Colon, (i + 1, null)),
          work1.slice(cb, Colon, (i, i + 1)),
          work3,
          blocksize,
          cb,
        )
  ...
  def whitenBlockSmallWindow(
    cb: EmitCodeBuilder,
    Q: SNDArrayValue,
    R: SNDArrayValue,
    A: SNDArrayValue,
    Qtemp: SNDArrayValue,
    Qtemp2: SNDArrayValue,
    Rtemp: SNDArrayValue,
    work1: SNDArrayValue,
    work2: SNDArrayValue,
    blocksize: Value[Long],
  ...
  def whitenBlock(cb: EmitCodeBuilder, _A: SNDArrayValue): Unit = {
    ...
    cb.if_(
      b > chunksize,
      cb._fatal("whitenBlock: A too large, found ", b.toS, ", expected ", chunksize.toS),
    )
    ...
    cb.if_(
      curSize < w, {
        ...
        cb.if_(
          curSize.cne(w),
          cb._fatal("whitenBlock: initial blocks didn't evenly divide window size"),
        )

        val bb = SizeValueDyn(cb.memoize(b * 2))
        whitenBlockLargeWindow(
          cb,
          Q,
          R,
          pivot,
