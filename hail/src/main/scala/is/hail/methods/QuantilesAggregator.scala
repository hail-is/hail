...
//  private val comp = ord.lt _
  ...
  inputBuffer(bufSize2) = Long.MaxValue
  ...
  def getBuffer(): Array[Long] = {
    if (bufferPool.isEmpty) {
      val newBuf = Array.ofDim[Long](bufSize + 1)
      newBuf(bufSize) = Long.MaxValue
      newBuf
    } else
      bufferPool.pop()
  }
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val agg = new QuantilesAggregator(0.5, 20, TInt64(), rand)
    val agg2 = new QuantilesAggregator(0.5, 20, TInt64(), rand)
    val n: Int = 1111
...
//    val data = Array.range(0, n)
    val time1 = time {
      var i = 0
      while (i < n/3) { agg.seqOp(data(i)); i += 1 }
      while (i < n) { agg2.seqOp(data(i)); i += 1 }
