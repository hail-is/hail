...
class RowMatrixSuite extends HailSuite {
  private def rowArrayToRowMatrix(a: Array[Array[Double]], nPartitions: Int = sc.defaultParallelism): RowMatrix = {
    ...
    RowMatrix(sc.parallelize(a.zipWithIndex.map { case (row, i) => (i.toLong, row) }, nPartitions), nCols, nRows)
  }
  ...
  def localizeRowMatrix() {
    ...
    BlockMatrix.fromBreezeMatrix(sc, localMatrix).write(ctx, fname)
  ...
  def readBlockSmall() {
    ...
    BlockMatrix.fromBreezeMatrix(sc, localMatrix).write(ctx, fname, forceRowMajor = true)
