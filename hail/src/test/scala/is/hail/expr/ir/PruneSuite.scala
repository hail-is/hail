...
class PruneSuite extends HailSuite {
  ...
  @Test def testWeird(): Unit = {
    val irString =
      """
        |(GetField rows
        |  (TableCollect
        |    (TableOrderBy (Anew_key Acol_idx)
        |      (MatrixEntriesTable
        |        (MatrixUnionCols inner
        |          (MatrixMapEntries
        |            (MatrixKeyRowsBy (new_key) False
        |              (MatrixMapRows
        |                (MatrixKeyRowsBy () False
        |                  (MatrixRead None False False "{\"nRows\":7,\"nCols\":3,\"nPartitions\":4,\"name\":\"MatrixRangeReader\"}"))
        |                (Let __cse_1
        |                  (GetField row_idx
        |                    (Ref va))
        |                  (InsertFields
        |                    (SelectFields (row_idx)
        |                      (Ref va))
        |                    None
        |                    (new_key
        |                      (If
        |                        (ApplySpecial lor () Boolean
    ...
    val ir = IRParser.parse_value_ir(this.ctx, irString)
    val pruned = PruneDeadFields(ir)
  }
