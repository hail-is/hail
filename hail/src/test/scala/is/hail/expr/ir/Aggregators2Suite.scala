...
class Aggregators2Suite extends HailSuite {
  ...
  @Test def testExtract() {
    val elt = TStruct("a"->TInt32())
    val array = Ref("array", TArray(elt))
    val pnn = AggSignature(PrevNonnull(), FastSeq(), None, FastSeq(elt))
    val arrayagg = AggSignature(AggElements2(FastIndexedSeq(pnn)), FastSeq[Type](), None, FastSeq(TInt32(), TVoid))

    val extracted = agg.Extract(AggArrayPerElement(array, "elt", "idx", ApplyAggOp(FastSeq(), None, FastSeq(Ref("elt", elt)), pnn), None, isScan = true), "RES")

    println(extracted.aggs.mkString("\n"))
