...
class PBaseStructSuite extends HailSuite {
  @Test def testStructCopy() {
    def runTests(deepCopy: Boolean, interpret: Boolean = false) {
      PhysicalTestUtils.copyTestExecutor(PCanonicalStruct(), PCanonicalStruct(), Annotation(),
        deepCopy = deepCopy, interpret = interpret)
      PhysicalTestUtils.copyTestExecutor(PCanonicalStruct("a" -> PInt64()), PCanonicalStruct("a" -> PInt64()), Annotation(12L),
        deepCopy = deepCopy, interpret = interpret)
      PhysicalTestUtils.copyTestExecutor(PCanonicalStruct("a" -> PInt64()), PCanonicalStruct("a" -> PInt64()), Annotation(null),
        deepCopy = deepCopy, interpret = interpret)
      PhysicalTestUtils.copyTestExecutor(PCanonicalStruct("a" -> PInt64(true)), PCanonicalStruct("a" -> PInt64(true)), Annotation(11L),
        deepCopy = deepCopy, interpret = interpret)

      PhysicalTestUtils.copyTestExecutor(PCanonicalStruct("a" -> PInt64(false)), PCanonicalStruct("a" -> PInt64(true)), Annotation(3L),
  ...
  @Test def tupleCopyTests() {
    def runTests(deepCopy: Boolean, interpret: Boolean = false) {
      PhysicalTestUtils.copyTestExecutor(PCanonicalTuple(false, PCanonicalString(true), PCanonicalString(true)), PCanonicalTuple(false, PCanonicalString(), PCanonicalString()), Annotation("1", "2"),
        deepCopy = deepCopy, interpret = interpret)
    }
