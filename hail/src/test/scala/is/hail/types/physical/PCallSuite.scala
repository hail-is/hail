...
class PCallSuite extends PhysicalTestUtils {
  @Test def copyTests() {
    def runTests(deepCopy: Boolean, interpret: Boolean = false) {
      copyTestExecutor(PCanonicalCall(), PCanonicalCall(),
        2,
        deepCopy = deepCopy, interpret = interpret)

      // downcast at top level allowed, since PCanonicalCall wraps a primitive
      copyTestExecutor(PCanonicalCall(), PCanonicalCall(true),
        2,
        deepCopy = deepCopy, interpret = interpret)

      copyTestExecutor(PCanonicalArray(PCanonicalCall(true), true), PCanonicalArray(PCanonicalCall()),
