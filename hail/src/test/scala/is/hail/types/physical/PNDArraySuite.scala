...
class PNDArraySuite extends PhysicalTestUtils {
  ...
  @Test def testRefCounted(): Unit = {
    ...
    region1.clear()
    assert(region1.memory.listNDArrayRefs().isEmpty)
    ...
    assert(region3.memory.listNDArrayRefs().isEmpty)
    ...
    region2.clear()
    assert(region2.memory.listNDArrayRefs().isEmpty)
  }
...
//    assert(PNDArray.getReferenceCount(addr4) == 1L)
    ...
    val annotationNDOfStructs =
      SafeNDArray(IndexedSeq(5L), (0 until 5).map(idx => Row(idx, idx + 100)))
