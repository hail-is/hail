  ...
  @Test def testWhitenBase(): Unit = {
    ...
    try {
      fb.emitWithBuilder { cb =>
        ...
        new LocalWhitening(cb, m, w, n, blocksize, region).whitenBase(cb, Q.slice(cb, ::, (null, w)), Q.slice(cb, ::, (w, null)), Qout, R, W, work1, work2)
  ...
  @Test def testWhitenNonrecur(): Unit = {
    ...
    try {
      fb.emitWithBuilder { cb =>
        ...
        state.whitenNonrecur(cb, state.Q, state.R, A, state.Qtemp, state.Qtemp2, state.Rtemp, state.work1, state.work2)
