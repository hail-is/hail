    ...
    def pytest_collection_modifyitems(self, items):
        for item in items:
            if (marker := item.get_closest_marker('skip')) is not None:
                print(f"Skipping {item.nodeid} because it was marked with reason: {marker.kwargs.get('reason')}.")
                continue
            ...
            self.items.append((item.nodeid, 5))
...
def run_submit(args: Namespace) -> None:
    ...
    all_benchmarks = [
        (nodeid, instance_id)
        for nodeid, num_instances in list_benchmarks(args.include, args.exclude)
        for instance_id in range(args.jobs or num_instances)
    ]
