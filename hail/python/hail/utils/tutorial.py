...
from hailtop.utils import retry_transient_errors, async_to_blocking
import os
import zipfile
...
def get_1kg(output_dir, overwrite: bool = False):
    ...
    if (overwrite
        ...
        info(f'downloading 1KG VCF ...\n'
             f'  Source: {source}')
        async_to_blocking(retry_transient_errors(urlretrieve, resources['1kg_matrix_table'], tmp_vcf))
        ...
        info(f'downloading 1KG annotations ...\n'
             f'  Source: {source}')
        async_to_blocking(retry_transient_errors(urlretrieve, source, tmp_sample_annot))
        ...
        info(f'downloading Ensembl gene annotations ...\n'
             f'  Source: {source}')
        async_to_blocking(retry_transient_errors(urlretrieve, source, tmp_gene_annot))
...
def get_movie_lens(output_dir, overwrite: bool = False):
    ...
    if overwrite or any(not Env.jutils().dirExists(jhc, f) for f in paths):
        ...
        info(f'downloading MovieLens-100k data ...\n'
             f'  Source: {source}')
        async_to_blocking(retry_transient_errors(urlretrieve, source, tmp_path))
