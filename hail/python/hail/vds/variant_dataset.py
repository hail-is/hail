...
class VariantDataset:
    ...
    def validate(self, *, check_data: bool = True):
        ...
        if (
            ...
            __validate_err(f"expect reference data to have a single row key 'locus' of type locus, found {rd_row_key}")
        ...
        if not isinstance(rd_col_key, hl.tstruct) or len(rd_row_key) != 1 or rd_col_key.types[0] != hl.tstr:
            __validate_err(f"expect reference data to have a single col key of type string, found {rd_col_key}")

        vd_col_key = vd.col_key.dtype
        if not isinstance(vd_col_key, hl.tstruct) or len(vd_col_key) != 1 or vd_col_key.types[0] != hl.tstr:
            __validate_err(f"expect variant data to have a single col key of type string, found {vd_col_key}")

        end_exists = 'END' in rd.entry
        len_exists = 'LEN' in rd.entry
        if not (end_exists or len_exists):
            __validate_err("expect at least one of 'END' or 'LEN' in entry of reference data")
        if end_exists and rd.END.dtype != hl.tint32:
            __validate_err("'END' field in entry of reference data must have type tint32")
        if len_exists and rd.LEN.dtype != hl.tint32:
            __validate_err("'LEN' field in entry of reference data must have type tint32")
    ...
    def __validate_filters_fields(self):
        ...
        if ref_has and not var_has:
            __validate_err(f"reference data has '{field}' when variant data does not")
        if var_has and not ref_has:
            __validate_err(f"variant data has '{field}' when reference data does not")
