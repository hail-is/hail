...
@typecheck(rmt=MatrixTable, gq_bins=sequenceof(int), dp_bins=sequenceof(int), dp_field=nullable(str))
def ref_sample_qc(
    rmt: 'MatrixTable',
    *,
    gq_bins: 'Sequence[int]' = (0, 20, 60),
    ...
    """"""
    require_first_key_field_locus(rmt, 'ref_sample_qc')

    if 'DP' in rmt.entry:
        dp_field_to_use = 'DP'
    elif 'MIN_DP' in rmt.entry:
    ...
    if dp_field_to_use is not None:
        ref_dp_expr['ref_bases_over_dp_threshold'] = hl.tuple(
            hl.agg.filter(rmt[dp_field_to_use] >= x, hl.agg.sum(1 + rmt.END - rmt.locus.position)) for x in dp_bins
        )
    return hl.struct(
        bases_over_gq_threshold=hl.tuple(
            hl.agg.filter(rmt.GQ >= x, hl.agg.sum(1 + rmt.END - rmt.locus.position)) for x in gq_bins
...
@typecheck(global_gt=Expression, alleles=Expression)
