...
will generate different values:
    ...
    >>> hl.eval(hl.array([x, x, x]))  # doctest: +SKIP
...
calls to :func:`.rand_unif` should be made:
    ...
    >>> b = hl.rand_unif(0, 1)
    >>> c = hl.rand_unif(0, 1)
    >>> hl.eval(hl.array([a, b, c]))  # doctest: +SKIP
...
return the same result, e.g.
    ...
    >>> hl.eval(hl.rand_unif(0, 1, seed=0))  # doctest: +SKIP
    0.5488135008937808
