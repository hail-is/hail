...
class LinearMixedModel(RegressionModel):
    ...
    **Testing alternative models**
    ...
    def regress_rows(self,
        ...
        **Efficiently varying the response or set of fixed effects**
        ...
        :meth:`.LinearRegressionModel.regress_rows`.
        ...
        >>> model = hl.LinearMixedModel(p @ y, p @ x, s)      # doctest: +SKIP
        ...
        """
        mt = matrix_table_source('LinearMixedModel.regress_rows', entry_expr)
        n = mt.count_cols()

        check_entry_indexed('LinearMixedModel.regress_rows', entry_expr)
        if not self._fitted:
            raise ValueError("LinearMixedModel.regress_rows: 'model' has not been fit "
                             "using 'fit()'")
        if self.p_path is None:
            raise ValueError("LinearMixedModel.regress_rows: 'model' property 'p_path' "
                             "was not set at initialization")

        if self.n != n:
            raise ValueError(f"LinearMixedModel.regress_rows: linear mixed model expects {model.n} samples, "
        ...
        p = BlockMatrix.read(self.p_path)
        ...
        ht = self.fit_alternatives(pa_t_path,
                                    a_t_path if self.low_rank else None,
