...
def ld_prune(call_expr, r2=0.2, bp_window_size=1000000, memory_per_core=256, keep_higher_maf=True, block_size=None):
    ...
    if keep_higher_maf:
        # entries = entries.annotate(imean = means[hl.int32(entries.i)][0],
        #                            jmean = means[hl.int32(entries.j)][0])
        entries.describe()
    ...
    return locally_pruned_table.filter(
        locally_pruned_table.variants_to_remove.contains(locally_pruned_table.idx),
        keep=False
    ).select().persist()
