...
def concordance(left, right) -> Tuple[List[List[int]], Table, Table]:
    ...
    **Using the table results**
    ...
    j_global_conc = r._1()
    col_conc = Table._from_java(r._2(), 'concordance')
    row_conc = Table._from_java(r._3(), 'concordance')
...
def vep(dataset: Union[Table, MatrixTable], config, block_size=1000, name='vep', csq=False):
    ...
    **Annotations**
    ...
    annotations = Table._from_java(Env.hail().methods.VEP.apply(ht._jt, config, csq, block_size), 'vep')
...
def nirvana(dataset: Union[MatrixTable, Table], config, block_size=500000, name='nirvana'):
    ...
    **Annotations**
    ...
    annotations = Table._from_java(Env.hail().methods.Nirvana.apply(ht._jt, config, block_size), 'nirvana')
