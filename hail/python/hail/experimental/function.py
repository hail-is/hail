...
@typecheck(f=anytype, param_types=hail_type, _name=nullable(str), type_args = tuple(hail_type))
def define_function(f, *param_types, _name=None, type_args=()):
    ...
    Env.hail().expr.ir.functions.IRFunctionRegistry.pyRegisterIR(
        mname, param_names, [ta._parsable_string() for ta in type_args], [pt._parsable_string() for pt in param_types], ret_type._parsable_string(),
        jbody)
