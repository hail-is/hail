...
def calculate_ld_scores(entry_expr,
    ...
    Given genotype data for a reference panel of samples and,
    optionally, variant annotation data, :func:`.calculate_ld_scores`
    uses the genotype data to calculate LD scores for each
    variant in the reference panel, across all annotations.
    ...
    **Further reading**
    ...
    ...     annotation_exprs=[mt.binary_annotation, mt.continuous_annotation])
    ...
    all ``annotation_exprs`` (if specified) must originate from the
    same matrix table.
    ...
    by the field ``locus`` of type :py:data:`.tlocus` and will contain
    one additional row field ``ld_scores`` of type :py:data:`.tarray`,
    where each element is of type :py:data:`.tfloat`.

    The length of this array will be equal to ``len(annotation_exprs) + 1``.
    The first element of the array will be the univariate LD score for
    that variant, and the remaining elements will be LD scores for the
    different annotations in ``annotation_exprs``, where the element at
    index ``i + 1`` of ``ld_scores`` will be the LD score for the
    annotation at index ``i`` of ``annotation_exprs``.
    ...
    annotation_exprs : :class:`.NumericExpression` or
        ...
        provided to this argument. The univariate annotation (LD scores across
        all variants) will be included by default and does not need to be
        specified here.
    ...
    -------
    :class:`.Table`
        Table keyed by ``locus_expr`` with a row field ``ld_scores`` that is an
        array of containing the univariate LD score for that variant and LD`
    ...
    ht_l2_tmp = new_temp_file()
    ht_l2.write(ht_l2_tmp)

    return hl.read_table(ht_l2_tmp)
