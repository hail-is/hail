...
def dict(collection) -> DictExpression:
    ...
    >>> hl.eval(hl.dict([('foo', 1), ('bar', 2), ('baz', 3)]))  # doctest: +SKIP
...
def json(x) -> StringExpression:
    ...
    >>> hl.eval(hl.json(hl.struct(a='Hello', b=0.12345, c=[1,2], d={'hi', 'bye'})))  # doctest: +SKIP
...
def rand_bool(p, seed=None) -> BooleanExpression:
    ...
    >>> hl.eval(hl.rand_bool(0.5))  # doctest: +SKIP
    True
...
def rand_norm(mean=0, sd=1, seed=None) -> Float64Expression:
    ...
    >>> hl.eval(hl.rand_norm())  # doctest: +SKIP
    1.5388475315213386
...
def rand_norm2d(mean=None, cov=None, seed=None) -> ArrayNumericExpression:
    ...
    >>> hl.eval(hl.rand_norm2d())  # doctest: +SKIP
...
def rand_pois(lamb, seed=None) -> Float64Expression:
    ...
    >>> hl.eval(hl.rand_pois(1))  # doctest: +SKIP
    2.0
...
def rand_unif(lower, upper, seed=None) -> Float64Expression:
    ...
    >>> hl.eval(hl.rand_unif(0, 1))  # doctest: +SKIP
    0.7983073825816226
...
def rand_beta(a, b, lower=None, upper=None, seed=None) -> Float64Expression:
    ...
    >>> hl.eval(hl.rand_beta(0, 1))  # doctest: +SKIP
    0.6696807666871818
...
def rand_gamma(shape, scale, seed=None) -> Float64Expression:
    ...
    >>> hl.eval(hl.rand_gamma(1, 1))  # doctest: +SKIP
    2.3915947710237537
...
def rand_cat(prob, seed=None) -> Int32Expression:
    ...
    >>> hl.eval(hl.rand_cat([0, 1.7, 2]))  # doctest: +SKIP
    2
...
def rand_dirichlet(a, seed=None) -> ArrayExpression:
    ...
    >>> hl.eval(hl.rand_dirichlet([1, 1, 1]))  # doctest: +SKIP
...
def corr(x, y) -> Float64Expression:
    ...
    Examples
    --------
    >>> hl.eval(hl.corr([1, 2, 4], [2, 3, 1]))  # doctest: +SKIP
    -0.6546536707079772
...
def group_by(f: Callable, collection) -> DictExpression:
    ...
    >>> hl.eval(hl.group_by(lambda x: hl.len(x), a))  # doctest: +SKIP
...
def float64(x) -> Float64Expression:
    ...
    >>> hl.eval(hl.float64('1.1'))  # doctest: +SKIP
    1.1

    >>> hl.eval(hl.float64(1))  # doctest: +SKIP
    1.0

    >>> hl.eval(hl.float64(True))  # doctest: +SKIP
    1.0
...
def float32(x) -> Float32Expression:
    ...
    >>> hl.eval(hl.float32('1.1'))  # doctest: +SKIP
    1.1

    >>> hl.eval(hl.float32(1))  # doctest: +SKIP
    1.0

    >>> hl.eval(hl.float32(True))  # doctest: +SKIP
    1.0
