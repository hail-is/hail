...
class TypeConstructor(NodeVisitor):
    ...
    def visit_locus(self, node, visited_children):
        tlocus, _, angle_bracket, gr, _ = visited_children
        return types.tlocus(gr)

    def visit_array(self, node, visited_children):
        tarray, _, angle_bracket, t, _ = visited_children
    ...
    def visit_ndarray(self, node, visited_children):
        tndarray, _, angle_bracket, elem_t, comma, ndim, _ = visited_children
        return types.tndarray(elem_t, ndim)

    def visit_set(self, node, visited_children):
        tset, _, angle_bracket, t, _ = visited_children
        return types.tset(t)

    def visit_dict(self, node, visited_children):
        tdict, _, angle_bracket, kt, comma, vt, _ = visited_children
        return types.tdict(kt, vt)

    def visit_struct(self, node, visited_children):
        tstruct, _, brace, maybe_fields, _ = visited_children
        if not maybe_fields:
    ...
    def visit_union(self, node, visited_children):
        tunion, _, brace, maybe_fields, _ = visited_children
        if not maybe_fields:
            return types.tunion()
        else:
            fields = maybe_fields[0]
    ...
    def visit_tuple(self, node, visited_children):
        ttuple, _, paren, [maybe_types], _ = visited_children
        if not maybe_types:
            return types.ttuple()
        else:
    ...
    def visit_interval(self, node, visited_children):
        tinterval, _, angle_bracket, point_t, _ = visited_children
...
class VCFTypeConstructor(NodeVisitor):
    ...
    def visit_array(self, node, visited_children):
        tarray, _, bracket, t, _ = visited_children
        return types.tarray(t)
