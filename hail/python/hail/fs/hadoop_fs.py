...
from hailtop.fs.stat_result import FileListEntry, FileStatus, FileType
...
class HadoopFS(FS):
    ...
    def ls(self, path: str) -> List[FileListEntry]:
        return [_file_list_entry_scala_to_python(st) for st in json.loads(self._py4jutils.ls(self._jfs, path))]
