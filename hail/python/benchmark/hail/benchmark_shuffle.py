...
def test_shuffle_key_rows_by_mt(profile25_mt):
...
def test_shuffle_order_by_10m_int():
    t = hl.utils.range_table(10_000_000, n_partitions=100)
    t = t.order_by(-t.idx)
    t._force_count()


def test_shuffle_key_rows_by_4096_byte_rows():
...
def test_shuffle_key_rows_by_65k_byte_rows():
...
def test_shuffle_key_by_aggregate_bad_locality(many_ints_ht):
    ht = hl.read_table(str(many_ints_ht))
    ht.group_by(x=ht.i0 % 1000).aggregate(c=hl.agg.count(), m=hl.agg.mean(ht.i2))._force_count()


def test_shuffle_key_by_aggregate_good_locality(many_ints_ht):
    ht = hl.read_table(str(many_ints_ht))
