...
def test_table_group_by_aggregate_sorted():
    n = 10_000_000
    ht = hl.utils.range_table(n)
...
def test_table_group_by_aggregate_unsorted():
    n = 10_000_000
    ht = hl.utils.range_table(n)
...
def test_table_range_force_count():
...
def test_table_python_construction():
    n = 100
...
def test_table_big_aggregate_compilation():
    n = 1_000
...
def test_table_big_aggregate_compile_and_execute():
    n = 1_000
    m = 1_000_000
...
def test_table_foreign_key_join(m, n):
    ht = hl.utils.range_table(m)
    ht2 = hl.utils.range_table(n)
    ht.annotate(x=ht2[(m - 1 - ht.idx) % n])._force_count()


def test_table_aggregate_array_sum():
    n = 10_000_000
    m = 100
    ht = hl.utils.range_table(n)
...
def test_table_annotate_many_flat():
    n = 1_000_000
    m = 100
...
def test_table_annotate_many_nested_no_dependence():
    n = 1_000_000
    m = 100
    ht = hl.utils.range_table(n, 16)
    for i in range(m):
...
@pytest.mark.skip(reason='never finishes')
def test_table_annotate_many_nested_dependence_constants():
    n = 1_000_000
    m = 100
...
@pytest.mark.skip(reason='never finishes')
def test_table_annotate_many_nested_dependence():
    n = 1_000_000
    m = 100
    ht = hl.utils.range_table(n, 16)
...
def test_table_read_force_count_ints(many_ints_ht):
    ht = hl.read_table(str(many_ints_ht))
    ht._force_count()


def test_table_read_force_count_strings(many_strings_ht):
    ht = hl.read_table(str(many_strings_ht))
    ht._force_count()


def test_table_import_ints(many_ints_tsv):
    hl.import_table(
        str(many_ints_tsv),
...
def test_table_import_ints_impute(many_ints_tsv):
    hl.import_table(str(many_ints_tsv), impute=True)._force_count()


def test_table_import_strings(many_strings_tsv):
    hl.import_table(str(many_strings_tsv))._force_count()


def test_table_aggregate_int_stats(many_ints_ht):
    ht = hl.read_table(str(many_ints_ht))
    ht.aggregate(
        tuple([
...
def test_table_aggregate_downsample_sparse():
    ht = hl.utils.range_table(250_000_000, 8)
    ht.aggregate(hl.agg.downsample(hl.rand_norm() ** 5, hl.rand_norm() ** 5))


def test_table_aggregate_linreg(many_ints_ht):
    ht = hl.read_table(str(many_ints_ht))
    ht.aggregate(hl.agg.array_agg(lambda i: hl.agg.linreg(ht.i0 + i, [ht.i1, ht.i2, ht.i3, ht.i4]), hl.range(75)))


def test_table_take(many_strings_ht):
    ht = hl.read_table(str(many_strings_ht))
    ht.take(100)


def test_table_show(many_strings_ht):
    ht = hl.read_table(str(many_strings_ht))
    ht.show(100)


def test_table_expr_take(many_strings_ht):
    ht = hl.read_table(str(many_strings_ht))
    hl.tuple([ht.f1, ht.f2]).take(100)


def test_read_force_count_partitions(many_partitions_ht):
...
def test_write_range_table(tmp_path, n, n_partitions):
...
def test_read_with_index(many_partitions_ht):
    rows = 10_000_000
    bins = 1_000
    width = rows // bins
...
def test_union_partitions_table(many_partitions_ht1, many_partitions_ht2):
    ht1 = hl.read_table(str(many_partitions_ht1))
    ht2 = hl.read_table(str(many_partitions_ht2))
    ht1.union(ht2)._force_count()


def test_join_partitions_table(many_partitions_ht1, many_partitions_ht2):
    ht1 = hl.read_table(str(many_partitions_ht1))
    ht2 = hl.read_table(str(many_partitions_ht2))
    ht1.join(ht2)._force_count()


def test_group_by_collect_per_row(gnomad_dp_sim):
...
def test_test_map_filter_region_memory():
...
def test_test_head_and_tail_region_memory():
...
def test_test_inner_join_region_memory():
...
def test_test_left_join_region_memory():
    high_mem_table = hl.utils.range_table(30).naive_coalesce(1).annotate(big_array=hl.zeros(50_000_000))
    high_mem_table2 = hl.utils.range_table(30).naive_coalesce(1).annotate(big_array=hl.zeros(50_000_000))
