...
def test_matrix_table_decode_and_count(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt._force_count_rows()


def test_matrix_table_decode_and_count_just_gt(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt)).select_entries('GT')
    mt._force_count_rows()


def test_matrix_table_array_arithmetic(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt = mt.filter_rows(mt.alleles.length() == 2)
    mt.select_entries(dosage=hl.pl_dosage(mt.PL)).select_rows()._force_count_rows()


def test_matrix_table_entries_table(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt.entries()._force_count()


def test_matrix_table_entries_table_no_key(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt)).key_rows_by().key_cols_by()
    mt.entries()._force_count()


def test_matrix_table_rows_force_count(profile25_mt):
    ht = hl.read_matrix_table(str(profile25_mt)).rows().key_by()
    ht._force_count()


def test_matrix_table_show(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt.show(100)


def test_matrix_table_rows_show(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt.rows().show(100)


def test_matrix_table_cols_show(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt.cols().show(100)


def test_matrix_table_take_entry(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt.GT.take(100)


def test_matrix_table_entries_show(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt.entries().show()


def test_matrix_table_take_row(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt.info.AF.take(100)


def test_matrix_table_take_col(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt.s.take(100)


def test_write_range_matrix_table_p100(tmp_path):
...
def test_write_profile_mt(profile25_mt, tmp_path):
    hl.read_matrix_table(str(profile25_mt)).write(str(tmp_path / 'tmp.mt'))


def test_matrix_table_rows_is_transition(profile25_mt):
    ht = hl.read_matrix_table(str(profile25_mt)).rows().key_by()
    ht.select(is_snp=hl.is_snp(ht.alleles[0], ht.alleles[1]))._force_count()


def test_matrix_table_filter_entries(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt.filter_entries((mt.GQ > 8) & (mt.DP > 2))._force_count_rows()


def test_matrix_table_filter_entries_unfilter(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt.filter_entries((mt.GQ > 8) & (mt.DP > 2)).unfilter_entries()._force_count_rows()


@pytest.mark.xtimeout
def test_matrix_table_nested_annotate_rows_annotate_entries(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
...
def test_matrix_table_many_aggs_row_wise(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt = mt.annotate_rows(**many_aggs(mt))
    mt.rows()._force_count()


def test_matrix_table_many_aggs_col_wise(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt = mt.annotate_cols(**many_aggs(mt))
    mt.cols()._force_count()


def test_matrix_table_aggregate_entries(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt.aggregate_entries(hl.agg.stats(mt.GQ))


def test_matrix_table_call_stats_star_star(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    mt.annotate_rows(**hl.agg.call_stats(mt.GT, mt.alleles))._force_count_rows()


def test_gnomad_coverage_stats(gnomad_dp_sim):
...
def test_gnomad_coverage_stats_optimized(gnomad_dp_sim):
    mt = hl.read_matrix_table(str(gnomad_dp_sim))
    mt = mt.annotate_rows(
        mean=hl.agg.mean(mt.x),
...
def test_per_row_stats_star_star(gnomad_dp_sim):
    mt = hl.read_matrix_table(str(gnomad_dp_sim))
    mt.annotate_rows(**hl.agg.stats(mt.x))._force_count_rows()


def test_read_decode_gnomad_coverage(gnomad_dp_sim):
    hl.read_matrix_table(str(gnomad_dp_sim))._force_count_rows()


def test_import_bgen_force_count_just_gp(sim_ukb_bgen, sim_ukb_sample):
    mt = hl.import_bgen(str(sim_ukb_bgen), sample_file=str(sim_ukb_sample), entry_fields=['GP'], n_partitions=8)
    mt._force_count_rows()


def test_import_bgen_force_count_all(sim_ukb_bgen, sim_ukb_sample):
    mt = hl.import_bgen(
...
def test_import_bgen_info_score(sim_ukb_bgen, sim_ukb_sample):
    mt = hl.import_bgen(str(sim_ukb_bgen), sample_file=str(sim_ukb_sample), entry_fields=['GP'], n_partitions=8)
    mt = mt.annotate_rows(info_score=hl.agg.info_score(mt.GP))
    mt.rows().select('info_score')._force_count()


def test_import_bgen_filter_count(sim_ukb_bgen, sim_ukb_sample):
...
def test_export_range_matrix_table_entry_field_p100(tmp_path):
    mt = hl.utils.range_matrix_table(n_rows=1_000_000, n_cols=10, n_partitions=100)
    mt = mt.annotate_entries(x=mt.col_idx + mt.row_idx)
    mt.x.export(str(tmp_path / 'result.txt'))


def test_export_range_matrix_table_row_p100(tmp_path):
    mt = hl.utils.range_matrix_table(n_rows=1_000_000, n_cols=10, n_partitions=100)
    mt.row.export(str(tmp_path / 'result.txt'))


def test_export_range_matrix_table_col_p100(tmp_path):
    mt = hl.utils.range_matrix_table(n_rows=1_000_000, n_cols=10, n_partitions=100)
    mt.col.export(str(tmp_path / 'result.txt'))


@pytest.mark.xtimeout(250)
def test_large_range_matrix_table_sum():
...
def test_kyle_sex_specific_qc(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
...
@pytest.mark.xtimeout
def test_matrix_multi_write_nothing(tmp_path):
    mt = hl.utils.range_matrix_table(1, 1, n_partitions=1)
    mts = [mt] * 1000
    hl.experimental.write_matrix_tables(mts, str(tmp_path / 'multi-write'))


def test_mt_localize_and_collect(profile25_mt):
    mt = hl.read_matrix_table(str(profile25_mt))
    ht = mt.localize_entries("ent")
    ht.head(150).collect()


def test_mt_group_by_memory_usage(random_doubles_mt):
