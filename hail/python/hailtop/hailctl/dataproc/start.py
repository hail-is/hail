    ...
    if args.vep:
        ...
        if not (args.requester_pays_allow_all or ('hail-us-vep' in allowed_buckets_list)):
            raise RuntimeError("Need to enable requester pays on bucket 'hail-us-vep' to use vep. See --requester-pays-allow-all and --requester-pays-allow-buckets \
                WARNING: This is likely prohibitively expensive if you are outside the US. If you need to run VEP outside the US, please contact the Hail team on https://discuss.hail.is for support.")
        conf.extend_flag('initialization-actions', [deploy_metadata[f'vep-{args.vep}.sh']])
        sys.stderr.write("WARNING: Running VEP outside of the US is VERY expensive. If you need to run VEP outside the US, please contact the Hail team on https://discuss.hail.is for support.")
