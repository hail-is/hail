...
async def get_gcp_bucket_information(bucket: str, verbose: bool) -> dict:
    ...
    try:
        info, _ = await check_exec_output(
            'gcloud', 'storage', 'buckets', 'describe', f'gs://{bucket}', '--format="json"', echo=verbose
        )
        ...
        if 'does not have storage.buckets.get access to the Google Cloud Storage bucket' in e.stderr.decode('utf-8'):
            msg = (
                f'ERROR: You do not have sufficient permissions to get information about bucket {bucket} or it does not exist. '
...
async def create_gcp_bucket(*, project: str, bucket: str, location: str, verbose: bool):
    from hailtop.utils import CalledProcessError, check_exec_output  # pylint: disable=import-outside-toplevel
    ...
    try:
        ...
        if 'does not have storage.buckets.create access to the Google Cloud project' in e.stderr.decode('utf-8'):
            msg = (
                f'ERROR: You do not have the necessary permissions to create buckets in project {project}. Ask a project administrator '
            ...
            raise InsufficientPermissions(msg) from e
        if 'Your previous request to create the named bucket succeeded and you already own it' in e.stderr.decode(
            'utf-8'
        ):
...
async def update_gcp_bucket(
    *, project: str, bucket: str, lifecycle_days: Optional[int], labels: Optional[Dict[str, str]], verbose: bool
):
    from hailtop.utils import CalledProcessError, check_exec_output  # pylint: disable=import-outside-toplevel
    ...
    try:
        ...
        if 'does not have storage.buckets.get access to the Google Cloud Storage bucket' in e.stderr.decode('utf-8'):
            msg = (
                f'ERROR: You do not have the necessary permissions to update bucket {bucket} in project {project}. Ask a project administrator '
...
async def grant_service_account_bucket_access_with_role(bucket: str, service_account: str, role: str, verbose: bool):
    ...
    try:
        ...
        if 'does not have storage.buckets.getIamPolicy access to the Google Cloud Storage bucket' in e.stderr.decode(
            'utf-8'
        ):
            msg = (
                f'ERROR: You do not have the necessary permissions to set permissions for bucket {bucket}. Ask a project administrator '
