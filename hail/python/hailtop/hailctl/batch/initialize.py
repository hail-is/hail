...
async def setup_existing_remote_tmpdir(service_account: str, verbose: bool) -> Tuple[Optional[str], str, bool]:
    from hailtop.aiocloud.aiogoogle.client.storage_client import (  # pylint: disable=import-outside-toplevel
    ...
    if bucket_info['locationType'] != 'region':
        typer.secho(
            f'WARNING: remote temporary directory {remote_tmpdir} is multi-regional. Using this bucket with the Batch Service will incur additional network fees.',
    ...
    if storage_class.upper() != 'STANDARD':
        typer.secho(
            f'WARNING: remote temporary directory {remote_tmpdir} does not have storage class "STANDARD". Additional data recovery charges will occur when accessing data.',
            fg=typer.colors.YELLOW,
        )
        warnings = True

    give_access_to_remote_tmpdir = Confirm.ask(
        f'Do you want to give service account {service_account} read/write access to bucket {bucket}?'
    )
    if give_access_to_remote_tmpdir:
        try:
            await grant_service_account_bucket_access_with_role(
                bucket=bucket, service_account=service_account, role='roles/storage.objectAdmin', verbose=verbose
            )
        except InsufficientPermissions as e:
            typer.secho(e.message, fg=typer.colors.RED)
            raise Abort() from e
        typer.secho(
            f'Granted service account {service_account} read and write access to {bucket}.', fg=typer.colors.GREEN
        )
    else:
        typer.secho(
            f'WARNING: Please verify service account {service_account} has the role "roles/storage.objectAdmin" or '
...
async def setup_new_remote_tmpdir(
    *, supported_regions: List[str], username: str, service_account: str, verbose: bool
    ...
    if bucket_region not in supported_regions:
        typer.secho(
            ...
            f'Creating a bucket in {bucket_region} will incur additional network fees when using the Batch Service.',
            fg=typer.colors.YELLOW,
        )
    ...
    try:
        ...
        continue_w_update = Confirm.ask(
            f'Do you wish to continue updating the lifecycle rules and permissions on bucket {bucket_name}?'
        )
        if not continue_w_update:
            typer.secho(
                f'WARNING: The lifecycle rules and permissions on bucket {bucket_name} were not updated. '
                'You will have to manually configure these yourself.',
                fg=typer.colors.YELLOW,
            )
            warnings = True
    ...
    typer.secho(
        f'Updated bucket {bucket_name} in project {project} with lifecycle rule set to {lifecycle_days} days and labels {labels}.',
    ...
    typer.secho(
        f'Granted service account {service_account} read and write access to {bucket_name} in project {project}.',
        fg=typer.colors.GREEN,
    )
...
async def initialize_gcp(
    username: str, hail_identity: str, supported_regions: List[str], verbose: bool
) -> Tuple[Optional[str], str, bool]:
    ...
    if not gcloud_installed:
        typer.secho(
            'Have you installed gcloud? For directions see https://cloud.google.com/sdk/docs/install '
...
async def async_basic_initialize(verbose: bool = False):
    ...
    if not already_logged_in:
        typer.secho(
            'You are not currently logged in to Hail. Redirecting you to a login screen.', fg=typer.colors.YELLOW
        )
    ...
    async with batch_client:
        ...
        if cloud == 'gcp':
            remote_tmpdir, tmpdir_region, warnings = await initialize_gcp(
                username, hail_identity, supported_regions, verbose
            )
        else:
            remote_tmpdir = Prompt.ask(
                'Enter a path to an existing remote temporary directory (ex: https://myaccount.blob.core.windows.net/mycontainer/batch/tmp)'
            )
            typer.secho(
                f'WARNING: You will need to grant read/write access to {remote_tmpdir} for account {hail_identity}',
                fg=typer.colors.YELLOW,
            )
            warnings = False
    ...
    if tmpdir_region != compute_region and not compute_region.startswith(tmpdir_region):
        typer.secho(
            f'WARNING: remote temporary directory "{remote_tmpdir}" is not located in the selected compute region for Batch jobs "{compute_region}". Found {tmpdir_region}.',
            fg=typer.colors.YELLOW,
        )
        warnings = True
    ...
    if warnings:
        typer.secho(
            'WARNING: The currently specified configuration will result in additional fees when using Hail Batch.',
            fg=typer.colors.YELLOW,
        )
        raise Exit()
