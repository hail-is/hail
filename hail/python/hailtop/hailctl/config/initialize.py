...
async def configure_gcp_requester_pays():
    from hailtop.config import get_user_config  # pylint: disable=import-outside-toplevel
    from ..config.cli import set as set_config, list as list_config  # pylint: disable=import-outside-toplevel

    default_gcp_project = await get_gcp_default_project()
    ...
    if selected_buckets:
        set_config('gcs_requester_pays/buckets', ','.join(selected_buckets))

    get_user_config(reload=True)
    list_config()
    print('Finished configuring GCP requester pays.')
...
async def create_docker_repository():
    ...
    from ..config.cli import get as get_config, list as list_config, set as set_config,  # pylint: disable=import-outside-toplevel

    already_logged_in = await already_logged_into_service()
    if not already_logged_in:
        await async_login(None)
    ...
    if create_repo:
        ...
        if location != default_multi_region_location and location != default_region:
            print(f'WARNING: The selected location {location} is not located within the same region as Batch jobs configured to run in {default_region}.')
            errors = True
...
''')
            ...
            print(f'ERROR: Could not create repository {repo_name} in the artifact registry for project {project}.')
            print('WARNING: Skipping repository creation.')
    ...
    grant_access = Prompt.ask(f'Do you want to give {hail_identity} access to the repository {repo_name} in project {project}?', default=True)
...
async def basic_initialize(incremental: bool = False):
    from hailtop.aiocloud.aiogoogle import GoogleStorageAsyncFS  # pylint: disable=import-outside-toplevel
    from hailtop.auth.auth import async_get_userinfo  # pylint: disable=import-outside-toplevel
    from hailtop.config import DeployConfig  # pylint: disable=import-outside-toplevel
    from hailtop.config import get_user_config, get_user_config_path  # pylint: disable=import-outside-toplevel
                ...
                async with GoogleStorageClient() as storage_client:
                    try:
                        ...
                        print(f'ERROR: Could not create bucket {maybe_bucket_name}. '
                              ...
                              f'Are you logged into your account by running `gcloud auth login`?')
                        remote_tmpdir = None
                        errors = True
                    else:
                        try:
...
async def async_initialize(basic: bool,
                           gcp_requester_pays: bool,
                           gcp_artifact_registry: bool,
                           incremental: bool):
    if basic:
        await basic_initialize(incremental=incremental)
    elif gcp_requester_pays:
        await configure_gcp_requester_pays()
    elif gcp_artifact_registry:
        await create_docker_repository()
