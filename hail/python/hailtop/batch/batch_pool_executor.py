...
class BatchPoolExecutor:
    ...
    def __init__(self, *,
        ...
        self.gcs_fs = GoogleStorageAsyncFS(project=project)
    ...
    async def async_submit(self,
        ...
        async with await self.gcs_fs.create(pickledfun_gcs) as f:
            await f.write(pipe.getbuffer().tobytes())
...
"''')
    ...
    def _cleanup(self):
        async def _delete_files(url):
            async with self.sem:
                await self.gcs_fs.rmtree(self.sem, url)

        if self.cleanup_bucket:
            async_to_blocking(_delete_files(self.directory))
        self.backend.close()
...
class BatchPoolFuture:
    ...
    async def _async_fetch_result(self):
        try:
            ...
            async with await self.executor.gcs_fs.open(self.output_gcs) as f:
                data = await f.read()
            value, traceback = dill.loads(data)
