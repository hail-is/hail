...
async def test_diff(diff_test_context):
    ...
    expected = {
        frozendict({
            "from": f"{src_base}diff",
            "to": f"{dest_base}diff",
            "from_size": 3,
            "to_size": 1,
        }),
        frozendict({
            "from": f"{src_base}src-only",
            "to": f"{dest_base}src-only",
            "from_size": 3,
            "to_size": None,
        }),
        frozendict({
            "from": f"{src_base}a/diff",
            "to": f"{dest_base}a/diff",
            "from_size": 3,
            "to_size": 1,
        }),
        frozendict({
            "from": f"{src_base}a/src-only",
            "to": f"{dest_base}a/src-only",
            "from_size": 3,
            "to_size": None,
        }),
        frozendict({
            "from": f"{src_base}b/diff",
            "to": f"{dest_base}b/diff",
            "from_size": 3,
            "to_size": 1,
        }),
        frozendict({
            "from": f"{src_base}b/src-only",
            "to": f"{dest_base}b/src-only",
            "from_size": 3,
            "to_size": None,
        }),
    }
