...
class Test(unittest.TestCase):
    ...
    def test_with_default_namespace_external_default_to_bar(self):
        deploy_config = DeployConfig('external', 'default', 'organization.tld', None)
        deploy_config = deploy_config.with_default_namespace('bar')
    ...
    def test_with_default_namespace_external_bar_to_baz(self):
        deploy_config = DeployConfig('external', 'bar', 'internal.organization.tld', '/bar')
        deploy_config = deploy_config.with_default_namespace('baz')

        self.assertEqual(deploy_config.default_namespace(), 'baz')
        self.assertEqual(deploy_config.auth_session_cookie_name(), 'sesh')
        self.assertEqual(deploy_config.base_path('foo'), '/baz/foo')
        self.assertEqual(deploy_config.base_url('foo'), 'https://internal.organization.tld/baz/foo')
        self.assertEqual(deploy_config.url('foo', '/moo'), 'https://internal.organization.tld/baz/foo/moo')

    def test_with_default_namespace_external_bar_to_default(self):
        deploy_config = DeployConfig('external', 'bar', 'internal.organization.tld', '/bar')
        deploy_config = deploy_config.with_default_namespace('default')

        self.assertEqual(deploy_config.default_namespace(), 'default')
        self.assertEqual(deploy_config.auth_session_cookie_name(), 'session')
        self.assertEqual(deploy_config.base_path('foo'), '')
        self.assertEqual(deploy_config.base_url('foo'), 'https://foo.organization.tld')
        self.assertEqual(deploy_config.url('foo', '/moo'), 'https://foo.organization.tld/moo')

    def test_with_default_namespace_k8s_default_to_bar(self):
        deploy_config = DeployConfig('k8s', 'default', 'organization.tld', None)
        deploy_config = deploy_config.with_default_namespace('bar')

        self.assertEqual(deploy_config.default_namespace(), 'bar')
        self.assertEqual(deploy_config.auth_session_cookie_name(), 'sesh')
        self.assertEqual(deploy_config.domain('foo'), 'foo.bar')
        self.assertEqual(deploy_config.base_path('foo'), '/bar/foo')
        self.assertEqual(deploy_config.base_url('foo'), 'https://foo.bar/bar/foo')
        self.assertEqual(deploy_config.url('foo', '/moo'), 'https://foo.bar/bar/foo/moo')
