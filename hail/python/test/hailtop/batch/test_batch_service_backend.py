...
def test_single_task_write_resource_group(backend: ServiceBackend, output_tmpdir: str):
    ...
    b.write_output(
        j.output.foo,
        os.path.join(output_tmpdir, "test_single_task_write_resource_group_file.txt"),
    )
    res = b.run()
    assert res
    res_status = res.status()
...
def test_query_on_batch_in_batch(backend: ServiceBackend, output_tmpdir: str):
    ...
    j.env(
        "HAIL_BATCH_BILLING_PROJECT",
        configuration_of(ConfigVariable.BATCH_BILLING_PROJECT, None, ""),
    )
...
def test_validate_cloud_storage_policy(backend, monkeypatch):
    ...
    _test_raises(
        ClientResponseError,
        no_perms_error,
        lambda: ServiceBackend(remote_tmpdir=no_perms_uri),
    )
