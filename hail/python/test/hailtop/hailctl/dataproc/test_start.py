...
def test_cluster_location(runner, gcloud_run, location_arg):
    runner.invoke(cli.app, ['start', location_arg, 'test-cluster'])
    assert location_arg in gcloud_run.call_args[0][0]


def test_creator_label(runner, gcloud_run, gcloud_config):
...
def test_workers_configuration(runner, gcloud_run):
...
def test_secondary_workers_configuration(runner, gcloud_run, workers_arg):
...
def test_machine_type_configuration(runner, gcloud_run, machine_arg):
...
def test_boot_disk_size_configuration(runner, gcloud_run, machine_arg):
    runner.invoke(cli.app, ['start', machine_arg, 'test-cluster'])
    assert f"{machine_arg}GB" in gcloud_run.call_args[0][0]


def test_vep_defaults_to_highmem_master_machine(runner, gcloud_run):
    runner.invoke(cli.app, ['start', 'test-cluster', '--vep=GRCh37'])
    assert "--master-machine-type=n1-highmem-8" in gcloud_run.call_args[0][0]


def test_vep_defaults_to_larger_worker_boot_disk(runner, gcloud_run):
    runner.invoke(cli.app, ['start', 'test-cluster', '--vep=GRCh37'])
...
def test_requester_pays_project_configuration(runner, gcloud_run, gcloud_config, requester_pays_arg):
...
def test_requester_pays_mode_configuration(runner, gcloud_run, requester_pays_arg, expected_mode):
    runner.invoke(cli.app, ['start', 'test-cluster', requester_pays_arg])
    properties = next(arg for arg in gcloud_run.call_args[0][0] if arg.startswith("--properties="))
    assert f"spark:spark.hadoop.fs.gs.requester.pays.mode={expected_mode}" in properties


def test_requester_pays_buckets_configuration(runner, gcloud_run):
...
def test_scheduled_deletion_configuration(runner, gcloud_run, scheduled_deletion_arg):
    runner.invoke(cli.app, ['start', scheduled_deletion_arg, 'test-cluster'])
    assert scheduled_deletion_arg in gcloud_run.call_args[0][0]


def test_master_tags(runner, gcloud_run):
...
def test_master_tags_project(runner, gcloud_run):
...
def test_master_tags_zone(runner, gcloud_run):
...
def test_master_tags_dry_run(runner, gcloud_run):
