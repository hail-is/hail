...
class ArtifactManifestCliRunner(CliRunner):
    def invoke(self, cli, args=None, **kwargs):
        args = list(args or [])
        if args and args[0] == 'start':
            if '--artifact-manifest' not in args:
                args.extend(['--artifact-manifest', 'test-artifact-manifest.json'])
...
runner = ArtifactManifestCliRunner()
...
def test_start_calls_run_job_flow_with_hail_config(emr_client_mock, upload_mock):
    ...
    assert ba['Path'].startswith('s3://artifact-bucket/bootstrap/install-hail-emr-')
    assert ba['Path'].endswith('.sh')
    assert upload_mock.call_args.args[0] == ba['Path']
    assert ba['Args'] == [
    ...
    emr_client_mock.describe_release_label.assert_called_once_with(ReleaseLabel='emr-spark-8.1.0')
...
def test_start_json_overlay_custom_roles_skip_default_role_preflight(
    emr_client_mock, check_default_roles_mock, check_custom_roles_mock
):
    emr_client_mock.run_job_flow.return_value = {'JobFlowId': 'j-1'}
    res = runner.invoke(
        cli.app,
    ...
    assert check_default_roles_mock.call_count == 0
    check_custom_roles_mock.assert_called_once_with('custom-service', 'custom-profile')
    kwargs = emr_client_mock.run_job_flow.call_args.kwargs
...
def test_start_overlay_must_retain_one_private_subnet(emr_client_mock):
    res = runner.invoke(
        cli.app,
        [
            'start',
            ...
            '--dry-run',
            '--run-job-flow-json',
            '{"Instances":{"Ec2SubnetId":null}}',
        ],
    )
    assert res.exit_code != 0
    assert 'must contain one Ec2SubnetId' in res.output
    assert emr_client_mock.run_job_flow.call_count == 0


def test_start_rejects_invalid_idle_timeout(emr_client_mock):
    res = runner.invoke(
        cli.app,
        ['start', 'c1', '--s3-scratch', 's3://bkt/tmp/', '--idle-timeout', '30', '--dry-run'],
    )
    assert res.exit_code != 0
    assert '--idle-timeout must be between 60 and 604800' in res.output
    assert emr_client_mock.run_job_flow.call_count == 0


def test_start_overlay_must_retain_auto_termination(emr_client_mock):
    res = runner.invoke(
        cli.app,
        [
            'start',
    ...
    assert res.exit_code != 0
    assert 'must contain an' in res.output
    assert 'AutoTerminationPolicy IdleTimeout' in res.output
    assert emr_client_mock.run_job_flow.call_count == 0


def test_start_overlay_must_retain_verified_bootstrap(emr_client_mock):
    res = runner.invoke(
        cli.app,
        [
            'start',
    ...
    assert res.exit_code != 0
    assert 'must retain the' in res.output
    assert 'checksum-verified install-hail bootstrap' in res.output
    assert emr_client_mock.run_job_flow.call_count == 0
