...
class Tests(unittest.TestCase):
    ...
    def test_struct_ops(self):
        ...
        self.assertEqual(
            s.annotate(**{"a": 5, "x": 10, "y": 15}),
            hl.Struct(a=5, b=2, c=3, x=10, y=15),
        )
    ...
    def test_json_encoder(self):
        self.assertEqual(
            json.dumps(hl.frozendict({"foo": "bar"}), cls=hl.utils.JSONEncoder),
            '{"foo": "bar"}',
        )
...
def test_hadoop_ls_one_folder_glob_and_qmarks_in_multiple_components(
