...
class Tests(unittest.TestCase):
    ...
    def test_hadoop_methods(self):
        ...
        with hadoop_open(f'{Tests.BUCKET}/test_out.txt', 'w') as f:
            for d in data:
                f.write(d)
                f.write('\n')

        with hadoop_open(f'{Tests.BUCKET}/test_out.txt') as f:
            data2 = [line.strip() for line in f]

        self.assertEqual(data, data2)

        with hadoop_open(f'{Tests.BUCKET}/test_out.txt.gz', 'w') as f:
            for d in data:
                f.write(d)
                f.write('\n')

        with hadoop_open(f'{Tests.BUCKET}/test_out.txt.gz') as f:
            data3 = [line.strip() for line in f]

        self.assertEqual(data, data3)

        hadoop_copy(f'{Tests.BUCKET}/test_out.txt.gz',
                    f'{Tests.BUCKET}/test_out.copy.txt.gz')

        with hadoop_open(f'{Tests.BUCKET}/test_out.copy.txt.gz') as f:
        ...
        with local_fs.open(resource('randomBytes'), buffer_size=100) as f:
            with hadoop_open(f'{Tests.BUCKET}/randomBytesOut', 'w', buffer_size=2**18) as out:
                b = f.read()
                out.write(b)

        with hadoop_open(f'{Tests.BUCKET}/randomBytesOut', buffer_size=2**18) as f:
            b2 = f.read()

        self.assertEqual(b, b2)

        with self.assertRaises(Exception):
            hadoop_open(f'{Tests.BUCKET}/randomBytesOut', 'xb')

    def test_hadoop_exists(self):
        with hadoop_open(f'{Tests.BUCKET}/test_exists.txt', 'w') as f:
            f.write("HELLO WORLD")

        r_exists = f'{Tests.BUCKET}/test_exists.txt'
        r_not_exists = f'{Tests.BUCKET}/not_exists.txt'
        self.assertTrue(hl.hadoop_exists(r_exists))
        self.assertFalse(hl.hadoop_exists(r_not_exists))

    def test_hadoop_is_file(self):
        a_file = f'{Tests.BUCKET}/test_hadoop_is_file.txt'
        with hadoop_open(a_file, 'w') as f:
            f.write("HELLO WORLD")

        self.assertTrue(hl.hadoop_is_file(a_file))
        self.assertFalse(hl.hadoop_is_file(f'{Tests.BUCKET}/'))
        self.assertFalse(hl.hadoop_is_file(f'{Tests.BUCKET}/invalid-path'))

    def test_hadoop_stat(self):
        stat1 = hl.hadoop_stat(f'{Tests.BUCKET}/')
        self.assertEqual(stat1['is_dir'], True)

        stat2 = hl.hadoop_stat(f'{Tests.BUCKET}/test_out.copy.txt.gz')
