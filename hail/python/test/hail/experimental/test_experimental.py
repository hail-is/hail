...
class Tests(unittest.TestCase):
    ...
    def test_define_function(self):
        ...
        f2 = hl.experimental.define_function(lambda a, b: (a + 7) * b, hl.tint32, hl.tint32)
    ...
    def test_mt_full_outer_join_self(self):
        ...
        assert (
            jmt.filter_rows(hl.is_defined(jmt.left_row) & hl.is_defined(jmt.right_row)).count_rows() == mt.count_rows()
        )
        assert (
            jmt.filter_entries(hl.is_defined(jmt.left_entry) & hl.is_defined(jmt.right_entry)).entries().count()
            == mt.entries().count()
        )
    ...
    def test_loop(self):
        def triangle_with_ints(n):
            return hl.experimental.loop(lambda f, x, c: hl.if_else(x > 0, f(x - 1, c + x), c), hl.tint32, n, 0)
        ...
        fails_typecheck("branch condition", lambda f, x: hl.if_else(f(x) == 0, x, 1))
        fails_typecheck("Type error", lambda f, x: hl.if_else(x == 0, f("foo"), 1))
    ...
    def test_loop_with_struct_of_strings(self):
        def loop_func(recur_f, my_struct):
            return hl.if_else(
                hl.len(my_struct.s1) > hl.len(my_struct.s2),
                my_struct,
        ...
        assert hl.eval(
            hl.experimental.loop(loop_func, hl.tstruct(s1=hl.tstr, s2=hl.tstr), initial_struct)
        ) == hl.Struct(s1="abcd", s2="gfe")
