    ...
    def value_irs_env(self):
        return {
            ...
            "whitenStream": hl.tstream(
                hl.tstruct(
                    prevWindow=hl.tndarray(hl.tfloat64, 2),
                    newChunk=hl.tndarray(hl.tfloat64, 2),
                )
            ),
    ...
    def value_irs(self):
        ...
        value_irs = [
            ...
            ir.ArraySort(
                ...
                "l",
                "r",
                ir.ApplyComparisonOp("LT", ir.Ref("l", hl.tint32), ir.Ref("r", hl.tint32)),
            ),
            ir.ArraySort(
                ir.ToStream(aa),
                "l",
                "r",
                ir.ApplyComparisonOp(
                    "LT",
                    ir.Ref("l", hl.tarray(hl.tint32)),
                    ir.Ref("r", hl.tarray(hl.tint32)),
                ),
            ),
            ir.ToSet(st),
            ir.ToDict(da),
            ...
            aggregate(
                ir.AggExplode(
                    ir.StreamRange(ir.I32(0), ir.I32(2), ir.I32(1)),
                    "x",
                    ir.I32(0),
                    False,
                )
            ...
            ir.TableAggregate(
                table,
                ir.MakeStruct([("foo", ir.ApplyAggOp("Collect", [], [ir.I32(0)]))]),
            ),
            ir.TableWrite(
                table,
                ir.TableNativeWriter(new_temp_file(), False, True, "fake_codec_spec$$"),
            ),
            ir.TableWrite(
                table,
            ...
            ir.MatrixWrite(
                matrix_read,
                ir.MatrixNativeWriter(new_temp_file(), False, False, "", None, None),
            ),
            ir.MatrixWrite(
                matrix_read,
            ...
            ir.MatrixWrite(
                matrix_read,
                ir.MatrixVCFWriter(new_temp_file(), None, ir.ExportType.CONCATENATED, None, False),
            ),
...
class TableIRTests(unittest.TestCase):
    def table_irs(self):
        ...
        table_read = ir.TableRead(
            ir.TableNativeReader(resource("backward_compatability/1.1.0/table/0.ht"), None, False),
            False,
        )
        ...
        table_irs = [
            ...
            ir.TableToTableApply(
                table_read,
                {"name": "TableFilterPartitions", "parts": [0], "keep": True},
            ),
            ir.BlockMatrixToTableApply(
                block_matrix_read,
                aa,
                {"name": "PCRelate", "maf": 0.01, "blockSize": 4096},
            ),
            ...
            ir.TableMapPartitions(
                table_read,
                "glob",
                "rows",
                ir.Ref("rows", hl.tstream(table_read_row_type)),
                0,
                1,
            ),
            ir.TableGen(
...
class MatrixIRTests(unittest.TestCase):
    def matrix_irs(self):
        ...
        matrix_irs = [
            ...
            ir.MatrixToMatrixApply(
                matrix_read,
                {"name": "MatrixFilterPartitions", "parts": [0], "keep": True},
            ),
            ir.MatrixRename(
                matrix_read,
...
class BlockMatrixIRTests(unittest.TestCase):
    def blockmatrix_irs(self):
        ...
        add_two_bms = ir.BlockMatrixMap2(
            ...
            "l",
            "r",
            ir.ApplyBinaryPrimOp("+", ir.Ref("l", hl.tfloat64), ir.Ref("r", hl.tfloat64)),
            "Union",
        )
        negate_bm = ir.BlockMatrixMap(
            read,
            "element",
            ir.ApplyUnaryPrimOp("-", ir.Ref("element", hl.tfloat64)),
            False,
        )
        sqrt_bm = ir.BlockMatrixMap(
            read,
            "element",
            hl.sqrt(construct_expr(ir.Ref("element", hl.tfloat64), hl.tfloat64))._ir,
            False,
        )
    ...
    def test_value_same_after_parsing(self):
        ...
        for t, v in self.values():
            ...
            map_globals_ir = ir.TableMapGlobals(
                range,
                ir.InsertFields(ir.Ref("global", range.typ.global_type), [("foo", row_v)], None),
            )
...
class CSETests(unittest.TestCase):
    ...
    def test_cse_debug(self):
        ...
        hl.eval(
            hl.tuple([
                hl.if_else(dlen[0] > 1, 1, 1),
                hl.if_else(dlen[0] > 1, hl.nd.array([0]), dlen),
            ])
        )
    ...
    def test_refs(self):
        ...
        x = ir.TableMapRows(
            table,
            ir.MakeStruct([("foo", ir.GetField(ref, "idx")), ("bar", ir.GetField(ref, "idx"))]),
        )
        expected = (
