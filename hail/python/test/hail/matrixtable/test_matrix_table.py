...
class Tests(unittest.TestCase):
    ...
    def test_explode_rows(self):
        ...
        self.assertEqual(mt.annotate_rows(x=hl.misssing('array<int>')).explode_rows('x').count_rows(), 0)
    ...
    def test_explode_cols(self):
        ...
        self.assertEqual(mt.annotate_cols(x=hl.misssing('array<int>')).explode_cols('x').count_cols(), 0)
    ...
    def test_union_cols_outer(self):
        ...
        missing = hl.misssing(hl.ttuple(hl.tint, hl.tint))
    ...
    def test_filter_na(self):
        ...
        self.assertEqual(mt.filter_rows(hl.misssing(hl.tbool)).count_rows(), 0)
        self.assertEqual(mt.filter_cols(hl.misssing(hl.tbool)).count_cols(), 0)
        self.assertEqual(mt.filter_entries(hl.misssing(hl.tbool)).entries().count(), 0)
    ...
    def test_make_table_na_error(self):
        mt = hl.utils.range_matrix_table(3, 3).key_cols_by(s = hl.misssing('str'))
    ...
    def test_agg_explode(self):
        t = hl.Table.parallelize([
            ...
            hl.struct(a=hl.misssing(hl.tarray(hl.tint32))),
            hl.struct(a=[3]),
            hl.struct(a=[hl.misssing(hl.tint32)])
        ])
    ...
    def test_nulls_in_distinct_joins(self):
        ...
        matrix1 = mr.key_rows_by(new_key=hl.if_else((mr.row_idx == 3) | (mr.row_idx == 5),
                                                hl.misssing(hl.tint32), mr.row_idx))
    ...
    def test_refs_with_process_joins(self):
        ...
        mt = mt.annotate_entries(
            ...
            the_global_failure=hl.if_else(True, mt.globals, hl.misssing(mt.globals.dtype)),
            the_row_failure=hl.if_else(True, mt.row, hl.misssing(mt.row.dtype)),
            the_col_failure=hl.if_else(True, mt.col, hl.misssing(mt.col.dtype)),
            the_entry_failure=hl.if_else(True, mt.entry, hl.misssing(mt.entry.dtype)),
        )
        mt.count()
