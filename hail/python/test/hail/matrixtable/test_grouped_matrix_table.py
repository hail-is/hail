...
class Tests(unittest.TestCase):
    ...
    def get_groupable_matrix():
        ...
        rt = rt.annotate_rows(group1=rt['row_idx'] % 6, group2=hl.Struct(a=rt['row_idx'] % 6, b="foo"))
        rt = rt.annotate_cols(group3=rt['col_idx'] % 6, group4=hl.Struct(a=rt['col_idx'] % 6, b="foo"))
        return rt.annotate_entries(
            c=rt['row_idx'],
            d=rt['col_idx'],
            e="foo",
            f=rt['group1'],
    ...
    def test_errors_caught_correctly(self):
        ...
        self.assertRaises(
            NotImplementedError, a.aggregate_cols, bar=hl.agg.sum(mt['col_idx'])
        )  # cannot aggregate cols when grouped by rows

        self.assertRaises(
            ExpressionException, a.aggregate_entries, group3=hl.agg.sum(mt['c'])
        ...
        self.assertRaises(
            ExpressionException, a.aggregate_rows, group3=hl.agg.sum(mt['row_idx'])
        )  # duplicate column field
        self.assertRaises(
            ExpressionException, a.aggregate_rows, group5=hl.agg.sum(mt['row_idx'])
        )  # duplicate row field
        self.assertRaises(
            ExpressionException, a.aggregate_rows, foo=hl.agg.sum(mt['row_idx'])
        )  # duplicate globals field
        self.assertRaises(
            ExpressionException, a.aggregate_rows, bar=mt['row_idx'] + hl.agg.sum(mt['row_idx'])
        )  # expression has to have global indices
        self.assertRaises(
            ExpressionException, a.aggregate_rows, bar=mt['col_idx'] + hl.agg.sum(mt['row_idx'])
        )  # expression has to have global indices
        self.assertRaises(
            ExpressionException, a.aggregate_rows, bar=hl.agg.sum(mt['c'])
        )  # aggregation scope is rows only - entry field
        self.assertRaises(
            ExpressionException, a.aggregate_rows, bar=hl.agg.sum(mt['col_idx'])
        )  # aggregation scope is rows only - column field

        b = mt.group_cols_by(group5=(mt['group4']['a'] + 1))
        self.assertRaises(
            NotImplementedError, b.aggregate_rows, bar=hl.agg.sum(mt['row_idx'])
        )  # cannot aggregate rows when grouped by cols

        self.assertRaises(ExpressionException, b.aggregate_entries, group1=hl.agg.sum(mt['c']))  # duplicate row field
        self.assertRaises(
            ExpressionException, b.aggregate_entries, group5=hl.agg.sum(mt['c'])
        )  # duplicate column field
        self.assertRaises(ExpressionException, b.aggregate_entries, foo=hl.agg.sum(mt['c']))  # duplicate globals field

        self.assertRaises(
            ExpressionException, b.aggregate_cols, group1=hl.agg.sum(mt['col_idx'])
        )  # duplicate row field
        self.assertRaises(
            ExpressionException, b.aggregate_cols, group5=hl.agg.sum(mt['col_idx'])
        )  # duplicate column field
        self.assertRaises(
            ExpressionException, b.aggregate_cols, foo=hl.agg.sum(mt['col_idx'])
        )  # duplicate globals field
        self.assertRaises(
            ExpressionException, b.aggregate_cols, bar=mt['col_idx'] + hl.agg.sum(mt['col_idx'])
        )  # expression has to have global indices
        self.assertRaises(
            ExpressionException, b.aggregate_cols, bar=mt['row_idx'] + hl.agg.sum(mt['col_idx'])
        )  # expression has to have global indices
        self.assertRaises(
            ExpressionException, b.aggregate_cols, bar=hl.agg.sum(mt['c'])
        )  # aggregation scope is cols only - entry field
        self.assertRaises(
            ExpressionException, b.aggregate_cols, bar=hl.agg.sum(mt['row_idx'])
        )  # aggregation scope is cols only - row field
    ...
    def test_joins_work_correctly(self):
        ...
        col_result = (
            mt.group_cols_by(group=mt2.cols()[mt.col_idx].col_idx2 < 2)
        ...
        row_result = (
            mt.group_rows_by(group=mt2.rows()[mt.row_idx].row_idx2 < 2)
            .aggregate(sum=hl.agg.sum(mt2[mt.row_idx, mt.col_idx].x + mt.glob) + mt.glob - 15)
            .drop('c1')
        )
    ...
    def test_group_rows_by_aggregate(self):
        ...
        row_result = (
            ...
            .aggregate_entries(
                sum=hl.agg.sum(mt2[mt.row_idx, mt.col_idx].x + mt.glob) + mt.glob - 15 - mt.col_idx
            )  # tests fixed indices
    ...
    def test_group_cols_by_aggregate(self):
        ...
        col_result = (
            ...
            .aggregate_entries(
                sum=hl.agg.sum(mt2[mt.row_idx, mt.col_idx].x + mt.glob) + mt.glob - 15 - mt.row_idx
            )  # tests fixed indices
            .aggregate_entries(x=5)
            .result()
        )
