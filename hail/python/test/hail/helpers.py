...
def create_all_values():
    return hl.struct(
        f32=hl.float32(3.14),
        i64=hl.int64(-9),
        m=hl.missingl(hl.tfloat64),
        astruct=hl.struct(a=hl.missingl(hl.tint32), b=5.5),
        mstruct=hl.missingl(hl.tstruct(x=hl.tint32, y=hl.tstr)),
        aset=hl.set(['foo', 'bar', 'baz']),
        mset=hl.missingl(hl.tset(hl.tfloat64)),
        d=hl.dict({hl.array(['a', 'b']): 0.5, hl.array(['x', hl.missingl(hl.tstr), 'z']): 0.3}),
        md=hl.missingl(hl.tdict(hl.tint32, hl.tstr)),
        h38=hl.locus('chr22', 33878978, 'GRCh38'),
        ml=hl.missingl(hl.tlocus('GRCh37')),
        i=hl.interval(
            hl.locus('1', 999),
            hl.locus('1', 1001)),
        c=hl.call(0, 1),
        mc=hl.missingl(hl.tcall),
        t=hl.tuple([hl.call(1, 2, phased=True), 'foo', hl.missingl(hl.tstr)]),
        mt=hl.missingl(hl.ttuple(hl.tlocus('GRCh37'), hl.tbool)),
