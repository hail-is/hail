...
class VCFTests(unittest.TestCase):
    ...
    def test_import_vcf_has_good_error_message_when_info_fields_have_missing_elements(
        self,
    ):
    ...
    def test_import_vcf_array_elements_required_is_false_parses_info_fields_with_missing_elements(
        self,
    ):
        mt = hl.import_vcf(
            resource("missingInfoArray.vcf"),
    ...
    def test_import_vcf_missing_format_field_elements(self):
        mt = hl.import_vcf(
            resource("missingFormatArray.vcf"),
            reference_genome="GRCh37",
            array_elements_required=False,
        )
    ...
    def test_import_vcf_skip_invalid_loci(self):
        mt = hl.import_vcf(
            resource("skip_invalid_loci.vcf"),
            reference_genome="GRCh37",
    ...
    def test_export_vcf_invalid_info_types(self):
        ...
        ds = ds.annotate_rows(
            info=ds.info.annotate(
                arr_bool=hl.missing(hl.tarray(hl.tbool)),
...
"""
    ...
    def test_vcf_different_info_errors(self):
        with self.assertRaisesRegex(FatalError, "Check that all files have same INFO fields"):
            mt = hl.import_vcf([
                resource("different_info_test1.vcf"),
                resource("different_info_test2.vcf"),
            ])
            mt.rows()._force_count()
...
class PLINKTests(unittest.TestCase):
    ...
    def test_export_plink_struct_locus(self):
        ...
        mt = mt.key_rows_by(
            locus=hl.struct(contig=hl.str(mt.row_idx), position=mt.row_idx),
            alleles=["A", "T"],
        ).select_rows()
...
class BGENTests(unittest.TestCase):
    ...
    def test_parallel_import(self):
        mt = hl.import_bgen(
            resource("parallelBgenExport.bgen"),
            ["GT", "GP"],
    ...
    def test_import_bgen_row_fields(self):
        ...
        self.assertEqual(
            ...
            hl.tstruct(locus=hl.tlocus("GRCh37"), alleles=hl.tarray(hl.tstr), varid=hl.tstr),
        )
        rsid_only = hl.import_bgen(
            resource("example.8bits.bgen"),
            entry_fields=["dosage"],
            _row_fields=["rsid"],
        )
        self.assertEqual(
            rsid_only.row.dtype,
            hl.tstruct(locus=hl.tlocus("GRCh37"), alleles=hl.tarray(hl.tstr), rsid=hl.tstr),
        )
    ...
    def test_import_bgen_variant_filtering_from_literals(self):
        ...
        self.assertEqual(
            part_199.rows().key_by("locus", "alleles").select().collect(),
            expected_result,
        )
    ...
    def test_multiple_files_disjoint(self):
        ...
        with pytest.raises(
            FatalError,
            match="Each BGEN file must contain a region of the genome disjoint from other files",
        ):
    ...
    def test_multiple_references_throws_error(self):
        ...
        with pytest.raises(
            FatalError,
            match="Found multiple reference genomes were specified in the BGEN index files",
        ):
            hl.import_bgen(
...
class ImportMatrixTableTests(unittest.TestCase):
    ...
    def test_headers_same_len_diff_elem(self):
        with pytest.raises(
            ValueError,
            match="invalid header: expected elements to be identical for all input paths",
        ):
            hl.import_matrix_table(
    ...
    def test_wrong_entry_type(self):
        with pytest.raises(
            HailUserError,
            match="error parsing value into int32 at column id 'col000003'",
        ):
            hl.import_matrix_table(
                resource("samplenonintentries.txt"),
                row_fields={"f0": hl.tstr},
                row_key=["f0"],
            )._force_count_rows()
    ...
    def test_key_by_after_empty_key_import1(self):
        ...
        mt = hl.import_matrix_table(
            resource("sample2_va_nomulti.tsv"),
            row_fields=fields,
            row_key=[],
    ...
    def test_devilish_nine_separated_eight_missing_file(self):
        ...
        mt = hl.import_matrix_table(
            resource("import_matrix_table_devlish.ninesv"),
            row_fields=fields,
            row_key=["chr", ""],
            sep="9",
            missing="8",
