...
def test_ibd_default_arguments():
    ...
    for row in hail_results:
        key = (row.i, row.j)
        assert plink_results[key][0][0] == pytest.approx(row.ibd.Z0, rel=1e-4)
        assert plink_results[key][0][1] == pytest.approx(row.ibd.Z1, rel=1e-4)
        assert plink_results[key][0][2] == pytest.approx(row.ibd.Z2, rel=1e-4)
        assert plink_results[key][0][3] == pytest.approx(row.ibd.PI_HAT, rel=1e-4)
