...
def test_linear_skat_no_weights_R_truth():
    ...
    genotypes = [[0, 1, 0, 0, 0], [1, 0, 0, 0, 0], [0, 1, 2, 0, 2], [1, 0, 0, 2, 1]]
    covariates = [[1, 2], [3, 4], [0, 9], [6, 1], [1, 1]]
...
def skat_dataset():
    ...
    phenotypes = hl.import_table(resource("skat.pheno"), types={"Pheno": hl.tfloat64}, missing="0").key_by("Sample")

    intervals = hl.import_locus_intervals(resource("skat.interval_list"))

    weights = hl.import_table(resource("skat.weights"), types={"locus": hl.tlocus(), "weight": hl.tfloat64}).key_by(
        "locus"
    )
...
def test_linear_skat_produces_same_results_as_old_scala_method():
    ...
    )  # TSV has 5 sigfigs, so we should match within 5e-5 relative


def test_skat_max_iteration_fails_explodes_in_37_steps():
    mt = hl.utils.range_matrix_table(3, 3)
    mt = mt.annotate_cols(y=hl.literal([1, 0, 1])[mt.col_idx])
    mt = mt.annotate_entries(x=hl.literal([[1, 0, 0], [10, 0, 0], [10, 5, 1]])[mt.row_idx])
    try:
        ht = hl.skat(
            hl.literal(0),
            mt.row_idx,
