...
def test_ndarray_ref():
    ...
    missing = hl.nd.array(hl.missingl(hl.tarray(hl.tint32)))
    ...
    assert_all_eval_to(
        ...
        (missing[1], None),
        (hl.nd.array([1, 2, 3])[hl.missingl(hl.tint32)], None),
        (h_cube[0, 0, hl.missingl(hl.tint32)], None)
    )
...
def test_ndarray_slice():
    ...
    assert hl.eval(flat[hl.missingl(hl.tint32):4:1]) is None
    assert hl.eval(flat[4:hl.missingl(hl.tint32)]) is None
    assert hl.eval(flat[4:10:hl.missingl(hl.tint32)]) is None
    assert hl.eval(rect_prism[:, :, 0:hl.missingl(hl.tint32):1]) is None
    assert hl.eval(rect_prism[hl.missingl(hl.tint32), :, :]) is None
...
def test_ndarray_eval():
    ...
    assert hl.eval(hl.nd.array(hl.missingl(hl.tarray(hl.tint32)))) is None
...
def test_ndarray_reshape():
    ...
    assert hl.eval(hl.missingl(hl.tndarray(hl.tfloat, 2)).reshape((4, 5))) is None
    assert hl.eval(hl.nd.array(hl.range(20)).reshape(
        hl.missingl(hl.ttuple(hl.tint64, hl.tint64)))) is None
...
def test_ndarray_map():
    ...
    assert hl.eval(hl.missingl(hl.tndarray(hl.tfloat, 1)).map(lambda x: x * 2)) is None
...
def test_ndarray_map2():
    ...
    # Missingness tests
    missing = hl.missingl(hl.tndarray(hl.tfloat64, 2))
...
def test_ndarray_transpose():
    ...
    assert hl.eval(hl.missingl(hl.tndarray(hl.tfloat, 1)).T) is None
...
def test_ndarray_matmul():
    ...
    assert hl.eval(hl.missingl(hl.tndarray(hl.tfloat64, 2)) @
                   hl.missingl(hl.tndarray(hl.tfloat64, 2))) is None
    assert hl.eval(hl.missingl(hl.tndarray(hl.tint64, 2)) @
                   hl.nd.array(np.arange(10).reshape(5, 2))) is None
    assert hl.eval(hl.nd.array(np.arange(10).reshape(5, 2)) @
                   hl.missingl(hl.tndarray(hl.tint64, 2))) is None
...
def test_ndarray_mixed():
    assert hl.eval(hl.missingl(hl.tndarray(hl.tint64, 2)).map(
        lambda x: x * x).reshape((4, 5)).T) is None
    assert hl.eval(
        (hl.nd.zeros((5, 10)).map(lambda x: x - 2) +
         hl.nd.ones((5, 10)).map(lambda x: x + 5)).reshape(hl.missingl(hl.ttuple(hl.tint64, hl.tint64))).T.reshape((10, 5))) is None
