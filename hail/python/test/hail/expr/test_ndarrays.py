...
def test_numpy_interop():
    ...
    assert hl.eval(np.array(v) * hl.literal(3)) == np.array([6, 9])
    assert hl.eval(hl.literal(3) * np.array(v)) == np.array([6, 9])

    assert hl.eval(np.array(v) * hl.nd.array(w)) == np.array([6, 15])
    assert hl.eval(hl.nd.array(w) * np.array(v)) == np.array([6, 15])

    assert hl.eval(np.array(v) + hl.literal(3)) == np.array([5, 6])
    assert hl.eval(hl.literal(3) + np.array(v)) == np.array([5, 6])

    assert hl.eval(np.array(v) + hl.nd.array(w)) == np.array([5, 8])
    assert hl.eval(hl.nd.array(w) + np.array(v)) == np.array([5, 8])

    assert hl.eval(np.array(v) @ hl.nd.array(w)) == 21
    assert hl.eval(hl.nd.array(v) @ np.array(w)) == 21

    assert hl.eval(np.array(a) @ hl.nd.array(b)) == np.array([[21]])
    assert hl.eval(hl.nd.array(a) @ np.array(b)) == np.array([[21]])

    assert hl.eval(hl.nd.array(b) @ np.array(a)) == \
        np.array([[6, 9], [10, 15]])
    assert hl.eval(np.array(b) @ hl.nd.array(a)) == \
        np.array([[6, 9], [10, 15]])
