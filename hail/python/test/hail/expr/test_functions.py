...
def test_deprecated_binom_test():
    binom = spst.binomtest(2, 10, 0.5, 'two-sided')
    assert hl.eval(hl.binom_test(2, 10, 0.5, 'two.sided')) == pytest.approx(binom.pvalue)


def test_binom_test():
    arglists = [[2, 10, 0.5, 'two-sided'], [4, 10, 0.5, 'less'], [32, 50, 0.4, 'greater']]
    for args in arglists:
        binom = spst.binomtest(*args)
        assert hl.eval(hl.binom_test(*args)) == pytest.approx(binom.pvalue), args
