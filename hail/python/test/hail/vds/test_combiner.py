...
def test_combiner_works():
    ...
    parts = [
        ...
        hl.Interval(
            start=hl.Locus('chr20', 18708367, reference_genome='GRCh38'),
            end=hl.Locus('chr20', 19776611, reference_genome='GRCh38'),
            includes_end=True,
        ),
        hl.Interval(
            start=hl.Locus('chr20', 19776612, reference_genome='GRCh38'),
    ...
    with hl.TemporaryDirectory() as tmpdir:
        ...
        hl.vds.new_combiner(
            temp_path=tmpdir, output_path=out, gvcf_paths=paths, intervals=parts, reference_genome='GRCh38'
        ).run()
        ...
        # see https://github.com/hail-is/hail/issues/14564 for why these assertions are here
        assert 'LGT' in comb.reference_data.entry
        assert 'LPGT' in comb.reference_data.entry
        assert comb.reference_data.LPGT.dtype == hl.tcall
...
def test_combiner_run():
    ...
    for input_gvcf, path in zip(input_paths[:2], final_paths_individual[:2]):
        combiner = hl.vds.new_combiner(
            output_path=path, intervals=parts, temp_path=tmpdir, gvcf_paths=[input_gvcf], reference_genome='GRCh38'
        )
        combiner.run()

    combiner = hl.vds.new_combiner(
        output_path=final_path_1,
        intervals=parts,
        temp_path=tmpdir,
        gvcf_paths=input_paths[2:],
        vds_paths=final_paths_individual[:2],
        reference_genome='GRCh38',
        branch_factor=2,
        batch_size=2,
    )
    ...
    combiner2 = hl.vds.new_combiner(
        output_path=final_path_2,
        intervals=parts,
        temp_path=tmpdir,
...
def test_ref_block_max_len_propagates_in_combiner():
    ...
    with hl.TemporaryDirectory() as tmpdir:
        ...
        hl.vds.new_combiner(
            output_path=final_path, temp_path=tmpdir, vds_paths=vds_paths, reference_genome='GRCh38'
        ).run()
