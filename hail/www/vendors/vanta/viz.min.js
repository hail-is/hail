...
this.elOnscreen=entries[0].intersectionRatio>intersectionThreshold;this.interval=1000/16;if(this.elOnscreen&&this.postInit==!1){try{window.requestAnimationFrame(()=>{this.init();this.postInit=!0;this.then=Date.now();this.listen()});window.requestAnimationFrame(this.animationLoop);setTimeout(()=>window.requestAnimationFrame(()=>this.el.style.opacity="1"),16)}catch(e){if(this.renderer&&this.renderer.domElement){this.el.removeChild(this.renderer.domElement)}
return}}};let observer=new IntersectionObserver(intersectionCallback,{threshold:intersectionThreshold});window.requestAnimationFrame(()=>observer.observe(this.renderer.domElement))}
...
animationLoop(){const now=Date.now();const delta=now-this.then;if(this.elOnscreen&&(!this.isScrolling||!this.postInit)){if(delta>this.interval){if(typeof this.onUpdate==="function"){this.onUpdate()}
...
this.then=now-(delta%this.interval);return this.req=window.setTimeout(this.animationLoop,!this.elOnscreen?1000:(this.postInit?24:0))}
