...
clearInterval(this.animationInterval)};let observer=new IntersectionObserver(intersectionCallback,{threshold:intersectionThreshold});window.requestAnimationFrame(()=>observer.observe(this.renderer.domElement))}
...
animationLoop(tInterval=24){this.animationInterval=window.setInterval(()=>{if(this.startedAnimation){return}
const now=Date.now();const delta=now-this.then;if(this.elOnscreen&&!this.isScrolling){if(delta>this.interval){this.onUpdate()
if(this.scene&&this.camera){this.renderer.render(this.scene,this.camera)}}}
if(this.hidden){this.startedAnimation=!0;const started=Date.now();window.requestAnimationFrame(()=>{this.el.style.opacity="1";this.hidden=!1;this.startedAnimation=!1;console.info("done",Date.now()-started)});this.then=now-1000-(delta%this.interval)}else{this.then=now-(delta%this.interval)}},tInterval)}
